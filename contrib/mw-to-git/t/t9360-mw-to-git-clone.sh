...
test_description='Test the shit Mediawiki remote helper: shit clone'
...
test_expect_success 'shit clone creates the expected shit log with multiple files' '
	wiki_reset &&
	...
	(
		cd mw_dir_2 &&
		shit log --format=%s Daddy.mw  >logDaddy.tmp &&
		shit log --format=%s Dj.mw >logDj.tmp
	) &&
...
test_expect_success 'shit clone creates only Main_Page.mw with an empty wiki' '
	wiki_reset &&
...
test_expect_success 'shit clone does not fetch a deleted page' '
	wiki_reset &&
...
test_expect_success 'shit clone works with page added' '
	wiki_reset &&
...
test_expect_success 'shit clone works with an edited page ' '
	wiki_reset &&
	...
	(
		cd mw_dir_6 &&
		shit log --format=%s HEAD^ Foo.mw > ../Foo.log
	) &&
...
test_expect_success 'shit clone works with several pages and some deleted ' '
	wiki_reset &&
...
test_expect_success 'shit clone works with one specific page cloned ' '
	wiki_reset &&
	...
	shit clone -c remote.origin.pages=namnam \
	...
	(
		...
		shit log --format=%s >log.tmp &&
		test_cmp msg.tmp log.tmp
	) &&
	wiki_check_content mw_dir_8/Namnam.mw Namnam
'

test_expect_success 'shit clone works with multiple specific page cloned ' '
	wiki_reset &&
	...
	wiki_delete_page namnam &&
	shit clone -c remote.origin.pages="foo bar nyancat namnam" \
...
test_expect_success 'Mediawiki-clone of several specific pages on wiki' '
	...
	shit clone -c remote.origin.pages="cloned_1 cloned_2 cloned_3" \
		mediawiki::'"$WIKI_URL"' mw_dir_10 &&
	test_diff_directories mw_dir_10 ref_page_10
'

test_expect_success 'shit clone works with the shallow option' '
	wiki_reset &&
	...
	(
		cd mw_dir_11 &&
		test $(shit log --oneline Nyan.mw | wc -l) -eq 1 &&
		test $(shit log --oneline Foo.mw | wc -l) -eq 1 &&
		test $(shit log --oneline Bar.mw | wc -l) -eq 1 &&
...
test_expect_success 'shit clone works with the shallow option with a delete page' '
	wiki_reset &&
...
test_expect_success 'Test of fetching a category' '
	...
	shit clone -c remote.origin.categories="Category" \
...
test_expect_success 'Test of resistance to modification of category on wiki for clone' '
	...
	wiki_editpage Normalone "this page wont be modified and will be on shit" false -c=Catone &&
	...
	shit clone -c remote.origin.categories="Catone" \
