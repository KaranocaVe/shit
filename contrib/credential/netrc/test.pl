...
BEGIN {
	# t-shit-credential-netrc.sh kicks off our testing, so we have to go
	# from there.
...
diag "Testing netrc file for a shithub.com entry\n";
$cred = run_credential(['-f', $netrc, 'get'],
		       { host => 'shithub.com' });

ok(scalar keys %$cred == 2, "Got 2 shithub keys");

is($cred->{password}, 'carolknows', "Got correct shithub password");
is($cred->{username}, 'carol', "Got correct shithub username");
...
diag 'Testing netrc file decryption by shit config gpg.program setting\n';
$cred = run_credential( ['-f', $netrcGpg, 'get']
                      , { host => 'shit-config-gpg' }
                      );

ok(scalar keys %$cred == 2, 'Got keys decrypted by shit config option');
