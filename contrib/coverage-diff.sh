...
# generated by the 'make' commands above with 'shit diff <version1> <version2>'
...
diff_lines () {
	perl -e '
		...
		while (<>) {
			...
			# Have we seen a hunk?  Ignore "diff --shit" etc.
...
files=$(shit diff --name-only "$V1" "$V2" -- \*.c)
...
(
	...
	do
		shit log --no-decorate --pretty=format:'%an      %h: %s' -1 $commit
		echo
	done
) | sort
