...
// The shit-remote-persistent-https binary speeds up SSL operations by running
// a daemon job that keeps a connection open to a shit server. This ensures the
// shit-remote-persistent-https--proxy is running and delegating execution
// to the shit-remote-http binary with the http_proxy set to the daemon job.
...
var (
	...
	proxyBin   = flag.String("proxy_bin", "shit-remote-persistent-https--proxy", "Path to the proxy binary")
...
func main() {
	...
	if *forceProxy || strings.HasSuffix(os.Args[0], "--proxy") {
		log.SetPrefix("shit-remote-persistent-https--proxy: ")
		proxy := &Proxy{
		...
		log.SetPrefix("shit-remote-persistent-https: ")
		client := &Client{
			ProxyBin: *proxyBin,
			Args:     flag.Args(),
		}
