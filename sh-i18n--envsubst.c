 ...
 * 67d0871a8c:gettext-runtime/src/envsubst.c from the gettext.shit
 * repository. It has been stripped down to only implement the
 * envsubst(1) features that we need in the shit-sh-i18n fallbacks.
 *
...
cmd_main (int argc, const char *argv[])
{
  ...
  switch (argc)
	...
	case 1:
	  ...
	  */
	case 2:
	  /* echo '$foo and $bar' | shit sh-i18n--envsubst --variables '$foo and $bar' */
	  all_variables = 0;
	  note_variables (argv[1]);
      subst_from_stdin ();
	  break;
	case 3:
	  /* shit sh-i18n--envsubst --variables '$foo and $bar' */
