...
struct fsmonitor_daemon_state {
	...
	struct strbuf path_shitdir_watch;
 ...
 * directory proper (inside the working directory and not ".shit" nor
 * inside of ".shit/").  That is, the client has read the index and is
 ...
 * file system changes within the ".shit/" directory (such as new loose
 * objects or packfiles).  So the client will only receive paths that
 * are classified as IS_WORKDIR_PATH.
 *
 * Note that ".shit" is usually a directory and is therefore inside
 * the cone of the FS watch that we have on the working directory root,
 * so we will also get FS events for disk activity on and within ".shit/"
 * that we need to respond to or filter from the client.
 *
 * But shit also allows ".shit" to be a *file* that points to a shitDIR
 ...
 * (external) shitDIR root.  (The latter is required because we put
 * cookie files inside it and use them to sync with the FS event
 * stream.)
 *
 * Note that in the context of this discussion, I'm using "shitDIR"
 * to only mean an external shitDIR referenced by a ".shit" file.
 *
 ...
 * here because ".shit/" is inside the working directory and the
 * (external) shitDIR is not, and therefore how we handle events may
 * vary slightly, so I have different enums for "IS...DOT_shit..." and
 * "IS...shitDIR...".
 *
 * The daemon uses the IS_DOT_shit and IS_shitDIR internally to mean the
 * exact ".shit" file/directory or shitDIR directory.  If the daemon
...
enum fsmonitor_path_type {
	...
	IS_DOT_shit,
	IS_INSIDE_DOT_shit,
	IS_INSIDE_DOT_shit_WITH_COOKIE_PREFIX,

	IS_shitDIR,
	IS_INSIDE_shitDIR,
	IS_INSIDE_shitDIR_WITH_COOKIE_PREFIX,
...
/*
 * Classify a pathname relative to a <shitdir> that is external to the
 * worktree directory.
 */
