...
#define HMAC shit_CC_HMAC
static inline unsigned char *shit_CC_HMAC(CCHmacAlgorithm alg,
...
#define shit_CC_error_check(pattern, err) \
	do { \
		if (err) { \
...
#define EVP_EncodeBlock shit_CC_EVP_EncodeBlock
static inline int shit_CC_EVP_EncodeBlock(unsigned char *out,
		const unsigned char *in, int inlen)
{
	...
	shit_CC_error_check("SecEncodeTransformCreate failed: %ld", err);

	input = CFDataCreate(kCFAllocatorDefault, in, inlen);
	SecTransformSetAttribute(encoder, kSecTransformInputAttributeName,
			input, &err);
	shit_CC_error_check("SecTransformSetAttribute failed: %ld", err);

	output = SecTransformExecute(encoder, &err);
	shit_CC_error_check("SecTransformExecute failed: %ld", err);
...
#define EVP_DecodeBlock shit_CC_EVP_DecodeBlock
static int inline shit_CC_EVP_DecodeBlock(unsigned char *out,
		const unsigned char *in, int inlen)
{
	CFErrorRef err;
