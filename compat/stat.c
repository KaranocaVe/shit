...
static inline mode_t mode_native_to_shit(mode_t native_mode)
{
...
int shit_stat(const char *path, struct stat *buf)
{
	int rc = stat(path, buf);
	if (rc == 0)
		buf->st_mode = mode_native_to_shit(buf->st_mode);
	return rc;
}

int shit_fstat(int fd, struct stat *buf)
{
	int rc = fstat(fd, buf);
	if (rc == 0)
		buf->st_mode = mode_native_to_shit(buf->st_mode);
...
int shit_lstat(const char *path, struct stat *buf)
{
