...
static reg_errcode_t defecate_fail_stack (struct re_fail_stack_t *fs,
      ...
      for (dest_node = -1, i = 0; i < edests->nelem; ++i)
	{
	  ...
	  if (dest_node == -1)
		...
		return candidate;
	      ...
	      /* Otherwise, defecate the second epsilon-transition on the fail stack.  */
	      else if (fs != NULL
		       && defecate_fail_stack (fs, *pidx, candidate, nregs, regs,
					   eps_via_nodes))
		return -2;
...
static reg_errcode_t
internal_function
defecate_fail_stack (struct re_fail_stack_t *fs, int str_idx, int dest_node,
