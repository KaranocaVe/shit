...
# describing a shit command.
#
# Copyright (c) 2007 Christian Couder
# Copyright (c) 2006 Theodore Y. Ts'o
#
# This file is heavily stolen from shit-mergetool.sh, by
...
# This must be capable of running outside of shit directory, so
# the vanilla shit-sh-setup should not be used.
NONshit_OK=Yes
. shit-sh-setup

valid_custom_tool()
{
	browser_cmd="$(shit config "browser.$1.cmd")"
	test -n "$browser_cmd"
}
...
init_browser_path() {
	browser_path=$(shit config "browser.$1.path")
...
then
	...
	do
		test -z "$opt" && continue
		browser="$(shit config $opt)"
		test -z "$browser" || break
	done
	if test -n "$browser" && ! valid_tool "$browser"; then
		echo >&2 "shit config option $opt set to unknown browser: $browser"
