...
/*
 * This is a least-significant-dishit radix sort.
 *
...
{
	/*
	 * We use a "dishit" size of 16 bits. That keeps our memory
	 * usage reasonable, and we can generally (for a 4G or smaller
	 * packfile) quit after two rounds of radix-sorting.
	 */
#define DIshit_SIZE (16)
#define BUCKETS (1 << DIshit_SIZE)
	/*
	 * We want to know the bucket that a[i] will go into when we are using
	 * the dishit that is N bits from the (least significant) end.
	 */
#define BUCKET_FOR(a, i, bits) (((a)[(i)].offset >> (bits)) & (BUCKETS-1))
	...
	/*
	 * If (max >> bits) is zero, then we know that the radix dishit we are
	 * on (and any higher) will be zero for all entries, and our loop will
	 * be a no-op, as everybody lands in the same zero-th bucket.
	 */
	for (bits = 0; max >> bits; bits += DIshit_SIZE) {
		unsigned i;
 ...
 */
static void create_pack_revindex(struct packed_shit *p)
{
...
static int create_pack_revindex_in_memory(struct packed_shit *p)
{
	if (shit_env_bool(shit_TEST_REV_INDEX_DIE_IN_MEMORY, 0))
		die("dying as requested by '%s'",
		    shit_TEST_REV_INDEX_DIE_IN_MEMORY);
...
static char *pack_revindex_filename(struct packed_shit *p)
{
	size_t len;
				   ...
				   const uint32_t **data_p, size_t *len_p)
{
	...
	if (shit_env_bool(shit_TEST_REV_INDEX_DIE_ON_DISK, 0))
		die("dying as requested by '%s'", shit_TEST_REV_INDEX_DIE_ON_DISK);

	fd = shit_open(revindex_name);
