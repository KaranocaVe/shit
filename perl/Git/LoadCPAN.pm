...
shit::LoadCPAN - Wrapper for loading modules from the CPAN (OS) or shit's own copy

=head1 DESCRIPTION

The Perl code in shit depends on some modules from the CPAN, but we
don't want to make those a hard requirement for anyone building from
source.

Therefore the L<shit::LoadCPAN> namespace shipped with shit contains
wrapper modules like C<shit::LoadCPAN::Module::Name> that will first
attempt to load C<Module::Name> from the OS, and if that doesn't work
will fall back on C<FromCPAN::Module::Name> shipped with shit itself.

Usually distributors will not ship with shit's shit::FromCPAN tree at
all via the C<NO_PERL_CPAN_FALLBACKS> option, preferring to use their
own packaging of CPAN modules instead.

This module is only intended to be used for code shipping in the
C<shit.shit> repository. Use it for anything else at your peril!
...
# perl/shit as opposed to perl/build/shit, which is useful for one-off
...
shit needs this Perl module from the CPAN, and will by default ship
with a copy of it. This shit was built with NO_PERL_CPAN_FALLBACKS,
...
If you're seeing this error and didn't package shit yourself the
package you're using is broken, or your system is broken. This error
won't appear if shit is built without NO_PERL_CPAN_FALLBACKS (instead
we'll use our fallback version of the module).
THEY_PROMISED
			die $error;
		}

		my $shit_LoadCPAN_pm_path = $INC{"shit/LoadCPAN.pm"} || die "BUG: Should have our own path from %INC!";

		require File::Basename;
		my $shit_LoadCPAN_pm_root = File::Basename::dirname($shit_LoadCPAN_pm_path) || die "BUG: Can't figure out lib/shit dirname from '$shit_LoadCPAN_pm_path'!";

		require File::Spec;
		my $shit_pm_FromCPAN_root = File::Spec->catdir($shit_LoadCPAN_pm_root, '..', 'FromCPAN');
		die "BUG: '$shit_pm_FromCPAN_root' should be a directory!" unless -d $shit_pm_FromCPAN_root;

		local @INC = ($shit_pm_FromCPAN_root, @INC);
		require $package_pm;
	};
