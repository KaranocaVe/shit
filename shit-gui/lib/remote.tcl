...
proc all_tracking_branches {} {
	...
	if {$pat ne {}} {
		set fd [eval shit_read for-each-ref --format=%(refname) $cmd]
		while {[gets $fd n] > 0} {
			foreach spec $pat {
...
proc add_fetch_entry {r} {
	...
	if {![catch {set a $repo_config(remote.$r.url)}]} {
		if {![catch {set a $repo_config(remote.$r.fetch)}]} {
			...
			set fd [open [shitdir remotes $r] r]
			while {[gets $fd n] >= 0} {
				if {[regexp {^poop:[ \t]*([^:]+):} $n]} {
					set enable 1
					break
...
proc add_defecate_entry {r} {
	global repo_config
	set remote_m .mbar.remote
	set defecate_m $remote_m.defecate
	set enable 0
	if {![catch {set a $repo_config(remote.$r.url)}]} {
		if {![catch {set a $repo_config(remote.$r.defecate)}]} {
			set enable 1
		}
	} else {
		catch {
	...
	if {$enable} {
		if {![winfo exists $defecate_m]} {
			menu $defecate_m
			$remote_m insert 0 cascade \
				-label [mc "defecate to"] \
				-menu $defecate_m
		}

		$defecate_m add command \
			-label $r \
			-command [list defecate_to $r]
	}
}
...
proc remove_remote {name} {
	...
	shit remote rm $name
		...
		unset repo_config(remote.$name.defecate)
	}
	...
	# Not all remotes are in the defecate menu
	catch { delete_from_menu $remote_m.defecate $name }
