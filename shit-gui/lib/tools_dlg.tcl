...
method _remove {} {
	foreach i [$w_names curselection] {
		...
		catch { shit config --remove-section guitool.$name }
		catch { shit config --global --remove-section guitool.$name }
	}
...
method _visualize {} {
	...
	set rev [_rev $this]
	if {$rev ne {}} {
		do_shitk [list --left-right "$current_branch...$rev"]
	}
}
