...
proc show_other_diff {path w m cont_info} {
	...
	# - shit won't give us the diff, there's nothing to compare to!
	#
	if {$m eq {_O}} {
		set max_sz 100000
		set type unknown
...
proc start_show_diff {cont_info {add_opts {}}} {
	...
	set diff_active 1
	set current_diff_header {}
	set conflict_size [shitattr $path conflict-marker-size 7]
	...
	if {![is_config_false gui.textconv] && [shit-version >= 1.6.1]} {
	...
	if {$is_submodule_diff && [shit-version < "1.6.6"]} {
		if {$w eq $ui_index} {
	...
	if {[catch {set fd [eval shit_read --nice $cmd]} err]} {
		set diff_active 0
		unlock_index
		ui_status [mc "Unable to display %s" [escape_path $path]]
...
proc read_diff {fd conflict_size cont_info} {
	...
	while {[gets $fd line] >= 0} {
		...
		if {   [string match {diff --shit *}      $line]
...
proc apply_or_revert_hunk {x y revert} {
	...
	if {[catch {
		...
		set p [eval shit_write $apply_cmd]
