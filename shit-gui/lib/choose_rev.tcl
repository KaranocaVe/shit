...
constructor _new {path unmerged_only title} {
	...
	append fmt {]}
	set all_refn [list]
	set fr_fd [shit_read for-each-ref \
		--tcl \
		--sort=-taggerdate \
		--format=$fmt \
		refs/heads \
	...
	if {$unmerged_only} {
		set fr_fd [shit_read rev-list --all ^$::HEAD]
...
method get_commit {} {
	...
	return [shit rev-parse --verify "$e^0"]
}
...
method _reflog_last {name} {
	...
	set last {}
	if {[catch {set last [file mtime [shitdir $name]]}]
	&& ![catch {set g [open [shitdir logs $name] r]}]} {
