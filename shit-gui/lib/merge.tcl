...
Another shit program has modified this repository since the last scan.  A rescan must be performed before a merge can be performed.
...
method _start {} {
	...
	set fh [open [shitdir FETCH_HEAD] w]
	...
	if {[shit-version >= "2.5.0"]} {
		set cmd [list shit merge --strategy=recursive FETCH_HEAD]
	} else {
		set cmd [list shit]
		lappend cmd merge
		lappend cmd --strategy=recursive
		lappend cmd [shit fmt-merge-msg <[shitdir FETCH_HEAD]]
		lappend cmd HEAD
		lappend cmd $name
	}
...
Continue with resetting the current changes?"]
	...
	if {[ask_popup $op_question] eq {yes}} {
		set fd [shit_read --stderr read-tree --reset -u -v HEAD]
...
proc _reset_wait {fd status_bar_operation} {
	...
	if {[eof $fd]} {
		...
		catch {file delete [shitdir rr-cache MERGE_RR]}
		catch {file delete [shitdir MERGE_RR]}
