...
method _load {jump} {
	...
	# shit-blame output and with Tk's text widget.
	#
	...
	set do_textconv 0
	if {![is_config_false gui.textconv] && [shit-version >= 1.7.2]} {
	...
	if {$commit eq {}} {
		...
		if {$do_textconv ne 0} {
			set fd [shit_read cat-file --textconv "$commit:$path"]
		} else {
			set fd [shit_read cat-file blob "$commit:$path"]
		}
	}
	fconfigure $fd \
		-blocking 0 \
...
method _exec_blame {cur_w cur_d options cur_s} {
	...
	set fd [eval shit_read --nice blame $options]
	...
	if {$dat eq {}} {
		...
		if {[catch {set msg $header($cmit,message)}]} {
			set msg {}
				...
				set fd [shit_read cat-file commit $cmit]
...
method _shitkcommit {} {
	...
	if {$dat ne {}} {
		...
		if {$cmit eq $nullid} {
			if {[catch {set cmit [shit rev-parse --verify HEAD]} err]} {
...
method _blameparent {} {
	...
	if {$dat ne {}} {
		...
		if {[catch {set cparent [shit rev-parse --verify $parent_ref]} err]} {
		...
		if {[catch {set fd [eval shit_read $diffcmd]} err]} {
