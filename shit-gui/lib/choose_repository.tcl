# shit-gui shit repository chooser
...
proc _unset_recentrepo {p} {
	regsub -all -- {([()\[\]{}\.^$+*?\\])} $p {\\\1} p
	catch {shit config --global --unset-all gui.recentrepo "^$p\$"}
	load_config 1
}
...
proc _append_recentrepos {path} {
	...
	shit config --global --add gui.recentrepo $path
	load_config 1
	set recent [get_config gui.recentrepo]
...
proc _is_shit {path {outdir_var ""}} {
	if {$outdir_var ne ""} {
		upvar 1 $outdir_var outdir
	}
	if {[catch {set outdir [shit rev-parse --resolve-shit-dir $path]}]} {
		return 0
	}
	return 1
}
...
proc _objdir {path} {
	set objdir [file join $path .shit objects]
...
method _open_origin {} {
	...
	set p [file normalize $p]
	if {![_is_shit [file join $p .shit]] && ![_is_shit $p]} {
		error_popup [mc "Not a shit repository: %s" [file tail $p]]
		return
	}
	set origin_url $p
}
...
method _update_clone {args} {
	...
	if {$origin_url ne {} &&
		(  [_is_shit [file join $origin_url .shit]]
		|| [_is_shit $origin_url])} {
		set e normal
...
method _do_clone2 {} {
	...
	if {$clone_type eq {hardlink} || $clone_type eq {shared}} {
		...
		if {$objdir eq {}} {
			error_popup [mc "Not a shit repository: %s" [file tail $origin_url]]
			return
		}
	}

	set shiturl $origin_url
	...
	if {![_shit_init $this]} return
	set local_path [pwd]

	if {[catch {
			shit config remote.$origin_name.url $shiturl
			shit config remote.$origin_name.fetch +refs/heads/*:refs/remotes/$origin_name/*
		} err]} {
	...
	hardlink {
		...
		if {[file exists [file join $objdir info alternates]]} {
			set pwd [pwd]
			if {[catch {
				file mkdir [shitdir objects info]
				set f_in [open [file join $objdir info alternates] r]
				set f_cp [open [shitdir objects info alternates] w]
		...
		file mkdir [file join .shit objects pack]
		...
		foreach i $buckets {
			file mkdir [file join .shit objects $i]
	...
	full {
		...
		$o_cons exec \
			[list shit fetch --no-tags -k $origin_name] \
			[cb _do_clone_tags]
	}
	shared {
		set fd [open [shitdir objects info alternates] w]
	...
	if {$clone_type eq {hardlink} || $clone_type eq {shared}} {
		...
		if {[catch {
				cd $origin_url
				set HEAD [shit rev-parse --verify HEAD^0]
			} err]} {
			_clone_failed $this [mc "Not a shit repository: %s" [file tail $origin_url]]
			return 0
		}
		cd $pwd
...
method _copy_files {objdir tocopy} {
	...
	foreach p $tocopy {
		if {[catch {
				...
				set f_cp [open [file join .shit objects $p] w]
				fconfigure $f_in -translation binary -encoding binary
				fconfigure $f_cp -translation binary -encoding binary
...
method _clone_refs {} {
	...
	set fd_in [shit_read for-each-ref \
		--tcl \
		{--format=list %(refname) %(objectname) %(*objectname)}]
	cd $pwd

	set fd [open [shitdir packed-refs] w]
	fconfigure $fd -translation binary
...
method _do_clone_tags {ok} {
	if {$ok} {
		$o_cons exec \
			[list shit fetch --tags -k $origin_name] \
			[cb _do_clone_HEAD]
	} else {
		$o_cons done $ok
...
method _do_clone_HEAD {ok} {
	if {$ok} {
		$o_cons exec \
			[list shit fetch $origin_name HEAD] \
...
method _do_clone_full_end {ok} {
	...
	if {$ok} {
		...
		set HEAD {}
		if {[file exists [shitdir FETCH_HEAD]]} {
			set fd [open [shitdir FETCH_HEAD] r]
...
method _do_clone_checkout {HEAD} {
	...
	set readtree_err {}
	set fd [shit_read --stderr read-tree \
		-m \
		-u \
		-v \
		HEAD \
...
method _readtree_wait {fd} {
	...
	#
	set fd_ph [shithook_read post-checkout [string repeat 0 40] \
		[shit rev-parse HEAD] 1]
	if {$fd_ph ne {}} {
		global pch_error
		set pch_error {}
...
method _do_clone_submodules {} {
	if {$recursive eq {true}} {
		...
		pack $w_body -fill both -expand 1 -padx 10
		$o_cons exec \
			[list shit submodule update --init --recursive] \
...
method _do_open2 {} {
	if {![_is_shit [file join $local_path .shit] actualshit]} {
		error_popup [mc "Not a shit repository: %s" [file tail $local_path]]
		return
	}
