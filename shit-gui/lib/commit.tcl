# shit-gui misc. commit reading/writing support
...
"]
	...
	if {[catch {
			set name ""
			set email ""
			set fd [shit_read cat-file commit $curHEAD]
...
set shit_COMMITTER_IDENT {}

proc committer_ident {} {
	global shit_COMMITTER_IDENT

	if {$shit_COMMITTER_IDENT eq {}} {
		if {[catch {set me [shit var shit_COMMITTER_IDENT]} err]} {
			error_popup [strcat [mc "Unable to obtain your identity:"] "\n\n$err"]
			return {}
		}
		if {![regexp {^(.*) [0-9]+ [-+0-9]+$} \
			$me me shit_COMMITTER_IDENT]} {
			error_popup [strcat [mc "Invalid shit_COMMITTER_IDENT:"] "\n\n$me"]
			return {}
		}
	}

	return $shit_COMMITTER_IDENT
}
...
Another shit program has modified this repository since the last scan.  A rescan must be performed before another commit can be created.
...
"]
	...
	#
	set msg_p [shitdir shitGUI_EDITMSG]
	...
	#
	set fd_ph [shithook_read pre-commit]
	if {$fd_ph eq {}} {
...
proc commit_commitmsg {curHEAD msg_p} {
	...
	if {$is_detached
	    && ![file exists [shitdir rebase-merge head-name]]
...
Do you really want to proceed with your Commit?"]
	...
	#
	set fd_ph [shithook_read commit-msg $msg_p]
	if {$fd_ph eq {}} {
...
proc commit_writetree {curHEAD msg_p} {
	...
	set fd_wt [shit_read write-tree]
...
proc commit_committree {fd_wt curHEAD msg_p} {
	...
	if {$commit_type eq {normal}} {
		set fd_ot [shit_read cat-file commit $PARENT]
...
"]
	...
	lappend cmd <$msg_p
	if {[catch {set cmt_id [eval shit $cmd]} err]} {
		catch {file delete $msg_p}
	...
	if {[catch {
			shit update-ref -m $reflogm HEAD $cmt_id $curHEAD
		} err]} {
		catch {file delete $msg_p}
	...
	catch {file delete [shitdir MERGE_HEAD]}
	catch {file delete [shitdir MERGE_MSG]}
	catch {file delete [shitdir SQUASH_MSG]}
	catch {file delete [shitdir shitGUI_MSG]}
	catch {file delete [shitdir CHERRY_PICK_HEAD]}

	# -- Let rerere do its thing.
	#
	if {[get_config rerere.enabled] eq {}} {
		set rerere [file isdirectory [shitdir rr-cache]]
	} else {
	...
	#
	set fd_ph [shithook_read post-commit]
	if {$fd_ph ne {}} {
		global pch_error
		set pch_error {}
		fconfigure $fd_ph -blocking 0 -translation binary -eofchar {}
...
proc commit_author_ident {details} {
	...
	set old [array get env shit_AUTHOR_*]
	set env(shit_AUTHOR_NAME) $author(name)
	set env(shit_AUTHOR_EMAIL) $author(email)
	set env(shit_AUTHOR_DATE) $author(date)
	return $old
}
proc commit_author_reset {details} {
	global env
	unset env(shit_AUTHOR_NAME) env(shit_AUTHOR_EMAIL) env(shit_AUTHOR_DATE)
	if {$details ne {}} {
