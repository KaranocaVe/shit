# shit-gui remote adding support
...
constructor dialog {} {
	...
	${NS}::radiobutton $w.action.defecate \
		-text [mc "Initialize Remote Repository and defecate"] \
		-value defecate \
		-variable @opt_action
	pack $w.action.defecate -anchor nw
...
method _add {} {
	...
	# that should be ok.
	if {[catch {shit check-ref-format "remotes/$name"}]} {
		tk_messageBox \
			-icon error \
			-type ok \
			-title [wm title $w] \
	...
	fetch {
		...
		console::exec $c [list shit fetch $name]
	}
	defecate {
		set cmds [list]

		# Parse the location
		if { [regexp {(?:shit\+)?ssh://([^/]+)(/.+)} $location xx host path]
		     || [regexp {([^:][^:]+):(.+)} $location xx host path]} {
			set ssh ssh
			if {[info exists env(shit_SSH)]} {
				set ssh $env(shit_SSH)
			}
			lappend cmds [list exec $ssh $host mkdir -p $location && shit --shit-dir=$path init --bare]
		} elseif { ! [regexp {://} $location xx] } {
			lappend cmds [list exec mkdir -p $location]
			lappend cmds [list exec shit --shit-dir=$location init --bare]
		} else {
			tk_messageBox \
				-icon error \
				-type ok \
		...
		lappend cmds [list exec shit defecate -v --all $name]
