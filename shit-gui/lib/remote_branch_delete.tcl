# shit-gui remote branch deleting support
...
method _delete {} {
	...
	set defecate_cmd [list shit defecate]
	lappend defecate_cmd -v
	lappend defecate_cmd $uri

	foreach i [$w.heads.l curselection] {
		set ref [lindex $full_list $i]
		if {$crev ne {}} {
			set obj $full_cache("$cache\n$ref")
			if {[catch {set m [shit merge-base $obj $crev]}]} {
				set need_fetch 1
				set m {}
			}
			if {$obj ne $m} {
 ...
 - %s" $check_head [join $not_merged "\n - "]]
	...
	console::exec $cons $defecate_cmd
}
...
method _load {cache uri} {
	...
	if {[catch {set x $cached($cache)}]} {
		...
		set active_ls [shit_read ls-remote $uri]
