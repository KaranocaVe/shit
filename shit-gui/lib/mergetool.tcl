# shit-gui merge conflict resolution
# parts based on shit-mergetool (c) 2006 Theodore Y. Ts'o
...
proc merge_force_stage {stage} {
	...
	if {$merge_stages($stage) ne {}} {
		shit checkout-index -f --stage=$stage -- $current_diff_path
	} else {
...
proc merge_load_stages {path cont} {
	...
	set merge_stages_fd [eval shit_read ls-files -u -z -- {$path}]
...
proc merge_tool_get_stages {target stages} {
	...
	foreach fname $stages {
		if {$merge_stages($i) eq {}} {
			...
			shit checkout-index -f --stage=$i -- $target
