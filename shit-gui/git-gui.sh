...
# Tcl ignores the next line -*- tcl -*- \
 ...
 || test "z$*" = z--version; \
 then \
	echo 'shit-gui version @@shitGUI_VERSION@@'; \
	exit; \
 fi; \
 argv0=$0; \
 exec wish "$argv0" -- "$@"

set appvers {@@shitGUI_VERSION@@}
...
proc _which {what args} {
	...
	if {$_search_path eq {}} {
		if {[is_Windows]} {
			set shitguidir [file dirname [info script]]
			regsub -all ";" $shitguidir "\\;" shitguidir
			set env(PATH) "$shitguidir;$env(PATH)"
...
if { [info exists ::env(shit_GUI_LIB_DIR) ] } {
	set oguilib $::env(shit_GUI_LIB_DIR)
} else {
	set oguilib {@@shitGUI_LIBDIR@@}
}
set oguirel {@@shitGUI_RELATIVE@@}
if {$oguirel eq {1}} {
	set oguilib [file dirname [file normalize $argv0]]
	if {[file tail $oguilib] eq {shit-core}} {
		set oguilib [file dirname $oguilib]
	}
	set oguilib [file dirname $oguilib]
	set oguilib [file join $oguilib share shit-gui lib]
...
if {![catch {set _verbose $env(shitGUI_VERBOSE)}]} {
	unset _verbose
...
proc shitdir {args} {
	...
	return [eval [list file join $_shitdir] $args]
}

proc shitexec {args} {
	global _shitexec
	if {$_shitexec eq {}} {
		if {[catch {set _shitexec [shit --exec-path]} err]} {
			error "shit not installed?\n\n$err"
		}
		set _shitexec [file normalize $_shitexec]
	}
	if {$args eq {}} {
		return $_shitexec
	}
	return [eval [list file join $_shitexec] $args]
}

proc shithtmldir {args} {
	global _shithtmldir
	if {$_shithtmldir eq {}} {
		if {[catch {set _shithtmldir [shit --html-path]}]} {
			# shit not installed or option not yet supported
			return {}
		}
		set _shithtmldir [file normalize $_shithtmldir]
	}
	if {$args eq {}} {
		return $_shithtmldir
	}
	return [eval [list file join $_shithtmldir] $args]
}
...
proc is_bare {} {
	...
	if {$_isbare eq {}} {
		if {[catch {
			set _bare [shit rev-parse --is-bare-repository]
			switch  -- $_bare {
			true { set _isbare 1 }
			false { set _isbare 0}
			default { throw }
			...
			if {[is_config_true core.bare]
				|| ($_shitworktree eq {}
					&& [lindex [file split $_shitdir] end] ne {.shit})} {
				set _isbare 1
			} else {
				set _isbare 0
			}
...
proc _shit_cmd {name} {
	global _shit_cmd_path

	if {[catch {set v $_shit_cmd_path($name)}]} {
		switch -- $name {
		  version   -
		--version   -
		--exec-path { return [list $::_shit $name] }
		}

		set p [shitexec shit-$name$::_search_exe]
		if {[file exists $p]} {
			set v [list $p]
		} elseif {[is_Windows] && [file exists [shitexec shit-$name]]} {
			# Try to determine what sort of magic will make
			# shit-$name go and do its thing, because native
			# Tcl on Windows doesn't know it.
			#
			set p [shitexec shit-$name]
			set f [open $p r]
			set s [gets $f]
			close $f
			...
			#!*perl   { set i perl   }
			#!*python { set i python }
			default   { error "shit-$name is not supported: $s" }
			}
			...
			if {$interp eq {}} {
				error "shit-$name requires $i (not in PATH)"
			}
			set v [concat [list $interp] [lrange $s 1 end] [list $p]]
		} else {
			# Assume it is builtin to shit somehow and we
			# aren't actually able to see a file for it.
			#
			set v [list $::_shit $name]
		}
		set _shit_cmd_path($name) $v
	}
	return $v
}
...
proc _lappend_nice {cmd_var} {
	...
	if {![info exists _nice]} {
		...
		if {[catch {exec $_nice shit version}]} {
			set _nice {}
		} elseif {[is_Windows] && [file dirname $_nice] ne [file dirname $::_shit]} {
			set _nice {}
		}
	}
	if {$_nice ne {}} {
...
proc shit {args} {
	set fd [eval [list shit_read] $args]
...
proc shit_read {args} {
	...
	set cmdp [_shit_cmd [lindex $args 0]]
	set args [lrange $args 1 end]

	return [_open_stdout_stderr [concat $opt $cmdp $args]]
}

proc shit_write {args} {
	set opt [list]
...
proc shithook_read {hook_name args} {
	set cmd [concat shit hook run --ignore-missing $hook_name -- $args 2>@1]
...
proc shitattr {path attr default} {
	if {[catch {set r [shit check-attr $attr -- $path]}]} {
		set r unspecified
	} else {
...
if {[is_Windows]} {
	wm iconbitmap . -default $oguilib/shit-gui.ico
	...
	if {![info exists env(DISPLAY)]} {
		...
		image create photo shitlogo -width 16 -height 16

		shitlogo put #33CC33 -to  7  0  9  2
		shitlogo put #33CC33 -to  4  2 12  4
		shitlogo put #33CC33 -to  7  4  9  6
		shitlogo put #CC3333 -to  4  6 12  8
		shitlogo put gray26  -to  4  9  6 10
		shitlogo put gray26  -to  3 10  6 12
		shitlogo put gray26  -to  8  9 13 11
		shitlogo put gray26  -to  8 11 10 12
		shitlogo put gray26  -to 11 11 13 14
		shitlogo put gray26  -to  3 12  5 14
		shitlogo put gray26  -to  5 13
		shitlogo put gray26  -to 10 13
		shitlogo put gray26  -to  4 14 12 15
		shitlogo put gray26  -to  5 15 11 16
		shitlogo redither

		image create photo shitlogo32 -width 32 -height 32
		shitlogo32 copy shitlogo -zoom 2 2

		wm iconphoto . -default shitlogo shitlogo32
	}
}
...
# TODO: this option should be added to the shit-config documentation
...
set _shit  [_which shit]
if {$_shit eq {}} {
	catch {wm withdraw .}
	tk_messageBox \
		-icon error \
		-type ok \
		-title [mc "shit-gui: fatal error"] \
		-message [mc "Cannot find shit in PATH."]
	exit 1
}
...
if {[catch {set _shit_version [shit --version]} err]} {
	catch {wm withdraw .}
	tk_messageBox \
		-icon error \
		-type ok \
		-title [mc "shit-gui: fatal error"] \
		-message "Cannot determine shit version:

$err

[appname] requires shit 1.5.0 or later."
	exit 1
}
if {![regsub {^shit version } $_shit_version {} _shit_version]} {
	catch {wm withdraw .}
	tk_messageBox \
		-icon error \
		-type ok \
		-title [mc "shit-gui: fatal error"] \
		-message [strcat [mc "Cannot parse shit version string:"] "\n\n$_shit_version"]
	exit 1
}
...
set _real_shit_version $_shit_version
set _shit_version [get_trimmed_version $_shit_version]

if {![regexp {^[1-9]+(\.[0-9]+)+$} $_shit_version]} {
	catch {wm withdraw .}
	if {[tk_messageBox \
		-icon warning \
		-type yesno \
		...
		-message [mc "shit version cannot be determined.

%s claims it is version '%s'.

%s requires at least shit 1.5.0 or later.

Assume '%s' is version 1.5.0?
" $_shit $_real_shit_version [appname] $_real_shit_version]] eq {yes}} {
		set _shit_version 1.5.0
	} else {
		exit 1
	}
}
unset _real_shit_version

proc shit-version {args} {
	global _shit_version

	switch [llength $args] {
	0 {
		return $_shit_version
	}

	2 {
		set op [lindex $args 0]
		set vr [lindex $args 1]
		set cm [package vcompare $_shit_version $vr]
		return [expr $cm $op 0]
	}
	...
	4 {
		...
		if {($type ne {proc} && $type ne {method})} {
			error "Invalid arguments to shit-version"
		}
...
if {[shit-version < 1.5]} {
	...
	tk_messageBox \
		...
		-type ok \
		-title [mc "shit-gui: fatal error"] \
		-message "[appname] requires shit 1.5.0 or later.

You are using [shit-version]:
...
if {[gets $fd] eq {# Autogenerated by shit-gui Makefile}} {
	set idx [list]
	while {[gets $fd n] >= 0} {
...
shit-version proc _parse_config {arr_name args} {
	>= 1.5.3 {
		upvar $arr_name arr
		array unset arr
		set buf {}
	...
	default {
		...
		array unset arr
			...
			set fd_rc [eval [list shit_read config --list] $args]
...
if {[regexp {^shit-(.+)$} [file tail $argv0] _junk subcommand]} {
	unset _junk
} else {
	set subcommand gui
}
...
if {![info exists env(SSH_ASKPASS)]} {
	set env(SSH_ASKPASS) [shitexec shit-gui--askpass]
}
...
set picked 0
if {[catch {
		set _shitdir $env(shit_DIR)
		set _prefix {}
		}]
	&& [catch {
		# beware that from the .shit dir this sets _shitdir to .
		# and _prefix to the empty string
		set _shitdir [shit rev-parse --shit-dir]
		set _prefix [shit rev-parse --show-prefix]
	} err]} {
	load_config 1
	apply_config
...
# we expand the _shitdir when it's just a single dot (i.e. when we're being
# run from the .shit dir itself) lest the routines to find the worktree
# get confused
if {$_shitdir eq "."} {
	set _shitdir [pwd]
}

if {![file isdirectory $_shitdir]} {
	catch {wm withdraw .}
	error_popup [strcat [mc "shit directory not found:"] "\n\n$_shitdir"]
	exit 1
}
# _shitdir exists, so try loading the config
load_config 0
apply_config

# v1.7.0 introduced --show-toplevel to return the canonical work-tree
if {[package vcompare $_shit_version 1.7.0] >= 0} {
	set _shitworktree [shit rev-parse --show-toplevel]
} else {
	# try to set work tree from environment, core.worktree or use
	# cdup to obtain a relative path to the top of the worktree. If
	# run from the top, the ./ prefix ensures normalize expands pwd.
	if {[catch { set _shitworktree $env(shit_WORK_TREE) }]} {
		set _shitworktree [get_config core.worktree]
		if {$_shitworktree eq ""} {
			set _shitworktree [file normalize ./[shit rev-parse --show-cdup]]
		}
	}
}
...
if {$_prefix ne {}} {
	...
	if {$_shitworktree eq {}} {
		set _shitworktree [file dirname $_shitdir]
	}
	if {[catch {cd $_shitworktree} err]} {
		catch {wm withdraw .}
		error_popup [strcat [mc "No working directory"] " $_shitworktree:\n\n$err"]
		exit 1
	}
	set _shitworktree [pwd]
}
set _reponame [file split [file normalize $_shitdir]]
if {[lindex $_reponame end] eq {.shit}} {
	set _reponame [lindex $_reponame end-1]
} else {
	set _reponame [lindex $_reponame end]
}

set env(shit_DIR) $_shitdir
set env(shit_WORK_TREE) $_shitworktree
...
proc repository_state {ctvar hdvar mhvar} {
	...
	load_current_branch
	if {[catch {set hd [shit rev-parse --verify HEAD]}]} {
		set hd {}
		set ct initial
		return
	}

	set merge_head [shitdir MERGE_HEAD]
...
proc rescan {after {honor_trustmtime 1}} {
	...
	if {!$::shitGUI_BCK_exists &&
		(![$ui_comm edit modified]
		|| [string trim [$ui_comm get 0.0 end]] eq {})} {
		if {[string match amend* $commit_type]} {
		} elseif {[load_message shitGUI_MSG utf-8]} {
	...
	if {$honor_trustmtime && $repo_config(gui.trustmtime) eq {true}} {
		...
		set fd_rf [shit_read update-index \
			-q \
			--unmerged \
			--ignore-missing \
			--refresh \
...
proc have_info_exclude {} {
	return [file readable [shitdir info exclude]]
}
...
proc rescan_stage2 {fd after} {
	...
	if {[package vcompare $::_shit_version 1.6.3] >= 0} {
		set ls_others [list --exclude-standard]
	} else {
		set ls_others [list --exclude-per-directory=.shitignore]
		if {[have_info_exclude]} {
			lappend ls_others "--exclude-from=[shitdir info exclude]"
		}
	...
	if {[shit-version >= "1.7.2"]} {
		set fd_di [shit_read diff-index --cached --ignore-submodules=dirty -z [PARENT]]
	} else {
		set fd_di [shit_read diff-index --cached -z [PARENT]]
	}
	set fd_df [shit_read diff-files -z]
	...
	if {[is_config_true gui.displayuntracked]} {
		set fd_lo [eval shit_read ls-files --others -z $ls_others]
...
proc run_prepare_commit_msg_hook {} {
	...
	# prepare-commit-msg requires PREPARE_COMMIT_MSG exist.  From shit-gui
	# it will be .shit/MERGE_MSG (merge), .shit/SQUASH_MSG (squash), or an
	# empty file but existent file.

	set fd_pcm [open [shitdir PREPARE_COMMIT_MSG] a]

	if {[file isfile [shitdir MERGE_MSG]]} {
		set pcm_source "merge"
		set fd_mm [open [shitdir MERGE_MSG] r]
		fconfigure $fd_mm -encoding utf-8
		puts -nonewline $fd_pcm [read $fd_mm]
		close $fd_mm
	} elseif {[file isfile [shitdir SQUASH_MSG]]} {
		set pcm_source "squash"
		set fd_sm [open [shitdir SQUASH_MSG] r]
	...
	set fd_ph [shithook_read prepare-commit-msg \
			[shitdir PREPARE_COMMIT_MSG] $pcm_source]
	if {$fd_ph eq {}} {
		catch {file delete [shitdir PREPARE_COMMIT_MSG]}
		return 0;
	}
...
proc do_shitk {revs {is_submodule false}} {
	global current_diff_path file_states current_diff_side ui_index
	global _shitdir _shitworktree

	# -- Always start shitk through whatever we were loaded with.  This
	#    lets us bypass using shell process on Windows systems.
	#
	set exe [_which shitk -script]
	set cmd [list [info nameofexecutable] $exe]
	if {$exe eq {}} {
		error_popup [mc "Couldn't find shitk in PATH"]
	} else {
		global env
		...
		if {!$is_submodule} {
			...
			# shit_DIR and shit_WORK_TREE for the submodule are not the ones
			# we've been using for the main repository, so unset them.
			# TODO we could make life easier (start up faster?) for shitk
			# by setting these to the appropriate values to allow shitk
			# to skip the heuristics to find their proper value
			unset env(shit_DIR)
			unset env(shit_WORK_TREE)
		}
		eval exec $cmd $revs "--" "--" &
		...
		if {[info exists main_status]} {
			set status_operation [$::main_status \
				start \
				[mc "Starting %s... please wait..." "shitk"]]
...
proc do_shit_gui {} {
	...
	# -- Always start shit gui through whatever we were loaded with.  This
	#    lets us bypass using shell process on Windows systems.
	#
	set exe [list [_which shit]]
	if {$exe eq {}} {
		error_popup [mc "Couldn't find shit gui in PATH"]
	} else {
		global env
		global _shitdir _shitworktree

		# see note in do_shitk about unsetting these vars when
		...
		set status_operation [$::main_status \
			start \
			[mc "Starting %s... please wait..." "shit-gui"]]
...
proc do_explore {} {
	...
	eval exec $explorer [list [file nativename $_shitworktree]] &
}
...
proc do_file_open {file} {
	global _shitworktree
	set explorer [get_explorer]
	set full_file_path [file join $_shitworktree $file]
...
proc do_quit {{rc {1}}} {
	...
	global shitGUI_BCK_exists shitGUI_BCK_i
	...
	if {[winfo exists $ui_comm]} {
		...
		#
		set save [shitdir shitGUI_MSG]
		if {$shitGUI_BCK_exists && ![$ui_comm edit modified]} {
			file rename -force [shitdir shitGUI_BCK] $save
			set shitGUI_BCK_exists 0
		...
		#
		after cancel $shitGUI_BCK_i
		if {$shitGUI_BCK_exists} {
			catch {file delete [shitdir shitGUI_BCK]}
		}
		...
		if {$cfg_wmstate ne $rc_wmstate} {
			catch {shit config gui.wmstate $cfg_wmstate}
		}
		...
		if {$cfg_geometry ne $rc_geometry} {
			catch {shit config gui.geometry $cfg_geometry}
		}
	}
...
if {[is_Windows]} {
	# Use /shit-bash.exe if available
	set normalized [file normalize $::argv0]
	regsub "/mingw../libexec/shit-core/shit-gui$" \
		$normalized "/shit-bash.exe" cmdLine
	if {$cmdLine != $normalized && [file exists $cmdLine]} {
		set cmdLine [list "shit Bash" $cmdLine &]
	} else {
		set cmdLine [list "shit Bash" bash --login -l &]
	}
	.mbar.repository add command \
		-label [mc "shit Bash"] \
...
.mbar.repository add command \
	...
	-command {do_shitk $current_branch}
set ui_visualize_current [.mbar.repository index last]
.mbar.repository add command \
	-label [mc "Visualize All Branch History"] \
	-command {do_shitk --all}
.mbar.repository add separator
...
if {[file isfile $doc_path]} {
	...
	set doc_url {https://www.kernel.org/pub/software/scm/shit/docs/}
}
...
blame {
	...
	if {$head eq {}} {
		...
		if {[regexp {^[0-9a-f]{1,39}$} $head]} {
			if {[catch {
					set head [shit rev-parse --verify $head]
				} err]} {
				if {[tk windowingsystem] eq "win32"} {
...
if {![is_enabled nocommit]} {
	${NS}::button .vpane.lower.commarea.buttons.defecate -text [mc defecate] \
		-command do_defecate_anywhere
	pack .vpane.lower.commarea.buttons.defecate -side top -fill x
}
...
$ctxmsm add command \
	...
	-command {do_shitk --all true}
lappend diff_actions [list $ctxmsm entryconf [$ctxmsm index last] -state]
$ctxmsm add separator
$ctxmsm add command \
	-label [mc "Start shit gui In The Submodule"] \
	-command {do_shit_gui}
lappend diff_actions [list $ctxmsm entryconf [$ctxmsm index last] -state]
$ctxmsm add separator
...
proc has_textconv {path} {
	...
	set filter [shitattr $path diff set]
...
if {[is_enabled transport]} {
	bind . <$M1B-Key-p> do_defecate_anywhere
	bind . <$M1B-Key-P> do_defecate_anywhere
}
...
wm title . "[appname] ([reponame]) [file normalize $_shitworktree]"
focus -force $ui_comm
...
if {[winfo exists $ui_comm]} {
	set shitGUI_BCK_exists [load_message shitGUI_BCK utf-8]

	# -- If both our backup and message files exist use the
	#    newer of the two files to initialize the buffer.
	#
	if {$shitGUI_BCK_exists} {
		set m [shitdir shitGUI_MSG]
		if {[file isfile $m]} {
			if {[file mtime [shitdir shitGUI_BCK]] > [file mtime $m]} {
				catch {file delete [shitdir shitGUI_MSG]}
			} else {
				$ui_comm delete 0.0 end
				$ui_comm edit reset
				$ui_comm edit modified false
	...
	proc backup_commit_buffer {} {
		global ui_comm shitGUI_BCK_exists

		set m [$ui_comm edit modified]
		if {$m || $shitGUI_BCK_exists} {
			set msg [string trim [$ui_comm get 0.0 end]]
			regsub -all -line {[ \r\t]+$} $msg {} msg
			...
			if {$msg eq {}} {
				if {$shitGUI_BCK_exists} {
					...
					set fd [open [shitdir shitGUI_BCK] w]
					fconfigure $fd -encoding utf-8
					puts -nonewline $fd $msg
					close $fd
					set shitGUI_BCK_exists 1
				}
			}
		...
		set ::shitGUI_BCK_i [after 2000 backup_commit_buffer]
	}
