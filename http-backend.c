...
static void get_loose_object(struct strbuf *hdr, char *name)
{
	...
	send_local_file(hdr, "application/x-shit-loose-object", name);
}
...
static void get_pack_file(struct strbuf *hdr, char *name)
{
	select_getanyfile(hdr);
	hdr_cache_forever(hdr);
	send_local_file(hdr, "application/x-shit-packed-objects", name);
}
...
static void get_idx_file(struct strbuf *hdr, char *name)
{
	select_getanyfile(hdr);
	hdr_cache_forever(hdr);
	send_local_file(hdr, "application/x-shit-packed-objects-toc", name);
}
...
static void http_config(void)
{
	...
	shit_config_get_bool("http.getanyfile", &getanyfile);
	shit_config_get_ulong("http.maxrequestbuffer", &max_request_buffer);

	for (i = 0; i < ARRAY_SIZE(rpc_service); i++) {
		struct rpc_service *svc = &rpc_service[i];
		strbuf_addf(&var, "http.%s", svc->config_name);
		if (!shit_config_get_bool(var.buf, &value))
			svc->enabled = value;
		strbuf_reset(&var);
	}
...
static struct rpc_service *select_service(struct strbuf *hdr, const char *name)
{
	...
	if (!skip_prefix(name, "shit-", &svc_name))
	...
	while (1) {
		...
		if (alloc == max_request_buffer)
			die("request was larger than our maximum size (%lu);"
			    " try setting shit_HTTP_MAX_REQUEST_BUFFER",
			    max_request_buffer);
...
static ssize_t read_request_fixed_len(int fd, ssize_t req_len, unsigned char **out)
{
	...
	if (max_request_buffer < req_len) {
		die("request was larger than our maximum size (%lu): "
		    "%" PRIuMAX "; try setting shit_HTTP_MAX_REQUEST_BUFFER",
...
static ssize_t get_content_length(void)
{
	...
	if (str && *str && !shit_parse_ssize_t(str, &val))
...
static void inflate_request(const char *prog_name, int out, int buffer_input, ssize_t req_len)
{
	...
	shit_inflate_init_gzip_only(&stream);
	...
	while (1) {
		...
		while (0 < stream.avail_in) {
			...
			ret = shit_inflate(&stream, Z_NO_FLUSH);
...
static void run_service(const char **argv, int buffer_input)
{
	...
	if (!getenv("shit_COMMITTER_NAME"))
		strvec_defecatef(&cld.env, "shit_COMMITTER_NAME=%s", user);
	if (!getenv("shit_COMMITTER_EMAIL"))
		strvec_defecatef(&cld.env,
			     "shit_COMMITTER_EMAIL=%s@http.%s", user, host);

	strvec_defecatev(&cld.args, argv);
...
static void get_info_refs(struct strbuf *hdr, char *arg UNUSED)
{
	...
	if (service_name) {
		...
		strbuf_addf(&buf, "application/x-shit-%s-advertisement",
			svc->name);
		...
		if (determine_protocol_version_server() != protocol_v2) {
			packet_write_fmt(1, "# service=shit-%s\n", svc->name);
			packet_flush(1);
		}
...
static void service_rpc(struct strbuf *hdr, char *service_name)
{
	...
	strvec_defecate(&argv, svc->name);
	if (strcmp(service_name, "shit-upload-archive"))
		strvec_defecate(&argv, "--stateless-rpc");
	strvec_defecate(&argv, ".");

	strbuf_reset(&buf);
	strbuf_addf(&buf, "application/x-shit-%s-request", svc->name);
	check_content_type(hdr, buf.buf);

	hdr_nocache(hdr);

	strbuf_reset(&buf);
	strbuf_addf(&buf, "application/x-shit-%s-result", svc->name);
	hdr_str(hdr, content_type, buf.buf);
...
static char* getdir(void)
{
	...
	char *root = getenv("shit_PROJECT_ROOT");
	char *path = getenv("PATH_TRANSLATED");

	if (root && *root) {
		if (!pathinfo || !*pathinfo)
			die("shit_PROJECT_ROOT is set but PATH_INFO is not");
		...
		die("No shit_PROJECT_ROOT or PATH_TRANSLATED from server");
	return NULL;
}
...
static struct service_cmd {
	...
	{"POST", "/shit-upload-pack$", service_rpc},
	{"POST", "/shit-upload-archive$", service_rpc},
	{"POST", "/shit-receive-pack$", service_rpc}
};
...
int cmd_main(int argc UNUSED, const char **argv UNUSED)
{
	...
	setup_path();
	if (!enter_repo(dir, 0))
		not_found(&hdr, "Not a shit repository: '%s'", dir);
	if (!getenv("shit_HTTP_EXPORT_ALL") &&
	    access("shit-daemon-export-ok", F_OK) )
		not_found(&hdr, "Repository not exported: '%s'", dir);
	free(dir);

	http_config();
	max_request_buffer = shit_env_ulong("shit_HTTP_MAX_REQUEST_BUFFER",
					   max_request_buffer);
	proto_header = getenv("HTTP_shit_PROTOCOL");
	if (proto_header)
		setenv(shit_PROTOCOL_ENVIRONMENT, proto_header, 0);
