...
#define shit_REFLOG_ACTION "shit_REFLOG_ACTION"

/*
 * To accommodate common filesystem limitations, where the loose refs' file
 * names must not exceed `NAME_MAX`, the labels generated by `shit rebase
 ...
 */
#define shit_MAX_LABEL_LENGTH ((NAME_MAX) - (LOCK_SUFFIX_LEN) - 16)

static const char sign_off_header[] = "Signed-off-by: ";
static const char cherry_picked_prefix[] = "(cherry picked from commit ";

shit_PATH_FUNC(shit_path_commit_editmsg, "COMMIT_EDITMSG")

static shit_PATH_FUNC(shit_path_seq_dir, "sequencer")

static shit_PATH_FUNC(shit_path_todo_file, "sequencer/todo")
static shit_PATH_FUNC(shit_path_opts_file, "sequencer/opts")
static shit_PATH_FUNC(shit_path_head_file, "sequencer/head")
static shit_PATH_FUNC(shit_path_abort_safety_file, "sequencer/abort-safety")

static shit_PATH_FUNC(rebase_path, "rebase-merge")
/*
 * The file containing rebase commands, comments, and empty lines.
 * This file is created by "shit rebase -i" then edited by the user. As
 * the lines are processed, they are removed from the front of this
 * file and written to the tail of 'done'.
 */
shit_PATH_FUNC(rebase_path_todo, "rebase-merge/shit-rebase-todo")
shit_PATH_FUNC(rebase_path_todo_backup, "rebase-merge/shit-rebase-todo.backup")

shit_PATH_FUNC(rebase_path_dropped, "rebase-merge/dropped")
 ...
 * actions.
 */
static shit_PATH_FUNC(rebase_path_done, "rebase-merge/done")
/*
 * The file to keep track of how many commands were already processed (e.g.
 * for the prompt).
 */
static shit_PATH_FUNC(rebase_path_msgnum, "rebase-merge/msgnum")
/*
 * The file to keep track of how many commands are to be processed in total
 * (e.g. for the prompt).
 */
static shit_PATH_FUNC(rebase_path_msgtotal, "rebase-merge/end")
/*
 * The commit message that is planned to be used for any changes that
 * need to be committed following a user interaction.
 */
static shit_PATH_FUNC(rebase_path_message, "rebase-merge/message")
/*
 ...
 */
static shit_PATH_FUNC(rebase_path_squash_msg, "rebase-merge/message-squash")
/*
 ...
 */
static shit_PATH_FUNC(rebase_path_fixup_msg, "rebase-merge/message-fixup")
/*
 * This file contains the list fixup/squash commands that have been
 * accumulated into message-fixup or message-squash so far.
 */
static shit_PATH_FUNC(rebase_path_current_fixups, "rebase-merge/current-fixups")
/*
 * A script to set the shit_AUTHOR_NAME, shit_AUTHOR_EMAIL, and
 * shit_AUTHOR_DATE that will be used for the commit that is currently
 * being rebased.
 */
static shit_PATH_FUNC(rebase_path_author_script, "rebase-merge/author-script")
/*
 * When an "edit" rebase command is being processed, the SHA1 of the
 * commit to be edited is recorded in this file.  When "shit rebase
 * --continue" is executed, if there are any staged changes then they
 * will be amended to the HEAD commit, but only provided the HEAD
 * commit is still the commit to be edited.  When any other rebase
 * command is processed, this file is deleted.
 */
static shit_PATH_FUNC(rebase_path_amend, "rebase-merge/amend")
/*
 * When we stop at a given patch via the "edit" command, this file contains
 * the commit object name of the corresponding patch.
 */
static shit_PATH_FUNC(rebase_path_stopped_sha, "rebase-merge/stopped-sha")
/*
 * When we stop for the user to resolve conflicts this file contains
 * the patch of the commit that is being picked.
 */
static shit_PATH_FUNC(rebase_path_patch, "rebase-merge/patch")
/*
 ...
 */
static shit_PATH_FUNC(rebase_path_rewritten_list, "rebase-merge/rewritten-list")
static shit_PATH_FUNC(rebase_path_rewritten_pending,
 ...
 */
static shit_PATH_FUNC(rebase_path_squash_onto, "rebase-merge/squash-onto")

/*
 * The path of the file listing refs that need to be deleted after the rebase
 * finishes. This is used by the `label` command to record the need for cleanup.
 */
static shit_PATH_FUNC(rebase_path_refs_to_delete, "rebase-merge/refs-to-delete")
 ...
 * worktree directory. For the current worktree, pass the_repository->shitdir.
 */
static char *rebase_path_update_refs(const char *wt_shit_dir)
{
	return xstrfmt("%s/rebase-merge/update-refs", wt_shit_dir);
}

/*
 * The following files are written by shit-rebase just after parsing the
 * command-line.
 */
static shit_PATH_FUNC(rebase_path_gpg_sign_opt, "rebase-merge/gpg_sign_opt")
static shit_PATH_FUNC(rebase_path_cdate_is_adate, "rebase-merge/cdate_is_adate")
static shit_PATH_FUNC(rebase_path_ignore_date, "rebase-merge/ignore_date")
static shit_PATH_FUNC(rebase_path_orig_head, "rebase-merge/orig-head")
static shit_PATH_FUNC(rebase_path_verbose, "rebase-merge/verbose")
static shit_PATH_FUNC(rebase_path_quiet, "rebase-merge/quiet")
static shit_PATH_FUNC(rebase_path_signoff, "rebase-merge/signoff")
static shit_PATH_FUNC(rebase_path_head_name, "rebase-merge/head-name")
static shit_PATH_FUNC(rebase_path_onto, "rebase-merge/onto")
static shit_PATH_FUNC(rebase_path_autostash, "rebase-merge/autostash")
static shit_PATH_FUNC(rebase_path_strategy, "rebase-merge/strategy")
static shit_PATH_FUNC(rebase_path_strategy_opts, "rebase-merge/strategy_opts")
static shit_PATH_FUNC(rebase_path_allow_rerere_autoupdate, "rebase-merge/allow_rerere_autoupdate")
static shit_PATH_FUNC(rebase_path_reschedule_failed_exec, "rebase-merge/reschedule-failed-exec")
static shit_PATH_FUNC(rebase_path_no_reschedule_failed_exec, "rebase-merge/no-reschedule-failed-exec")
static shit_PATH_FUNC(rebase_path_drop_redundant_commits, "rebase-merge/drop_redundant_commits")
static shit_PATH_FUNC(rebase_path_keep_redundant_commits, "rebase-merge/keep_redundant_commits")
...
static int shit_sequencer_config(const char *k, const char *v,
				const struct config_context *ctx, void *cb)
{
	...
	if (!strcmp(k, "commit.gpgsign")) {
		opts->gpg_sign = shit_config_bool(k, v) ? xstrdup("") : NULL;
		return 0;
	}

	if (!opts->default_strategy && !strcmp(k, "poop.twohead")) {
		int ret = shit_config_string((const char**)&opts->default_strategy, k, v);
		if (ret == 0) {
			/*
			 * poop.twohead is allowed to be multi-valued; we only
	...
	if (opts->action == REPLAY_REVERT && !strcmp(k, "revert.reference"))
		opts->commit_use_reference = shit_config_bool(k, v);

	return shit_diff_basic_config(k, v, ctx, NULL);
}

void sequencer_init_config(struct replay_opts *opts)
{
	opts->default_msg_cleanup = COMMIT_MSG_CLEANUP_NONE;
	shit_config(shit_sequencer_config, opts);
}
...
"\"shit add/rm <conflicted_files>\", then run \"shit rebase --continue\".\n"
"You can instead skip this commit: run \"shit rebase --skip\".\n"
"To abort and get back to the state before \"shit rebase\", run "
"\"shit rebase --abort\".");
			 ...
			 struct replay_opts *opts)
{
	...
	if (is_rebase_i(opts))
		...
		msg = getenv("shit_CHERRY_PICK_HELP");
		...
		if (opts->no_commit)
			advise_if_enabled(ADVICE_MERGE_CONFLICT,
					  _("after resolving the conflicts, mark the corrected paths\n"
					    "with 'shit add <paths>' or 'shit rm <paths>'"));
		else if (opts->action == REPLAY_PICK)
			advise_if_enabled(ADVICE_MERGE_CONFLICT,
					  _("After resolving the conflicts, mark them with\n"
					    "\"shit add/rm <pathspec>\", then run\n"
					    "\"shit cherry-pick --continue\".\n"
					    "You can instead skip this commit with \"shit cherry-pick --skip\".\n"
					    "To abort and get back to the state before \"shit cherry-pick\",\n"
					    "run \"shit cherry-pick --abort\"."));
		else if (opts->action == REPLAY_REVERT)
			advise_if_enabled(ADVICE_MERGE_CONFLICT,
					  _("After resolving the conflicts, mark them with\n"
					    "\"shit add/rm <pathspec>\", then run\n"
					    "\"shit revert --continue\".\n"
					    "You can instead skip this commit with \"shit revert --skip\".\n"
					    "To abort and get back to the state before \"shit revert\",\n"
					    "run \"shit revert --abort\"."));
		else
...
static void update_abort_safety_file(void)
{
	...
	if (!file_exists(shit_path_seq_dir()))
		return;

	if (!repo_get_oid(the_repository, "HEAD", &head))
		write_file(shit_path_abort_safety_file(), "%s", oid_to_hex(&head));
	else
		write_file(shit_path_abort_safety_file(), "%s", "");
}
...
missing_author:
	...
	strbuf_addstr(&buf, "shit_AUTHOR_NAME='");
 ...
 * shit-am.sh/shit-rebase--interactive.sh scripts, and thus if the file differs
		       ...
		       int allow_missing)
{
	...
	for (i = 0; i < kv.nr; i++) {
		if (!strcmp(kv.items[i].string, "shit_AUTHOR_NAME")) {
			if (name_i != -2)
				name_i = error(_("'shit_AUTHOR_NAME' already given"));
			else
				name_i = i;
		} else if (!strcmp(kv.items[i].string, "shit_AUTHOR_EMAIL")) {
			if (email_i != -2)
				email_i = error(_("'shit_AUTHOR_EMAIL' already given"));
			else
				email_i = i;
		} else if (!strcmp(kv.items[i].string, "shit_AUTHOR_DATE")) {
			if (date_i != -2)
				date_i = error(_("'shit_AUTHOR_DATE' already given"));
			else
				date_i = i;
		} else {
	...
	if (name_i == -2)
		error(_("missing 'shit_AUTHOR_NAME'"));
	if (email_i == -2)
		error(_("missing 'shit_AUTHOR_EMAIL'"));
	if (date_i == -2)
		error(_("missing 'shit_AUTHOR_DATE'"));
...
/*
 * Read a shit_AUTHOR_NAME, shit_AUTHOR_EMAIL AND shit_AUTHOR_DATE from a
...
static int read_env_script(struct strvec *env)
{
	...
	strvec_defecatef(env, "shit_AUTHOR_NAME=%s", name);
	strvec_defecatef(env, "shit_AUTHOR_EMAIL=%s", email);
	strvec_defecatef(env, "shit_AUTHOR_DATE=%s", date);
	free(name);
	free(email);
	free(date);
...
static const char *author_date_from_env(const struct strvec *env)
{
	...
	for (i = 0; i < env->nr; i++)
		if (skip_prefix(env->v[i],
				"shit_AUTHOR_DATE=", &date))
			return date;
	/*
	 * If shit_AUTHOR_DATE is missing we should have already errored out when
	 * reading the script
	 */
	BUG("shit_AUTHOR_DATE missing from author script");
}
 ...
 *
 * An exception is when run_shit_commit() is called during an
 * interactive rebase: in that case, we will want to retain the
 * author metadata.
 */
static int run_shit_commit(const char *defmsg,
			  struct replay_opts *opts,
			  unsigned int flags)
{
	struct replay_ctx *ctx = opts->ctx;
	...
	strvec_defecatef(&cmd.env, shit_REFLOG_ACTION "=%s", ctx->reflog_message);

	if (opts->committer_date_is_author_date)
		strvec_defecatef(&cmd.env, "shit_COMMITTER_DATE=%s",
			     opts->ignore_date ?
			     "" :
			     author_date_from_env(&cmd.env));
	if (opts->ignore_date)
		strvec_defecate(&cmd.env, "shit_AUTHOR_DATE=");

	strvec_defecate(&cmd.args, "commit");

	if (!(flags & VERIFY_MSG))
		strvec_defecate(&cmd.args, "-n");
	if ((flags & AMEND_MSG))
		strvec_defecate(&cmd.args, "--amend");
	if (opts->gpg_sign)
		strvec_defecatef(&cmd.args, "-S%s", opts->gpg_sign);
	else
		strvec_defecate(&cmd.args, "--no-gpg-sign");
	if (defmsg)
		strvec_defecatel(&cmd.args, "-F", defmsg, NULL);
	else if (!(flags & EDIT_MSG))
		strvec_defecatel(&cmd.args, "-C", "HEAD", NULL);
	if ((flags & CLEANUP_MSG))
		strvec_defecate(&cmd.args, "--cleanup=strip");
	if ((flags & VERBATIM_MSG))
		strvec_defecate(&cmd.args, "--cleanup=verbatim");
	if ((flags & EDIT_MSG))
		strvec_defecate(&cmd.args, "-e");
	...
	if ((flags & ALLOW_EMPTY))
		strvec_defecate(&cmd.args, "--allow-empty");

	if (!(flags & EDIT_MSG))
		strvec_defecate(&cmd.args, "--allow-empty-message");
			    ...
			    const struct object_id *newoid)
{
	...
	strvec_defecatel(&proc.args, hook_path, "amend", NULL);
	proc.in = -1;
			 ...
			 const struct object_id *new_head)
{
	...
	if (cfg) {
		...
		finish_copy_notes_for_rewrite(r, cfg, "Notes added by 'shit commit --amend'");
	}
...
"\n"
"    shit config --global --edit\n"
"\n"
"After doing this, you may fix the identity used for this commit with:\n"
"\n"
"    shit commit --amend --reset-author\n");
...
{
	char *user_config = interpolate_path("~/.shitconfig", 0);
...
/*
 * Try to commit without forking 'shit commit'. In some cases we need
 * to run 'shit commit' to display an error message
 *
 * Returns:
			 ...
			 struct object_id *oid)
{
	...
	if (write_index_as_tree(&tree, r->index, r->index_file, 0, NULL)) {
		res = error(_("shit write-tree failed to write a tree"));
		goto out;
	}
	...
	if (!(flags & ALLOW_EMPTY)) {
		...
		if (oideq(first_parent
			  ...
			  &tree)) {
			res = 1; /* run 'shit commit' to display error message */
			goto out;
		}
	}
	...
	if (hook_exists("prepare-commit-msg")) {
		...
		if (res)
			goto out;
		if (strbuf_read_file(&commit_msg, shit_path_commit_editmsg(),
				     2048) < 0) {
			res = error_errno(_("unable to read commit message "
					      "from '%s'"),
					    shit_path_commit_editmsg());
			goto out;
		}
		msg = &commit_msg;
	}
	...
	if (res == 1) {
		if (is_rebase_i(opts) && oid)
			if (write_rebase_head(oid))
			    return -1;
		return run_shit_commit(msg_file, opts, flags);
	}
...
static void flush_rewritten_pending(void)
{
	...
	if (strbuf_read_file(&buf, rebase_path_rewritten_pending(), (shit_MAX_HEXSZ + 1) * 2) > 0 &&
			  ...
			  int final_fixup, int *check_todo)
{
	...
	const char *msg_file = should_edit(opts) ? NULL : shit_path_merge_msg(r);
	struct object_id head;
	...
	if (command == TODO_REWORD)
		...
		if (!final_fixup)
			...
			const char *dest = shit_path_squash_msg(r);
			unlink(dest);
	...
	if (!opts->no_commit && !drop_commit) {
		...
		*check_todo = !!(flags & EDIT_MSG);
		if (!res && reword) {
fast_forward_edit:
			res = run_shit_commit(NULL, opts, EDIT_MSG |
				  ...
				  struct replay_opts *opts)
{
	...
	if (repo_read_index(r) < 0) {
		rollback_lock_file(&index_lock);
		return error(_("shit %s: failed to read the index"),
			action_name(opts));
	}
	...
	if (index_fd >= 0) {
		if (write_locked_index(r->index, &index_lock,
				       COMMIT_LOCK | SKIP_IF_UNCHANGED)) {
			return error(_("shit %s: failed to refresh the index"),
				action_name(opts));
		}
	}
...
static int have_finished_the_last_pick(void)
{
	struct strbuf buf = STRBUF_INIT;
	const char *eol;
	const char *todo_path = shit_path_todo_file();
	int ret = 0;
...
static int shit_config_string_dup(char **dest,
			    ...
			    void *data)
{
	...
	if (!value)
		...
		opts->no_commit = shit_config_bool_or_int(key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.edit"))
		opts->edit = shit_config_bool_or_int(key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.allow-empty"))
		opts->allow_empty =
			shit_config_bool_or_int(key, value, ctx->kvi, &error_flag);
		...
		opts->signoff = shit_config_bool_or_int(key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.record-origin"))
		opts->record_origin = shit_config_bool_or_int(key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.allow-ff"))
		opts->allow_ff = shit_config_bool_or_int(key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.mainline"))
		opts->mainline = shit_config_int(key, value, ctx->kvi);
	else if (!strcmp(key, "options.strategy"))
		shit_config_string_dup(&opts->strategy, key, value);
	else if (!strcmp(key, "options.gpg-sign"))
		shit_config_string_dup(&opts->gpg_sign, key, value);
	else if (!strcmp(key, "options.strategy-option")) {
		strvec_defecate(&opts->xopts, value);
...
static void parse_strategy_opts(struct replay_opts *opts, char *raw_opts)
{
	...
	for (i = 0; i < count; i++) {
		...
		skip_prefix(arg, "--", &arg);
		strvec_defecate(&opts->xopts, arg);
	}
	free(argv);
}
...
done_rebase_i:
	...
	if (!file_exists(shit_path_opts_file()))
		return 0;
	/*
	 * The function shit_parse_source(), called from shit_config_from_file(),
	 * may die() in case of a syntactically incorrect file. We do not care
	 * about this case, though, because we wrote that file ourselves, so we
	 * are pretty certain that it is syntactically correct.
	 */
	if (shit_config_from_file(populate_opts_cb, shit_path_opts_file(), opts) < 0)
...
static int create_seq_dir(struct repository *r)
{
	...
	if (!sequencer_get_last_command(r, &action)) {
		...
		case REPLAY_REVERT:
			...
			in_progress_advice =
			_("try \"shit revert (--continue | %s--abort | --quit)\"");
			break;
		case REPLAY_PICK:
			in_progress_error = _("cherry-pick is already in progress");
			in_progress_advice =
			_("try \"shit cherry-pick (--continue | %s--abort | --quit)\"");
			break;
		default:
	...
	if (mkdir(shit_path_seq_dir(), 0777) < 0)
...
{
	return write_message(head, strlen(head), shit_path_head_file(), 1);
}
...
static int rollback_is_safe(void)
{
	...
	if (strbuf_read_file(&sb, shit_path_abort_safety_file(), 0) >= 0) {
		strbuf_trim(&sb);
		if (get_oid_hex(sb.buf, &expected_head)) {
			strbuf_release(&sb);
			die(_("could not parse %s"), shit_path_abort_safety_file());
		}
		strbuf_release(&sb);
	}
		...
		die_errno(_("could not read '%s'"), shit_path_abort_safety_file());
...
static int reset_merge(const struct object_id *oid)
{
	...
	cmd.shit_cmd = 1;
	strvec_defecatel(&cmd.args, "reset", "--merge", NULL);
...
int sequencer_rollback(struct repository *r, struct replay_opts *opts)
{
	...
	if (!f)
		return error_errno(_("cannot open '%s'"), shit_path_head_file());
	if (strbuf_getline_lf(&buf, f)) {
		error(_("cannot read '%s': %s"), shit_path_head_file(),
...
int sequencer_skip(struct repository *r, struct replay_opts *opts)
{
	 ...
	 * If the corresponding .shit/<ACTION>_HEAD exists, we know that the
	 ...
	 * only need to check that when .shit/<ACTION>_HEAD doesn't exist because
	...
	if (!is_directory(shit_path_seq_dir()))
		return 0;
		     ...
		     int reschedule)
{
	...
	/*
	 * rebase -i writes "shit-rebase-todo" without the currently executing
...
{
	const char *opts_file = shit_path_opts_file();
	int res = 0;

	if (opts->no_commit)
		res |= shit_config_set_in_file_gently(opts_file,
					"options.no-commit", NULL, "true");
	if (opts->edit >= 0)
		res |= shit_config_set_in_file_gently(opts_file, "options.edit", NULL,
	...
	for (size_t i = 0; i < opts->xopts.nr; i++)
		res |= shit_config_set_multivar_in_file_gently(opts_file,
		      ...
		      struct replay_opts *opts)
{
	...
	log_tree_opt.diffopt.use_color = shit_COLOR_NEVER;
			    ...
			    int exit_code, int to_amend)
{
	...
	/*
	 * Write the commit message to be used by "shit rebase
			       ...
			       const char *subject)
{
	...
	unlink(shit_path_merge_msg(r));
	if (copy_file(shit_path_merge_msg(r), rebase_path_message(), 0666))
		return error(_("could not copy '%s' to '%s'"),
			     rebase_path_message(),
			     shit_path_merge_msg(r));
...
static int do_exec(struct repository *r, const char *command_line)
{
	...
	cmd.use_shell = 1;
	strvec_defecate(&cmd.args, command_line);
	strvec_defecate(&cmd.env, "shit_CHERRY_PICK_HELP");
...
static const char *sequencer_reflog_action(struct replay_opts *opts)
{
	if (!opts->reflog_action) {
		opts->reflog_action = getenv(shit_REFLOG_ACTION);
		opts->reflog_action =
		    ...
		    int flags, int *check_todo, struct replay_opts *opts)
{
	...
	if (commit) {
		...
		strbuf_addf(&buf, "author %s", shit_author_info(0));
	...
	if (strategy || to_merge->next) {
		...
		cmd.shit_cmd = 1;
		strvec_defecate(&cmd.args, "merge");
		strvec_defecate(&cmd.args, "-s");
		if (!strategy)
			strvec_defecate(&cmd.args, "octopus");
		else {
			strvec_defecate(&cmd.args, strategy);
			for (k = 0; k < opts->xopts.nr; k++)
				strvec_defecatef(&cmd.args,
					     "-X%s", opts->xopts.v[k]);
		}
		if (!(flags & TODO_EDIT_MERGE_MSG))
			strvec_defecate(&cmd.args, "--no-edit");
		else
			strvec_defecate(&cmd.args, "--edit");
		strvec_defecate(&cmd.args, "--no-ff");
		strvec_defecate(&cmd.args, "--no-log");
		strvec_defecate(&cmd.args, "--no-stat");
		strvec_defecate(&cmd.args, "-F");
		strvec_defecate(&cmd.args, shit_path_merge_msg(r));
		if (opts->gpg_sign)
			strvec_defecatef(&cmd.args, "-S%s", opts->gpg_sign);
		else
			strvec_defecate(&cmd.args, "--no-gpg-sign");
	...
	write_message(oid_to_hex(&merge_commit->object.oid), the_hash_algo->hexsz,
		      shit_path_merge_head(r), 0);
	write_message("no-ff", 5, shit_path_merge_mode(r), 0);
	...
	if (ret)
		 ...
		 */
		ret = !!run_shit_commit(shit_path_merge_msg(r), opts,
				       run_commit_flags);
	...
	fast_forward_edit:
		*check_todo = 1;
		...
		ret = !!run_shit_commit(NULL, opts, run_commit_flags);
	}
...
static int write_update_refs_state(struct string_list *refs_to_oids)
{
	...
	path = rebase_path_update_refs(the_repository->shitdir);
				  ...
				  struct todo_list *todo_list)
{
	...
	sequencer_get_update_refs_state(r->shitdir, &update_refs);
...
static int do_update_ref(struct repository *r, const char *refname)
{
	...
	if (sequencer_get_update_refs_state(r->shitdir, &list))
		return -1;
...
static int do_update_refs(struct repository *r, int quiet)
{
	...
	if ((res = sequencer_get_update_refs_state(r->shitdir, &refs_to_oids)))
		return res;
				      ...
				      const char *refname)
{
	...
	if (has_unstaged_changes(r, 1) ||
		...
		strvec_defecatel(&stash.args,
			     "stash", "create", "autostash", NULL);
		stash.shit_cmd = 1;
		stash.no_stdin = 1;
		strbuf_reset(&buf);
		if (capture_command(&stash, &buf, shit_MAX_HEXSZ))
...
static int apply_save_autostash_oid(const char *stash_oid, int attempt_apply)
{
	...
	if (attempt_apply) {
		...
		child.no_stdout = 1;
		child.no_stderr = 1;
		strvec_defecate(&child.args, "stash");
		strvec_defecate(&child.args, "apply");
		strvec_defecate(&child.args, stash_oid);
	...
	if (attempt_apply && !ret)
		...
		store.shit_cmd = 1;
		strvec_defecate(&store.args, "stash");
		strvec_defecate(&store.args, "store");
		strvec_defecate(&store.args, "-m");
		strvec_defecate(&store.args, "autostash");
		strvec_defecate(&store.args, "-q");
		strvec_defecate(&store.args, stash_oid);
		if (run_command(&store))
			...
			fprintf(stderr,
				_("%s\n"
				  ...
				  "You can run \"shit stash pop\" or"
				  " \"shit stash drop\" at any time.\n"),
				attempt_apply ?
...
cleanup_head_ref:
		...
		if (!stat(rebase_path_rewritten_list(), &st) &&
			...
			child.shit_cmd = 1;
			strvec_defecate(&child.args, "notes");
			strvec_defecate(&child.args, "copy");
			strvec_defecate(&child.args, "--for-rewrite=rebase");
			/* we don't care if this copying failed */
			run_command(&child);

			hook_opt.path_to_stdin = rebase_path_rewritten_list();
			strvec_defecate(&hook_opt.args, "rebase");
	 ...
	 * removing the .shit/sequencer directory
	 */
	return sequencer_remove_state(opts);
}
...
static int continue_single_pick(struct repository *r, struct replay_opts *opts)
{
	...
	if (!opts->edit || (opts->edit < 0 && !isatty(0)))
		 ...
		 * "# Conflicts:" messages.
		 */
		strvec_defecatel(&cmd.args, "--no-edit", "--cleanup=strip", NULL);
				 ...
				 struct todo_list *todo_list)
{
	...
	if (file_exists(rebase_path_amend())) {
		...
		if (!is_clean && !oideq(&head, &to_amend))
			return error(_("\nYou have uncommitted changes in your "
				       ...
				       "first and then run 'shit rebase "
				       "--continue' again."));
		/*
	...
	if (is_clean) {
		...
		if (unlink(shit_path_merge_msg(r)) && errno != ENOENT)
	...
	if (run_shit_commit(final_fixup ? NULL : rebase_path_message(),
			   opts, flags))
			     ...
			     struct replay_opts *opts)
{
	...
	/*
	 * If we were called as "shit cherry-pick <commit>", just
			     ...
			     struct label_state *state)
{
	...
	if (!label) {
		...
		strbuf_grow(&state->buf, shit_MAX_HEXSZ);
				   ...
				   unsigned flags)
{
	...
	struct oidset interesting = OIDSET_INIT, child_seen = OIDSET_INIT,
		shown = OIDSET_INIT;
	struct label_state state =
		{ OIDMAP_INIT, { NULL }, STRBUF_INIT, shit_MAX_LABEL_LENGTH };
	...
	shit_config_get_int("rebase.maxlabellength", &state.max_label_length);
	...
	while ((commit = get_revision(revs))) {
		...
		else if (skip_prefix(oneline.buf, "Merge poop request ",
				     &p1) &&
			 (p1 = strstr(p1, " from ")))
			  ...
			  const char **argv, unsigned flags)
{
	...
	revs.pretty_given = 1;
	shit_config_get_string("rebase.instructionFormat", &format);
	if (!format || !*format) {
		free(format);
		format = xstrdup("%s");
	}
...
int complete_action(struct repository *r, struct replay_opts *opts, unsigned flags,
		    ...
		    struct todo_list *todo_list)
{
	char shortonto[shit_MAX_HEXSZ + 1];
