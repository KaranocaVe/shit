 ...
 * start with this signature, as all older shit binaries would find this
 ...
 *
 * Very old shit binaries will also compare the first 4 bytes to the
...
int check_pack_crc(struct packed_shit *p, struct pack_window **w_curs, off_t offset, off_t len, unsigned int nr);
int verify_pack_index(struct packed_shit *);
int verify_pack(struct repository *, struct packed_shit *, verify_fn fn, struct progress *, uint32_t);
