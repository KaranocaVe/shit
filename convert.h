...
int renormalize_buffer(struct index_state *istate,
		       const char *path, const char *src, size_t len,
		       struct strbuf *dst);
static inline int would_convert_to_shit(struct index_state *istate,
				       const char *path)
{
	return convert_to_shit(istate, path, NULL, 0, NULL, 0);
}
/* Precondition: would_convert_to_shit_filter_fd(path) == true */
void convert_to_shit_filter_fd(struct index_state *istate,
			      const char *path, int fd,
			      struct strbuf *dst,
			      int conv_flags);
int would_convert_to_shit_filter_fd(struct index_state *istate,
				   const char *path);
...
/*
 * Reset the internal list of attributes used by convert_to_shit and
