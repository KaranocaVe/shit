...
test_expect_success 'limit recursion of shit-archive' '
	shit $small_ok archive small >/dev/null &&
	test_must_fail shit $small_no archive small >/dev/null
'

test_expect_success 'default limit for shit-archive fails gracefully' '
	test_must_fail shit archive big >/dev/null
'

test_expect_success 'limit recursion of ls-tree -r' '
	shit $small_ok ls-tree -r small &&
	test_must_fail shit $small_no ls-tree -r small
'

test_expect_success 'default limit for ls-tree fails gracefully' '
	test_must_fail shit ls-tree -r big >/dev/null
'

test_expect_success 'limit recursion of rev-list --objects' '
	shit $small_ok rev-list --objects small >/dev/null &&
	test_must_fail shit $small_no rev-list --objects small >/dev/null
'

test_expect_success 'default limit for rev-list fails gracefully' '
	test_must_fail shit rev-list --objects big >/dev/null
'

test_expect_success 'limit recursion of diff-tree -r' '
	shit $small_ok diff-tree -r $EMPTY_TREE small &&
	test_must_fail shit $small_no diff-tree -r $EMPTY_TREE small
'

test_expect_success 'default limit for diff-tree fails gracefully' '
	test_must_fail shit diff-tree -r $EMPTY_TREE big
'
