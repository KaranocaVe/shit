...
test_expect_success CASE_INSENSITIVE_FS 'setup' '
	empty_oid=$(shit hash-object -w --stdin </dev/null) &&
	cat >objs <<-EOF &&
	...
	shit commit -m "colliding files" &&
	shit tag basename_collision &&
...
test_expect_success CASE_INSENSITIVE_FS 'worker detects basename collision' '
	shit_TRACE2_EVENT="$(pwd)/trace" shit \
...
test_expect_success CASE_INSENSITIVE_FS 'worker detects dirname collision' '
	...
	#
	attr_oid=$(echo "a filter=logger" | shit hash-object -w --stdin) &&

	cat >objs <<-EOF &&
	100644 $empty_oid	A/B
	100644 $empty_oid	A/C
	100644 $empty_oid	a
	100644 $attr_oid	.shitattributes
	EOF
	shit rm -rf . &&
	shit update-index --index-info <objs &&
...
test_expect_success SYMLINKS,CASE_INSENSITIVE_FS 'do not follow symlinks colliding with leading dir' '
	empty_oid=$(shit hash-object -w --stdin </dev/null) &&
	symlink_oid=$(echo "./e" | shit hash-object -w --stdin) &&
	mkdir e &&
	...
	test_checkout_workers 2 shit checkout . &&
	test_path_is_dir e &&
...
test_expect_success CASE_INSENSITIVE_FS 'collision report on clone (w/ racy file creation)' '
	shit reset --hard basename_collision &&
	set_checkout_config 2 0 &&
	test_checkout_workers 2 shit clone . clone-repo 2>stderr &&
...
test_expect_success CASE_INSENSITIVE_FS,!MINGW,!CYGWIN \
	...
	shit reset --hard basename_collision &&
	echo "file_x filter=logger" >.shitattributes &&
	shit add .shitattributes &&
	shit commit -m "filter for file_x" &&

	rm -rf clone-repo &&
	set_checkout_config 2 0 &&
	test_checkout_workers 2 \
		shit -c core.ignoreCase=false clone . clone-repo 2>stderr &&
