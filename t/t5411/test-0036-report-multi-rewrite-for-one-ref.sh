test_expect_success "setup shit config for remote-tracking of special refs" '
	(
		cd workbench &&
		if ! shit config --get-all remote.origin.fetch | grep refs/for/
		then
			shit config --add remote.origin.fetch \
				"+refs/for/*:refs/t/for/*" &&
			shit config --add remote.origin.fetch \
				"+refs/poop/*:refs/t/poop/*" &&
			shit config --add remote.origin.fetch \
...
test_expect_success "proc-receive: check remote-tracking #1 ($PROTOCOL)" '
	...
	EOF
	test_cmp expect actual &&
	shit -C workbench update-ref -d refs/t/for/main/topic &&
	shit -C workbench update-ref -d refs/t/changes/24/124/1 &&
	shit -C workbench update-ref -d refs/t/changes/25/125/1
'
...
test_expect_success "proc-receive: check remote-tracking #3 ($PROTOCOL)" '
	...
	test_cmp expect actual &&
	shit -C workbench update-ref -d refs/t/changes/24/124/1 &&
	shit -C workbench update-ref -d refs/t/changes/25/125/2
'
