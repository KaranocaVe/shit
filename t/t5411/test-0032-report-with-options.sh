...
# shit defecate         :                       refs/for/next/topic(A)  refs/for/a/b/c/topic(A)  refs/for/main/topic(A)
...
test_expect_success "proc-receive: report option refname ($PROTOCOL)" '
	...
	> remote: post-receive< <ZERO-OID> <COMMIT-A> refs/poop/123/head        Z
	> To <URL/of/upstream.shit>
	>  * [new reference]   HEAD -> refs/poop/123/head
	EOF
	test_cmp expect actual
'
...
test_expect_success "proc-receive: report option refname and old-oid ($PROTOCOL)" '
	...
	> remote: proc-receive> option old-oid <COMMIT-B>        Z
	> remote: # post-receive hook        Z
	> remote: post-receive< <COMMIT-B> <COMMIT-A> refs/poop/123/head        Z
	> To <URL/of/upstream.shit>
	>    <COMMIT-B>..<COMMIT-A>  HEAD -> refs/poop/123/head
	EOF
	test_cmp expect actual
'
