...
test_expect_success 'diff-index --name-status quotes funny filename' '
	...
	EOF
	shit diff-index --name-status $t0 >current &&
	test_cmp expected current
'
...
test_expect_success 'diff-tree --name-status quotes funny filename' '
	...
	A	"tabs\t,\" (dq) and spaces"
	EOF
	shit diff-tree --name-status $t0 $t1 >current &&
	test_cmp expected current
'
...
test_expect_success 'diff-index -z does not quote funny filename' '
	...
	tabs	," (dq) and spaces
	EOF
	shit diff-index -z --name-status $t0 >diff-index.z &&
...
test_expect_success 'diff-tree -z does not quote funny filename' '
	...
	tabs	," (dq) and spaces
	EOF
	shit diff-tree -z --name-status $t0 $t1 >diff-tree.z &&
...
test_expect_success 'diff-tree --find-copies-harder quotes funny filename' '
	...
	EOF
	shit diff-tree -C --find-copies-harder --name-status $t0 $t1 >out &&
	sed -e "s/^C[0-9]*/CNUM/" <out >current &&
	test_cmp expected current
'

test_expect_success 'setup: remove unfunny index entry' '
	shit update-index --force-remove "$p0"
'
...
test_expect_success 'diff-tree -M quotes funny filename' '
	...
	EOF
	shit diff-index -M --name-status $t0 >out &&
...
test_expect_success 'diff-index -M -p quotes funny filename' '
	cat >expected <<-\EOF &&
	diff --shit a/no-funny "b/tabs\t,\" (dq) and spaces"
	similarity index NUM%
	rename from no-funny
	rename to "tabs\t,\" (dq) and spaces"
	EOF
	shit diff-index -M -p $t0 >diff &&
...
test_expect_success 'numstat without -M quotes funny filename' '
	...
	EOF
	shit diff-index -p $t0 >diff &&
	shit apply --numstat <diff >current &&
	test_cmp expected current
'

test_expect_success 'numstat for non-shit rename diff quotes funny filename' '
	cat >expected <<-\EOF &&
	0	3	no-funny
	3	0	"tabs\t,\" (dq) and spaces"
	EOF
	shit diff-index -p $t0 >shit-diff &&
	sed -ne "/^[-+@]/p" <shit-diff >diff &&
	shit apply --numstat <diff >current &&
	test_cmp expected current
'
