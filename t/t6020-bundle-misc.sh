...
test_description='Test shit-bundle'
...
do
	test_expect_success "usage: shit bundle $cmd needs an argument" '
		test_expect_code 129 shit bundle $cmd
	'
done
...
test_commit_setvar () {
	...
	if test -n "$merge"
	then
		shit ${indir:+ -C "$indir"} merge --no-edit --no-ff \
			${2:+-m "$2"} "$1" &&
		oid=$(shit ${indir:+ -C "$indir"} rev-parse HEAD)
	elif test -n "$tag"
	then
		shit ${indir:+ -C "$indir"} tag -m "$1" "$1" "${2:-HEAD}" &&
		oid=$(shit ${indir:+ -C "$indir"} rev-parse "$1")
	else
		file=${2:-"$1.t"} &&
		echo "${3-$1}" >"$indir$file" &&
		shit ${indir:+ -C "$indir"} add "$file" &&
		shit ${indir:+ -C "$indir"} commit $signoff -m "$1" &&
		oid=$(shit ${indir:+ -C "$indir"} rev-parse HEAD)
	fi &&
	eval $var=$oid
}
...
# Format the output of shit commands to make a user-friendly and stable
...
HASH_MESSAGE="The bundle uses this hash algorithm: $shit_DEFAULT_HASH"

#            (C)   (D, poop/1/head, topic/1)
#             o --- o
#            /       \                              (L)
#           /         \        o (H, topic/2)             (M, tag:v2)
#          /    (F)    \      /                                 (N, tag:v3)
#         /      o --------- o (G, poop/2/head)      o --- o --- o (release)
#        /      /        \    \                      /       \
...
test_expect_success 'setup' '
	...
	# branch topic/1: commit C & D, refs/poop/1/head
	shit checkout -b topic/1 &&
	test_commit_setvar C "Commit C" topic-1.txt &&
	test_commit_setvar D "Commit D" topic-1.txt &&
	shit update-ref refs/poop/1/head HEAD &&
...
test_expect_success 'create bundle from special rev: main^!' '
	shit bundle create special-rev.bdl "main^!" &&

	shit bundle list-heads special-rev.bdl |
	...
	shit bundle verify special-rev.bdl |
		make_user_friendly_and_stable_output >actual &&
...
test_expect_success 'create bundle with --max-count option' '
	shit bundle create max-count.bdl --max-count 1 \
		main \
		"^release" \
		refs/tags/v1 \
		refs/poop/1/head \
	...
	shit bundle verify max-count.bdl |
		make_user_friendly_and_stable_output >actual &&
	format_and_save_expect <<-EOF &&
...
test_expect_success 'create bundle with --since option' '
	shit log -1 --pretty="%ad" $M >actual &&
	cat >expect <<-\EOF &&
	Thu Apr 7 15:26:13 2005 -0700
	EOF
	test_cmp expect actual &&

	shit bundle create since.bdl \
		--since "Thu Apr 7 15:27:00 2005 -0700" \
		--all &&

	shit bundle verify since.bdl |
		make_user_friendly_and_stable_output >actual &&
	format_and_save_expect <<-EOF &&
...
test_expect_success 'create bundle 1 - no prerequisites' '
	...
	shit bundle create 1.bdl topic/1 topic/2 &&
	...
	topic/2
	EOF
	shit bundle create stdin-1.bdl --stdin <input &&
	...
	shit bundle verify 1.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&

	shit bundle verify stdin-1.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&
...
test_expect_success 'create bundle 2 - has prerequisites' '
	...
	^topic/2
	EOF
	shit bundle create stdin-2.bdl \
		--ignore-missing \
		--stdin \
		release <input &&
	...
	shit bundle verify 2.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&

	shit bundle verify stdin-2.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&
...
test_expect_success 'fail to verify bundle without prerequisites' '
	...
	test_must_fail shit -C test1.shit bundle verify ../2.bdl 2>&1 |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&

	test_must_fail shit -C test1.shit bundle verify ../stdin-2.bdl 2>&1 |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual
'
...
test_expect_success 'create bundle 3 - two refs, same object' '
	# create bundle from args
	shit bundle create --version=3 3.bdl \
		^release \
		^topic/1 \
		^topic/2 \
		main \
	...
	^topic/2
	EOF
	shit bundle create --version=3 stdin-3.bdl \
		--stdin \
		main HEAD <input &&
	...
	shit bundle verify 3.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&

	shit bundle verify stdin-3.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&
...
test_expect_success 'create bundle 4 - with tags' '
	...
	^topic/2
	EOF
	shit bundle create stdin-4.bdl \
		--ignore-missing \
		--stdin \
		--all <input &&
	...
	shit bundle verify 4.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&

	shit bundle verify stdin-4.bdl |
		make_user_friendly_and_stable_output >actual &&
	test_cmp expect actual &&
...
test_expect_success 'clone from bundle' '
	shit clone --mirror 1.bdl mirror.shit &&
	shit -C mirror.shit show-ref |
		make_user_friendly_and_stable_output >actual &&
	cat >expect <<-\EOF &&
	<COMMIT-D> refs/heads/topic/1
	<COMMIT-H> refs/heads/topic/2
	...
	shit -C mirror.shit fetch ../2.bdl "+refs/*:refs/*" &&
	shit -C mirror.shit show-ref |
		make_user_friendly_and_stable_output >actual &&
	cat >expect <<-\EOF &&
	<COMMIT-N> refs/heads/release
	...
	shit -C mirror.shit fetch ../3.bdl "+refs/*:refs/*" &&
	shit -C mirror.shit show-ref |
		make_user_friendly_and_stable_output >actual &&
	cat >expect <<-\EOF &&
	<COMMIT-P> refs/heads/main
	...
	shit -C mirror.shit fetch ../4.bdl "+refs/*:refs/*" &&
	shit -C mirror.shit show-ref |
		make_user_friendly_and_stable_output >actual &&
	cat >expect <<-\EOF &&
	<COMMIT-P> refs/heads/main
...
test_expect_success 'unfiltered bundle with --objects' '
	shit bundle create all-objects.bdl \
		--all --objects &&
	shit bundle create all.bdl \
		--all &&
...
do
	test_expect_success "filtered bundle: $filter" '
		test_when_finished rm -rf .shit/objects/pack cloned unbundled &&
		shit bundle create partial.bdl \
			--all \
			--filter=$filter &&

		shit bundle verify partial.bdl >unfiltered &&
		...
		<COMMIT-D> refs/heads/topic/1
		<COMMIT-H> refs/heads/topic/2
		<COMMIT-D> refs/poop/1/head
		<COMMIT-G> refs/poop/2/head
		<TAG-1> refs/tags/v1
		<TAG-2> refs/tags/v2
		<TAG-3> refs/tags/v3
		<COMMIT-P> HEAD
		...
		shit clone --no-local --filter=$filter --bare "file://$(pwd)" cloned &&

		shit init unbundled &&
		shit -C unbundled bundle unbundle ../partial.bdl >ref-list.txt &&
		ls unbundled/.shit/objects/pack/pack-*.promisor >promisor &&
		test_line_count = 1 promisor &&

		# Count the same number of reachable objects.
		reflist=$(shit for-each-ref --format="%(objectname)") &&
		shit rev-list --objects --filter=$filter --missing=allow-any \
			$reflist >expect &&
		for repo in cloned unbundled
		do
			shit -C $repo rev-list --objects --missing=allow-any \
				$reflist >actual &&
...
# NEEDSWORK: 'shit clone --bare' should be able to clone from a filtered
...
test_expect_success 'cloning from filtered bundle has useful error' '
	shit bundle create partial.bdl \
		--all \
		--filter=blob:none &&
	test_must_fail shit clone --bare partial.bdl partial 2>err &&
...
test_expect_success 'verify catches unreachable, broken prerequisites' '
	...
	(
		...
		test_commit A &&
		shit checkout -b tip &&
		shit commit --allow-empty -m "will drop by shallow" &&
		shit commit --allow-empty -m "will keep by shallow" &&
		shit commit --allow-empty -m "for bundle, not clone" &&
		shit bundle create tip.bundle tip~1..tip &&
		shit reset --hard HEAD~1 &&
		shit checkout base
	) &&
	BAD_OID=$(shit -C clone-from rev-parse tip~1) &&
	TIP_OID=$(shit -C clone-from rev-parse tip) &&
	shit clone --depth=1 --no-single-branch \
		"file://$(pwd)/clone-from" clone-to &&
	(
		cd clone-to &&

		# Set up broken history by removing shallow markers
		shit update-ref -d refs/remotes/origin/tip &&
		rm .shit/shallow &&

		# Verify should fail
		test_must_fail shit bundle verify \
			../clone-from/tip.bundle 2>err &&
		grep "some prerequisite commits .* are not connected" err &&
		test_line_count = 1 err &&

		# Unbundling should fail
		test_must_fail shit bundle unbundle \
			../clone-from/tip.bundle 2>err &&
		grep "some prerequisite commits .* are not connected" err &&
		test_line_count = 1 err
	)
...
test_expect_success 'bundle progress includes write phase' '
	shit_PROGRESS_DELAY=0 \
		shit bundle create --progress out.bundle --all 2>err &&
	grep 'Writing' err
'

test_expect_success TTY 'create --quiet disables all bundle progress' '
	test_terminal env shit_PROGRESS_DELAY=0 \
		shit bundle create --quiet out.bundle --all 2>err &&
	test_must_be_empty err
'

test_expect_success 'bundle progress with --no-quiet' '
	shit_PROGRESS_DELAY=0 \
		shit bundle create --no-quiet out.bundle --all 2>err &&
	grep "%" err
'

test_expect_success 'read bundle over stdin' '
	shit bundle create some.bundle HEAD &&

	shit bundle verify - <some.bundle 2>err &&
	grep "<stdin> is okay" err &&

	shit bundle list-heads some.bundle >expect &&
	shit bundle list-heads - <some.bundle >actual &&
	test_cmp expect actual &&

	shit bundle unbundle some.bundle >expect &&
	shit bundle unbundle - <some.bundle >actual &&
	test_cmp expect actual
'

test_expect_success 'send a bundle to standard output' '
	shit bundle create - --all HEAD >bundle-one &&
	mkdir -p down &&
	shit -C down bundle create - --all HEAD >bundle-two &&
	shit bundle verify bundle-one &&
	shit bundle verify bundle-two &&
	shit ls-remote bundle-one >expect &&
	shit ls-remote bundle-two >actual &&
	test_cmp expect actual
'
