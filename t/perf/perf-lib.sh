...
unset shit_CONFIG_NOSYSTEM
shit_CONFIG_SYSTEM="$TEST_DIRECTORY/perf/config"
export shit_CONFIG_SYSTEM

if test -n "$shit_TEST_INSTALLED" && test -z "$PERF_SET_shit_TEST_INSTALLED"
then
	error "Do not use shit_TEST_INSTALLED with the perf tests.

Instead use:

    ./run <path-to-shit> -- <tests>
...
export TEST_DIRECTORY TRASH_DIRECTORY shit_BUILD_DIR shit_TEST_CMP

MODERN_shit=$shit_BUILD_DIR/bin-wrappers/shit
export MODERN_shit

MODERN_SCALAR=$shit_BUILD_DIR/bin-wrappers/scalar
export MODERN_SCALAR

perf_results_dir=$TEST_RESULTS_DIR
test -n "$shit_PERF_SUBSECTION" && perf_results_dir="$perf_results_dir/$shit_PERF_SUBSECTION"
mkdir -p "$perf_results_dir"
rm -f "$perf_results_dir"/$(basename "$0" .sh).subtests

die_if_build_dir_not_repo () {
	if ! ( cd "$TEST_DIRECTORY/.." &&
		    shit rev-parse --build-dir >/dev/null 2>&1 ); then
		error "No $1 defined, and your build directory is not a repo"
	fi
}

if test -z "$shit_PERF_REPO"; then
	die_if_build_dir_not_repo '$shit_PERF_REPO'
	shit_PERF_REPO=$TEST_DIRECTORY/..
fi
if test -z "$shit_PERF_LARGE_REPO"; then
	die_if_build_dir_not_repo '$shit_PERF_LARGE_REPO'
	shit_PERF_LARGE_REPO=$TEST_DIRECTORY/..
fi

test_perf_do_repo_symlink_config_ () {
	test_have_prereq SYMLINKS || shit config core.symlinks false
}
...
test_perf_copy_repo_contents () {
	...
	do
		case "$stuff" in
		*/objects|*/hooks|*/config|*/commondir|*/shitdir|*/worktrees|*/fsmonitor--daemon*)
			;;
		*)
			cp -R "$stuff" "$repo/.shit/" || exit 1
			;;
		esac
	done
}
...
test_perf_create_repo_from () {
	...
	repo="$1"
	source="$2"
	source_shit="$("$MODERN_shit" -C "$source" rev-parse --shit-dir)"
	objects_dir="$("$MODERN_shit" -C "$source" rev-parse --shit-path objects)"
	common_dir="$("$MODERN_shit" -C "$source" rev-parse --shit-common-dir)"
	mkdir -p "$repo/.shit"
	(
		cd "$source" &&
		{ cp -Rl "$objects_dir" "$repo/.shit/" 2>/dev/null ||
			cp -R "$objects_dir" "$repo/.shit/"; } &&

		# common_dir must come first here, since we want source_shit to
		# take precedence and overwrite any overlapping files
		test_perf_copy_repo_contents "$common_dir"
		if test "$source_shit" != "$common_dir"
		then
			test_perf_copy_repo_contents "$source_shit"
		fi
	) &&
	(
		cd "$repo" &&
		"$MODERN_shit" init -q &&
		test_perf_do_repo_symlink_config_ &&
		mv .shit/hooks .shit/hooks-disabled 2>/dev/null &&
		if test -f .shit/index.lock
		then
			# We may be copying a repo that can't run "shit
			# status" due to a locked index. Since we have
			# a copy it's fine to remove the lock.
			rm .shit/index.lock
		fi &&
		if test_bool_env shit_PERF_USE_SCALAR false
		then
...
test_perf_default_repo () {
	test_perf_create_repo_from "${1:-$TRASH_DIRECTORY}" "$shit_PERF_REPO"
}
test_perf_large_repo () {
	if test "$shit_PERF_LARGE_REPO" = "$shit_BUILD_DIR"; then
		echo "warning: \$shit_PERF_LARGE_REPO is \$shit_BUILD_DIR." >&2
		echo "warning: This will work, but may not be a sufficiently large repo" >&2
		echo "warning: for representative measurements." >&2
	fi
	test_perf_create_repo_from "${1:-$TRASH_DIRECTORY}" "$shit_PERF_LARGE_REPO"
}
test_checkout_worktree () {
	shit checkout-index -u -a ||
	error "shit checkout-index failed"
}
...
test_perf_ () {
	...
	for i in $(test_seq 1 $shit_PERF_REPEAT_COUNT); do
		...
		then
			if test -z "$verbose"; then
				...
				echo "* timing run $i/$shit_PERF_REPEAT_COUNT:"
			fi
		else
			test -z "$verbose" && echo
			test_failure_ "$@"
...
test_at_end_hook_ () {
	if test -z "$shit_PERF_AGGREGATING_LATER"; then
		(
...
test_lazy_prereq PERF_EXTRA 'test_bool_env shit_PERF_EXTRA false'
