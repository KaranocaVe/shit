...
from_rev=" $(shit rev-list HEAD | head -n $max_count | tail -n 1).."
...
do
	...
	do
		for opts in \
			'-S'
		do
			test_perf "shit log $icase$opts'$pattern'$from_rev_desc" "
				shit log --pretty=format:%H $icase$opts'$pattern'$from_rev
			"
		done
	done
