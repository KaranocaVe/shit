...
find_pack () {
	...
	do
		if shit show-index <$idx | grep -q "$1"
		then
			basename $idx
		fi || return 1
	done
...
repack_into_n_chunks () {
	...
	sz="$(($(shit rev-list --count --all) / $1))"
	for rev in $(shit rev-list --all | awk "NR % $sz == 0" | tac)
	do
		pack="$(echo "$rev" | shit pack-objects --revs \
	...
	shit pack-objects --revs --all --honor-pack-keep --delta-base-offset \
		$packdir/pack &&
...
do
	...
	test_expect_success "setup bitmaps for $nr_packs-pack scenario" '
		...
		shit multi-pack-index write --stdin-packs --bitmap \
			--preferred-pack="$(find_pack $(shit rev-parse HEAD))"
	'

	for reuse in single multi
	do
		test_perf "clone for $nr_packs-pack scenario ($reuse-pack reuse)" "
			shit for-each-ref --format='%(objectname)' refs/heads refs/tags >in &&
			shit -c pack.allowPackReuse=$reuse pack-objects \
