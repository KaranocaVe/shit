...
test_expect_success 'create evil repo' '
	shit init tmp &&
	test_commit -C tmp a &&
	shit -C tmp config uploadpack.allowfilter 1 &&
	shit clone --filter=blob:none --no-local --no-checkout tmp evil &&
	rm -rf tmp &&

	shit -C evil config remote.origin.uploadpack \"\$TRASH_DIRECTORY/fake-upload-pack\" &&
...
test_expect_success 'local clone must not fetch from promisor remote and execute script' '
	...
	test_must_fail shit clone \
		--upload-pack="shit_TEST_ASSUME_DIFFERENT_OWNER=true shit-upload-pack" \
		evil clone1 2>err &&
...
test_expect_success 'pack-objects should fetch from promisor remote and execute script' '
	rm -f script-executed &&
	echo "HEAD" | test_must_fail shit -C evil pack-objects --revs --stdout >/dev/null 2>err &&
	test_grep "fake-upload-pack running" err &&
...
test_expect_success 'clone from promisor remote does not lazy-fetch by default' '
	rm -f script-executed &&
	test_must_fail shit clone evil no-lazy 2>err &&
...
test_expect_success 'promisor lazy-fetching can be re-enabled' '
	rm -f script-executed &&
	test_must_fail env shit_NO_LAZY_FETCH=0 \
		shit clone evil lazy-ok 2>err &&
	test_grep "fake-upload-pack running" err &&
	test_path_is_file script-executed
'
