...
test_expect_success 'two-way setup' '
	...
	shit update-index --add file1 file2 subdir/file1 subdir/file2 &&
	shit commit -m initial &&

	shit branch side &&
	shit tag -f branch-point &&

	echo file2 is not tracked on the main branch anymore &&
	rm -f file2 subdir/file2 &&
	shit update-index --remove file2 subdir/file2 &&
	shit commit -a -m "main removes file2 and subdir/file2"
'
...
echo file2 >.shitignore

test_expect_success 'two-way with incorrect --exclude-per-directory (1)' '

	if err=$(read_tree_u_must_succeed -m --exclude-per-directory=.shitignore main side 2>&1)
	then
		echo should have complained
		false
	else
...
test_expect_success 'two-way with incorrect --exclude-per-directory (2)' '
	...
	if err=$(read_tree_u_must_succeed -m -u --exclude-per-directory=foo --exclude-per-directory=.shitignore main side 2>&1)
	then
		echo should have complained
		false
	else
...
test_expect_success 'two-way clobbering a ignored file' '
	...
	read_tree_u_must_succeed -m -u --exclude-per-directory=.shitignore main side
'
...
test_expect_success 'three-way not complaining on an untracked path in both' '
	...
	shit update-index --add file3 subdir/file3 &&
	shit commit -a -m "side adds file3 and removes file2" &&
...
test_expect_success 'three-way not complaining on an untracked file' '
	...
	read_tree_u_must_succeed -m -u --exclude-per-directory=.shitignore branch-point main side
'

test_expect_success '3-way not overwriting local changes (setup)' '

	shit reset --hard &&
	shit checkout -b side-a branch-point &&
	echo >>file1 "new line to be kept in the merge result" &&
	shit commit -a -m "side-a changes file1" &&
	shit checkout -b side-b branch-point &&
	echo >>file2 "new line to be kept in the merge result" &&
	shit commit -a -m "side-b changes file2" &&
	shit checkout side-a
...
test_expect_success 'funny symlink in work tree' '
	...
	shit reset --hard &&
	shit checkout -b sym-b side-b &&
	mkdir -p a &&
	>a/b &&
	shit add a/b &&
	shit commit -m "side adds a/b" &&

	rm -fr a &&
	shit checkout -b sym-a side-a &&
	mkdir -p a &&
	test_ln_s_add ../b a/b &&
	shit commit -m "we add a/b" &&
...
test_expect_success SANITY 'funny symlink in work tree, un-unlink-able' '
	...
	shit checkout sym-a &&
	chmod a-w a &&
	test_must_fail shit read-tree -m -u sym-a sym-a sym-b
...
test_expect_success 'D/F setup' '
	...
	shit add subdir/file2/another &&
	test_tick &&
	shit commit -m "side-a changes file2 to directory"

'

test_expect_success 'D/F' '

	shit checkout side-b &&
	read_tree_u_must_succeed -m -u branch-point side-b side-a &&
	shit ls-files -u >actual &&
	(
		a=$(shit rev-parse branch-point:subdir/file2) &&
		b=$(shit rev-parse side-a:subdir/file2/another) &&
...
test_expect_success 'D/F resolve' '
	...
	shit reset --hard &&
	shit checkout side-b &&
	shit merge-resolve branch-point -- side-b side-a
...
test_expect_success 'D/F recursive' '
	...
	shit reset --hard &&
	shit checkout side-b &&
	shit merge-recursive branch-point -- side-b side-a
