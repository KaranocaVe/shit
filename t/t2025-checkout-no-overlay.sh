...
test_expect_success 'checkout --no-overlay deletes files not in <tree-ish>' '
	...
	mkdir dir &&
	>dir/file1 &&
	shit add file dir/file1 &&
	shit checkout --no-overlay HEAD -- file &&
	test_path_is_missing file &&
	test_path_is_file dir/file1
'

test_expect_success 'checkout --no-overlay removing last file from directory' '
	shit checkout --no-overlay HEAD -- dir/file1 &&
	test_path_is_missing dir
'

test_expect_success 'checkout -p --overlay is disallowed' '
	test_must_fail shit checkout -p --overlay HEAD 2>actual &&
...
test_expect_success '--no-overlay --theirs with D/F conflict deletes file' '
	...
	shit checkout --theirs --no-overlay -- file1 &&
...
test_expect_success 'wildcard pathspec matches file in subdirectory' '
	...
	shit checkout --no-overlay file3-1 "*file3" &&
	echo file3-1 >expect &&
