...
test_description='shit blame textconv support'
. ./test-lib.sh
...
test_expect_success 'setup ' '
	...
	shit add . &&
	shit_AUTHOR_NAME=Number1 shit commit -a -m First --date="2010-01-01 18:00:00" &&
	echo "bin: test 1 version 2" >one.bin &&
	echo "bin: test number 2 version 2" >>two.bin &&
	rm -f symlink.bin &&
	test_ln_s_add two.bin symlink.bin &&
	shit_AUTHOR_NAME=Number2 shit commit -a -m Second --date="2010-01-01 20:00:00"
'
...
test_expect_success 'no filter specified' '
	shit blame one.bin >blame &&
	find_blame Number2 <blame >result &&
	test_cmp expected result
'

test_expect_success 'setup textconv filters' '
	echo "*.bin diff=test" >.shitattributes &&
	echo "zero.bin eol=crlf" >>.shitattributes &&
	shit config diff.test.textconv ./helper &&
	shit config diff.test.cachetextconv false
'

test_expect_success 'blame with --no-textconv' '
	shit blame --no-textconv one.bin >blame &&
...
test_expect_success 'blame --textconv going through revisions' '
	shit blame --textconv two.bin >blame &&
	find_blame <blame >result &&
	test_cmp expected result
'

test_expect_success 'blame --textconv with local changes' '
	test_when_finished "shit checkout zero.bin" &&
	printf "bin: updated number 0\015" >zero.bin &&
	shit blame --textconv zero.bin >blame &&
...
test_expect_success 'setup +cachetextconv' '
	shit config diff.test.cachetextconv true
'
...
test_expect_success 'blame --textconv works with textconvcache' '
	...
	find_blame <blame >result &&
	test_cmp expected result &&
	shit blame --textconv one.bin >blame &&
	find_blame  <blame >result &&
...
test_expect_success 'make a new commit' '
	...
	shit_AUTHOR_NAME=Number3 shit commit -a -m Third --date="2010-01-01 22:00:00"
'

test_expect_success 'blame from previous revision' '
	shit blame HEAD^ two.bin >blame &&
	find_blame <blame >result &&
	test_cmp expected result
'
...
test_expect_success SYMLINKS 'blame with --no-textconv (on symlink)' '
	shit blame --no-textconv symlink.bin >blame &&
	find_blame <blame >result &&
	test_cmp expected result
'

test_expect_success SYMLINKS 'blame --textconv (on symlink)' '
	shit blame --textconv symlink.bin >blame &&
	find_blame <blame >result &&
	test_cmp expected result
'
...
EOF
	shit add three.bin &&
	shit_AUTHOR_NAME=Number4 shit commit -a -m Fourth --date="2010-01-01 23:00:00"
'

test_expect_success 'blame on last commit (-C -C, symlink)' '
	shit blame -C -C three.bin >blame &&
	find_blame <blame >result &&
	cat >expected <<\EOF &&
(Number1 2010-01-01 18:00:00 +0000 1) converted: test number 2
(Number2 2010-01-01 20:00:00 +0000 2) converted: test number 2 version 2
