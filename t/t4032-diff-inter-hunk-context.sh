...
t() {
	use_config=
	shit config --unset diff.interHunkContext

	case $# in
	4) hunks=$4; cmd="diff -U$3";;
	5) hunks=$5; cmd="diff -U$3 --inter-hunk-context=$4";;
	6) hunks=$5; cmd="diff -U$3"; shit config diff.interHunkContext $4; use_config="(diff.interHunkContext=$4) ";;
	esac
	...
	test_expect_success "$label: count hunks ($hunks)" "
		test $(shit $cmd $file | grep '^@@ ' | wc -l) = $hunks
	"

	test -f $expected &&
	test_expect_success "$label: check output" "
		shit $cmd $file | grep -v '^index ' >actual &&
...
test_expect_success 'diff.interHunkContext invalid' '
	shit config diff.interHunkContext asdf &&
	test_must_fail shit diff &&
	shit config diff.interHunkContext -1 &&
	test_must_fail shit diff
'
