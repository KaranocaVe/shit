...
test_description='shit p4 unshelve'
...
EOF
	...
	(
		...
		shit p4 unshelve $change &&
		shit show refs/remotes/p4-unshelved/$change >actual &&
		grep -q "Further description" actual &&
		shit cherry-pick refs/remotes/p4-unshelved/$change &&
...
EOF
	...
	(
		...
		change=$(last_shelved_change) &&
		shit p4 unshelve $change &&
		shit diff refs/remotes/p4-unshelved/$change.0 refs/remotes/p4-unshelved/$change | grep -q file3
	)
'
...
# match shit-p4's idea of the base revision
#
...
test_expect_success 'try to unshelve the change' '
	...
	(
		change=$(last_shelved_change) &&
		cd "$shit" &&
		shit p4 unshelve $change >out.txt &&
...
test_expect_success 'unshelve specifying the origin' '
	...
	(
		cd "$shit" &&
		change=$(last_shelved_change) &&
		shit p4 unshelve --origin HEAD~ $change &&
		shit checkout refs/remotes/p4-unshelved/$change &&
