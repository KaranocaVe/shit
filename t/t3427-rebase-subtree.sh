...
test_description='shit rebase tests for -Xsubtree

This test runs shit rebase and tests the subtree strategy.
'
...
# directory directly.
#
# Then, in subsequent test cases, `shit filter-branch` is used to distill just
...
test_expect_success 'setup' '
	...
	shit fetch files refs/heads/main:refs/heads/files-main &&
	shit merge -s ours --no-commit --allow-unrelated-histories \
		files-main &&
	shit read-tree --prefix=files_subtree -u files-main &&
	shit commit -m "Add subproject main" &&
	...
	shit fast-export --no-data HEAD -- files_subtree/ |
		sed -e "s%\([0-9a-f]\{40\} \)files_subtree/%\1%" |
		shit fast-import --force --quiet &&
	shit reset --hard &&
...
test_expect_success 'Rebase -Xsubtree --empty=ask --onto commit' '
	reset_rebase &&
	shit checkout -b rebase-onto to-rebase &&
	test_must_fail shit rebase -Xsubtree=files_subtree --empty=ask --onto files-main main &&
...
test_expect_success 'Rebase -Xsubtree --empty=ask --rebase-merges --onto commit' '
	reset_rebase &&
	shit checkout -b rebase-merges-onto to-rebase &&
	test_must_fail shit rebase -Xsubtree=files_subtree --empty=ask --rebase-merges --onto files-main --root &&
	: first pick results in no changes &&
	shit rebase --skip &&
	test "$(commit_message HEAD~2)" = "topic_4" &&
	test "$(commit_message HEAD~)" = "files_subtree/topic_5" &&
