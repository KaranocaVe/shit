...
enable_global_excludes () {
	init_vars &&
	shit config core.excludesfile "$global_excludes"
}
...
test_check_ignore () {
	...
	echo shit $global_args check-ignore $quiet_opt $verbose_opt $non_matching_opt $no_index_opt $args \
		>"$HOME/cmd" &&
	echo "$expect_code" >"$HOME/expected-exit-code" &&
	test_expect_code "$expect_code" \
		shit $global_args check-ignore $quiet_opt $verbose_opt $non_matching_opt $no_index_opt $args \
...
test_expect_success 'setup' '
	...
	(
		...
		echo a >a &&
		shit add a &&
		shit commit -m"commit in submodule"
	) &&
	shit add a/submodule &&
	cat <<-\EOF >.shitignore &&
		one
		ignored-*
		top-level-dir/
	...
	shit add -f ignored-but-in-index a/ignored-but-in-index &&
	cat <<-\EOF >a/.shitignore &&
		two*
		*three
	EOF
	cat <<-\EOF >a/b/.shitignore &&
		four
		five
		...
		!two
	EOF
	echo "seven" >a/b/ignored-dir/.shitignore &&
	test -n "$HOME" &&
...
do
	...
	test_expect_success_no_index_multi "existing tracked file $where shown as ignored" \
		".shitignore:2:ignored-*	${subdir}ignored-but-in-index" \
		"test_check_ignore '${subdir}ignored-but-in-index'"

	test_expect_success_multi "existing untracked file $where ignored" \
		".shitignore:2:ignored-*	${subdir}ignored-and-untracked" \
...
test_expect_success 'sub-directory local ignore with --verbose'  '
	expect "a/.shitignore:2:*three	a/3-three" &&
...
test_expect_success 'local ignore inside a sub-directory with --verbose' '
	expect "a/.shitignore:2:*three	3-three" &&
	(
		cd a &&
...
test_expect_success 'nested include of negated pattern with -v' '
	expect "a/b/.shitignore:8:!on*	a/b/one" &&
...
test_expect_success_multi 'ignored sub-directory' \
	'a/b/.shitignore:5:ignored-dir/	a/b/ignored-dir' '
...
test_expect_success 'multiple files inside ignored sub-directory with -v' '
	expect_from_stdin <<-\EOF &&
		a/b/.shitignore:5:ignored-dir/	a/b/ignored-dir/foo
		a/b/.shitignore:5:ignored-dir/	a/b/ignored-dir/twoooo
		a/b/.shitignore:5:ignored-dir/	a/b/ignored-dir/seven
	EOF
...
test_expect_success 'cd to ignored sub-directory with -v' '
	expect_from_stdin <<-\EOF &&
		a/b/.shitignore:5:ignored-dir/	foo
		a/b/.shitignore:5:ignored-dir/	twoooo
		a/b/.shitignore:8:!on*	../one
		a/b/.shitignore:5:ignored-dir/	seven
		.shitignore:1:one	../../one
	EOF
	(
		cd a/b/ignored-dir &&
...
test_expect_success 'global ignore not yet enabled' '
	expect_from_stdin <<-\EOF &&
		.shit/info/exclude:1:per-repo	per-repo
		a/.shitignore:2:*three	a/globalthree
		.shit/info/exclude:1:per-repo	a/per-repo
	EOF
...
cat <<-EOF >expected-verbose
	.shitignore:1:one	one
	.shitignore:1:one	a/one
	a/b/.shitignore:8:!on*	a/b/on
	a/b/.shitignore:8:!on*	a/b/one
	a/b/.shitignore:8:!on*	a/b/one one
	a/b/.shitignore:8:!on*	a/b/one two
	a/b/.shitignore:8:!on*	"a/b/one\\"three"
	a/b/.shitignore:9:!two	a/b/two
	a/.shitignore:1:two*	a/b/twooo
	$global_excludes:2:!globaltwo	globaltwo
...
cat <<-EOF >expected-all
	...
	.shitignore:1:one	one
	::	not-ignored
	a/b/.shitignore:8:!on*	b/on
	a/b/.shitignore:8:!on*	b/one
	a/b/.shitignore:8:!on*	b/one one
	a/b/.shitignore:8:!on*	b/one two
	a/b/.shitignore:8:!on*	"b/one\\"three"
	a/b/.shitignore:9:!two	b/two
	::	b/not-ignored
	a/.shitignore:1:two*	b/twooo
	$global_excludes:2:!globaltwo	../globaltwo
	$global_excludes:2:!globaltwo	globaltwo
	$global_excludes:2:!globaltwo	b/globaltwo
	$global_excludes:2:!globaltwo	../b/globaltwo
...
test_expect_success PIPE 'streaming support for --stdin' '
	mkfifo in out &&
	(shit check-ignore -n -v --stdin <in >out &) &&
	...
	echo >&9 one &&
	read response <&8 &&
	echo "$response" | grep "^\.shitignore:1:one	one" &&
	echo >&9 two &&
	read response <&8 &&
...
test_expect_success 'existing file and directory' '
	...
	>one &&
	mkdir top-level-dir &&
	shit check-ignore one top-level-dir >actual &&
	grep one actual &&
...
test_expect_success 'existing directory and file' '
	...
	>one &&
	mkdir top-level-dir &&
	shit check-ignore top-level-dir one >actual &&
	grep one actual &&
	grep top-level-dir actual
'
...
test_expect_success 'exact prefix matching (with root)' '
	...
	mkdir -p a/shit a/shit-foo &&
	touch a/shit/foo a/shit-foo/bar &&
	echo /shit/ >a/.shitignore &&
	shit check-ignore a/shit a/shit/foo a/shit-foo a/shit-foo/bar >actual &&
	cat >expect <<-\EOF &&
	a/shit
	a/shit/foo
	EOF
...
whitespace/untracked
EOF
	shit ls-files -o -X ignore whitespace >actual 2>err &&
	test_cmp expect actual &&
...
test_expect_success 'info/exclude trumps core.excludesfile' '
	...
	echo >>.shit/info/exclude "!usually-ignored" &&
	>usually-ignored &&
	echo "?? usually-ignored" >expect &&

	shit status --porcelain usually-ignored >actual &&
	test_cmp expect actual
'

test_expect_success SYMLINKS 'set up ignore file for symlink tests' '
	echo "*" >ignore &&
	rm -f .shitignore .shit/info/exclude
'
...
test_expect_success SYMLINKS 'symlinks respected in core.excludesFile' '
	...
	echo file >expect &&
	shit check-ignore file >actual 2>err &&
	test_cmp expect actual &&
	test_must_be_empty err
'

test_expect_success SYMLINKS 'symlinks respected in info/exclude' '
	test_when_finished "rm .shit/info/exclude" &&
	ln -s ../../ignore .shit/info/exclude &&
	echo file >expect &&
	shit check-ignore file >actual 2>err &&
	test_cmp expect actual &&
	test_must_be_empty err
...
test_expect_success SYMLINKS 'symlinks not respected in-tree' '
	test_when_finished "rm .shitignore" &&
	ln -s ignore .shitignore &&
	mkdir subdir &&
	ln -s ignore subdir/.shitignore &&
	test_must_fail shit check-ignore subdir/file >actual 2>err &&
	test_must_be_empty actual &&
	test_grep "unable to access.*shitignore" err
'
