...
test_description='test shit fast-import utility'
...
verify_packs () {
	for p in .shit/objects/pack/*.pack
	do
		shit verify-pack "$@" "$p" || return
	done
}
...
test_expect_success 'empty stream succeeds' '
	...
	shit fast-import </dev/null
'

test_expect_success 'truncated stream complains' '
	echo "tag foo" | test_must_fail shit fast-import
'
...
test_expect_success 'A: create pack from stdin' '
	...
	INPUT_END
	shit fast-import --export-marks=marks.out <input &&
...
test_expect_success 'A: verify commit' '
	cat >expect <<-EOF &&
	author $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE

	initial
	EOF
	shit cat-file commit main | sed 1d >actual &&
...
test_expect_success 'A: verify tree' '
	...
	100755 blob file4
	EOF
	shit cat-file -p main^{tree} | sed "s/ [0-9a-f]*	/ /" >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify file2' '
	echo "$file2_data" >expect &&
	shit cat-file blob main:file2 >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify file3' '
	echo "$file3_data" >expect &&
	shit cat-file blob main:file3 >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify file4' '
	printf "$file4_data" >expect &&
	shit cat-file blob main:file4 >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify tag/series-A' '
	cat >expect <<-EOF &&
	object $(shit rev-parse refs/heads/main)
	type commit
	tag series-A

	An annotated tag without a tagger
	EOF
	shit cat-file tag tags/series-A >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify tag/series-A-blob' '
	cat >expect <<-EOF &&
	object $(shit rev-parse refs/heads/main:file3)
	type blob
	tag series-A-blob

	An annotated tag that annotates a blob.
	EOF
	shit cat-file tag tags/series-A-blob >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify tag deletion is successful' '
	test_must_fail shit rev-parse --verify refs/tags/to-be-deleted
'

test_expect_success 'A: verify marks output' '
	cat >expect <<-EOF &&
	:2 $(shit rev-parse --verify main:file2)
	:3 $(shit rev-parse --verify main:file3)
	:4 $(shit rev-parse --verify main:file4)
	:5 $(shit rev-parse --verify main^0)
	:6 $(shit cat-file tag nested | grep object | cut -d" " -f 2)
	:7 $(shit rev-parse --verify nested)
	:8 $(shit rev-parse --verify main^0)
	EOF
...
test_expect_success 'A: tag blob by sha1' '
	test_tick &&
	new_blob=$(echo testing | shit hash-object --stdin) &&
	cat >input <<-INPUT_END &&
	tag series-A-blob-2
	from $(shit rev-parse refs/heads/main:file3)
	data <<EOF
	Tag blob by sha1.
	EOF
	...
	shit cat-file tag tags/series-A-blob-2 >actual &&
	shit cat-file tag tags/series-A-blob-3 >>actual &&
	test_cmp expect actual
'
...
test_expect_success 'A: verify marks import does not crash' '
	...
	shit fast-import --import-marks=marks.out <input &&
	shit whatchanged verify--import-marks
'
...
test_expect_success 'A: verify diff' '
	copy=$(shit rev-parse --verify main:file2) &&
	cat >expect <<-EOF &&
	:000000 100755 $ZERO_OID $copy A	copy-of-file2
	EOF
	shit diff-tree -M -r main verify--import-marks >actual &&
	compare_diff_raw expect actual &&
	test $(shit rev-parse --verify main:file2) \
	    = $(shit rev-parse --verify verify--import-marks:copy-of-file2)
'

test_expect_success 'A: export marks with large values' '
	test_tick &&
	mt=$(shit hash-object --stdin < /dev/null) &&
	>input.blob &&
	>marks.exp &&
	>tree.exp &&
	...
	cat input.blob input.commit | shit fast-import --export-marks=marks.large &&
	shit ls-tree refs/heads/verify--dump-marks >tree.out &&
...
test_expect_success 'B: accept branch name "TEMP_TAG"' '
	...
	test_when_finished "rm -f .shit/TEMP_TAG && shit gc --prune=now" &&
	shit fast-import <input &&
	test $(test-tool ref-store main resolve-ref TEMP_TAG 0 | cut -f1 -d " " ) != "$ZERO_OID" &&
	test $(shit rev-parse main) = $(shit rev-parse TEMP_TAG^)
'

test_expect_success 'B: accept empty committer' '
	cat >input <<-INPUT_END &&
	commit refs/heads/empty-committer-1
	committer  <> $shit_COMMITTER_DATE
	data <<COMMIT
	empty commit
	COMMIT
	INPUT_END

	test_when_finished "shit update-ref -d refs/heads/empty-committer-1
		shit gc --prune=now" &&
	shit fast-import <input &&
	out=$(shit fsck) &&
	echo "$out" &&
...
test_expect_success 'B: reject invalid timezone' '
	...
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> 1234567890 +051800
	data <<COMMIT
	empty commit
	COMMIT
	INPUT_END

	test_when_finished "shit update-ref -d refs/heads/invalid-timezone" &&
	test_must_fail shit fast-import <input
'
...
test_expect_success 'B: accept invalid timezone with raw-permissive' '
	...
	shit init invalid-timezone &&
	shit -C invalid-timezone fast-import --date-format=raw-permissive <input &&
	shit -C invalid-timezone cat-file -p invalid-timezone >out &&
...
test_expect_success 'B: accept and fixup committer with no name' '
	...
	committer <a@b.com> $shit_COMMITTER_DATE
	data <<COMMIT
	empty commit
	COMMIT
	INPUT_END

	test_when_finished "shit update-ref -d refs/heads/empty-committer-2
		shit gc --prune=now" &&
	shit fast-import <input &&
	out=$(shit fsck) &&
	echo "$out" &&
...
test_expect_success 'B: fail on invalid committer (1)' '
	...
	committer Name email> $shit_COMMITTER_DATE
	data <<COMMIT
	empty commit
	COMMIT
	INPUT_END

	test_when_finished "shit update-ref -d refs/heads/invalid-committer" &&
	test_must_fail shit fast-import <input
'
...
test_expect_success 'B: fail on invalid committer (2)' '
	cat >input <<-INPUT_END &&
	commit refs/heads/invalid-committer
	committer Name <e<mail> $shit_COMMITTER_DATE
	data <<COMMIT
	empty commit
	COMMIT
	INPUT_END
...
test_expect_success 'C: incremental import create pack from stdin' '
	newf=$(echo hi newf | shit hash-object -w --stdin) &&
	oldf=$(shit rev-parse --verify main:file2) &&
	thrf=$(shit rev-parse --verify main:file3) &&
	test_tick &&
	cat >input <<-INPUT_END &&
	commit refs/heads/branch
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
...
test_expect_success 'C: validate reuse existing blob' '
	test $newf = $(shit rev-parse --verify branch:file2/newf) &&
	test $oldf = $(shit rev-parse --verify branch:file2/oldf)
'

test_expect_success 'C: verify commit' '
	cat >expect <<-EOF &&
	parent $(shit rev-parse --verify main^0)
	author $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	...
	shit cat-file commit branch | sed 1d >actual &&
	test_cmp expect actual
'
...
test_expect_success 'C: validate rename result' '
	...
	EOF
	shit diff-tree -M -r main branch >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success 'D: validate new files added' '
	f5id=$(echo "$file5_data" | shit hash-object --stdin) &&
	f6id=$(echo "$file6_data" | shit hash-object --stdin) &&
	cat >expect <<-EOF &&
	:000000 100755 $ZERO_OID $f6id A	newdir/exec.sh
	:000000 100644 $ZERO_OID $f5id A	newdir/interesting
	EOF
	shit diff-tree -M -r branch^ branch >actual &&
	compare_diff_raw expect actual
'

test_expect_success 'D: verify file5' '
	echo "$file5_data" >expect &&
	shit cat-file blob branch:newdir/interesting >actual &&
	test_cmp expect actual
'

test_expect_success 'D: verify file6' '
	echo "$file6_data" >expect &&
	shit cat-file blob branch:newdir/exec.sh >actual &&
	test_cmp expect actual
'
...
test_expect_success 'E: rfc2822 date, --date-format=raw' '
	cat >input <<-INPUT_END &&
	commit refs/heads/branch
	author $shit_AUTHOR_NAME <$shit_AUTHOR_EMAIL> Tue Feb 6 11:22:18 2007 -0500
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> Tue Feb 6 12:35:02 2007 -0500
	data <<COMMIT
	RFC 2822 type date
	COMMIT
	...
	test_must_fail shit fast-import --date-format=raw <input
'
test_expect_success 'E: rfc2822 date, --date-format=rfc2822' '
	shit fast-import --date-format=rfc2822 <input
'
...
test_expect_success 'E: verify commit' '
	cat >expect <<-EOF &&
	author $shit_AUTHOR_NAME <$shit_AUTHOR_EMAIL> 1170778938 -0500
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> 1170783302 -0500

	RFC 2822 type date
	EOF
	shit cat-file commit branch | sed 1,2d >actual &&
	test_cmp expect actual
'
...
test_expect_success 'F: non-fast-forward update skips' '
	old_branch=$(shit rev-parse --verify branch^0) &&
	test_tick &&
	cat >input <<-INPUT_END &&
	commit refs/heads/branch
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	...
	test $old_branch = $(shit rev-parse --verify branch^0)
'
...
test_expect_success 'F: verify other commit' '
	cat >expect <<-EOF &&
	tree $(shit rev-parse branch~1^{tree})
	parent $(shit rev-parse branch~1)
	author $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE

	losing things already?
	EOF
	shit cat-file commit other >actual &&
	test_cmp expect actual
'
...
test_expect_success 'G: branch changed, but logged' '
	test $old_branch != $(shit rev-parse --verify branch^0) &&
	test $old_branch = $(shit rev-parse --verify branch@{1})
'
...
test_expect_success 'H: validate old files removed, new files added' '
	f4id=$(shit rev-parse HEAD:file4) &&
	cat >expect <<-EOF &&
	...
	EOF
	shit diff-tree -M -r H^ H >actual &&
	compare_diff_raw expect actual
'

test_expect_success 'H: verify file' '
	echo "$file5_data" >expect &&
	shit cat-file blob H:h/e/l/lo >actual &&
	test_cmp expect actual
'
...
test_expect_success 'I: export-pack-edges' '
	...
	INPUT_END
	shit fast-import --export-pack-edges=edges.list <input
'

test_expect_success 'I: verify edge list' '
	cat >expect <<-EOF &&
	.shit/objects/pack/pack-.pack: $(shit rev-parse --verify export-boundary)
	EOF
...
test_expect_success 'J: branch has 1 commit, empty tree' '
	test 1 = $(shit rev-list J | wc -l) &&
	test 0 = $(shit ls-tree J | wc -l)
'
...
test_expect_success 'K: verify K^1 = branch^1' '
	test $(shit rev-parse --verify branch^1) \
		= $(shit rev-parse --verify K^1)
'
...
test_expect_success 'L: verify internal tree sorting' '
	...
	shit fast-import <input &&
	shit_PRINT_SHA1_ELLIPSIS="yes" shit diff-tree --abbrev --raw L^ L >output &&
...
test_expect_success 'L: nested tree copy does not corrupt deltas' '
	...
	test_when_finished "shit update-ref -d refs/heads/L2" &&
	shit fast-import <input &&
	shit ls-tree L2 g/b/ >tmp &&
	cut -f 2 <tmp >actual &&
	test_cmp expect actual &&
	shit fsck $(shit rev-parse L2)
'
...
test_expect_success 'M: rename file in same subdirectory' '
	...
	EOF
	shit fast-import <input &&
	shit diff-tree -M -r M1^ M1 >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success 'M: rename file to new subdirectory' '
	...
	EOF
	shit fast-import <input &&
	shit diff-tree -M -r M2^ M2 >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success 'M: rename subdirectory to new subdirectory' '
	...
	EOF
	shit fast-import <input &&
	shit diff-tree -M -r M3^ M3 >actual &&
	compare_diff_raw expect actual
'
...
do
	test_expect_success "M: rename root ($root) to subdirectory" '
		...
		EOF
		shit fast-import <input &&
		shit diff-tree -M -r M4^ M4 >actual &&
		compare_diff_raw expect actual
	'
done
...
test_expect_success 'N: copy file in same subdirectory' '
	...
	EOF
	shit fast-import <input &&
	shit diff-tree -C --find-copies-harder -r N1^ N1 >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success 'N: copy then modify subdirectory' '
	...
	EOF
	shit fast-import <input &&
	shit diff-tree -C --find-copies-harder -r N2^^ N2 >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success 'N: copy dirty subdirectory' '
	...
	shit fast-import <input &&
	test $(shit rev-parse N2^{tree}) = $(shit rev-parse N3^{tree})
'
...
test_expect_success 'N: copy directory by id' '
	...
	:100644 100644 $oldf $oldf C100	file2/oldf	file3/oldf
	EOF
	subdir=$(shit rev-parse refs/heads/branch^0:file2) &&
	cat >input <<-INPUT_END &&
	...
	INPUT_END
	shit fast-import <input &&
	shit diff-tree -C --find-copies-harder -r N4^ N4 >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success PIPE 'N: read and copy directory' '
	...
	:100644 100644 $oldf $oldf C100	file2/oldf	file3/oldf
	EOF
	shit update-ref -d refs/heads/N4 &&
	rm -f backflow &&
	mkfifo backflow &&
	(
		exec <backflow &&
	...
	shit fast-import --cat-blob-fd=3 3>backflow &&
	shit diff-tree -C --find-copies-harder -r N4^ N4 >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success PIPE 'N: empty directory reads as missing' '
	...
	shit update-ref -d refs/heads/read-empty &&
	rm -f backflow &&
	mkfifo backflow &&
	(
		exec <backflow &&
	...
	shit rev-list read-empty |
	shit diff-tree -r --root --stdin |
...
do
	test_expect_success "N: copy root ($root) by tree hash" '
		...
		EOF
		root_tree=$(shit rev-parse refs/heads/branch^0^{tree}) &&
		cat >input <<-INPUT_END &&
		...
		INPUT_END
		shit fast-import <input &&
		shit diff-tree -C --find-copies-harder -r N4 N6 >actual &&
		compare_diff_raw expect actual
	'
	...
	test_expect_success "N: copy root ($root) by path" '
		...
		INPUT_END
		shit fast-import <input &&
		shit diff-tree -C --find-copies-harder -r branch N-copy-root-path >actual &&
		compare_diff_raw expect actual
	'
done
...
test_expect_success 'N: delete directory by copying' '
	...
	EOF
	empty_tree=$(shit mktree </dev/null) &&
	cat >input <<-INPUT_END &&
	...
	INPUT_END
	shit fast-import <input &&
	shit rev-list N-delete |
		shit diff-tree -r --stdin --root --always |
...
test_expect_success 'N: modify copied tree' '
	...
	INPUT_END
	shit fast-import <input &&
	shit diff-tree -C --find-copies-harder -r N5^^ N5 >actual &&
	compare_diff_raw expect actual
'

test_expect_success 'N: reject foo/ syntax' '
	subdir=$(shit rev-parse refs/heads/branch^0:file2) &&
	test_must_fail shit fast-import <<-INPUT_END
...
do
	test_expect_success "N: copy to root ($root) by id and modify" '
		...
		tree=$(shit rev-parse --verify N7:) &&
		shit fast-import <<-INPUT_END &&
		...
		EOF
		INPUT_END
		shit show N8:foo/foo >actual.foo &&
		shit show N8:foo/bar >actual.bar &&
		test_cmp expect.foo actual.foo &&
		test_cmp expect.bar actual.bar
	'

	test_expect_success "N: extract subtree to the root ($root)" '
		branch=$(shit rev-parse --verify refs/heads/branch^{tree}) &&
		cat >input <<-INPUT_END &&
		...
		INPUT_END
		shit fast-import <input &&
		shit diff --exit-code branch:newdir N9
	'
	...
	test_expect_success "N: modify subtree, extract it to the root ($root), and modify again" '
		...
		tree=$(shit rev-parse --verify N10:) &&
		shit fast-import <<-INPUT_END &&
		...
		C "bar/qux" "bar/quux"
		INPUT_END
		shit show N11:bar/baz >actual.baz &&
		shit show N11:bar/qux >actual.qux &&
		shit show N11:bar/quux >actual.quux &&
...
test_expect_success 'O: comments are all skipped' '
	...
	shit fast-import <input &&
	test $(shit rev-parse N3) = $(shit rev-parse O1)
'
...
test_expect_success 'O: blank lines not necessary after data commands' '
	...
	shit fast-import <input &&
	test $(shit rev-parse N3) = $(shit rev-parse O2)
'
...
test_expect_success 'O: blank lines not necessary after other commands' '
	...
	shit fast-import <input &&
	ls -la .shit/objects/pack/pack-*.pack >packlist &&
	ls -la .shit/objects/pack/pack-*.pack >idxlist &&
	test_line_count = 4 idxlist &&
	test_line_count = 4 packlist &&
	test $(shit rev-parse refs/tags/O3-2nd) = $(shit rev-parse O3^) &&
	shit log --reverse --pretty=oneline O3 | sed s/^.*z// >actual &&
	test_cmp expect actual
'
...
test_expect_success 'P: superproject & submodule mix' '
	...
	shit fast-import <input &&
	shit checkout subuse1 &&
	rm -rf sub &&
	mkdir sub &&
	(
		cd sub &&
		shit init &&
		shit fetch --update-head-ok .. refs/heads/sub:refs/heads/main &&
		shit checkout main
	) &&
	shit submodule init &&
...
test_expect_success 'P: verbatim SHA shitlinks' '
	SUBLAST=$(shit rev-parse --verify sub) &&
	SUBPREV=$(shit rev-parse --verify sub^) &&
	...
	shit gc --prune=now &&
	shit fast-import <input &&
	test $(shit rev-parse --verify subuse2) = $(shit rev-parse --verify subuse1)
'

test_expect_success 'P: fail on inline shitlink' '
	test_tick &&
	cat >input <<-INPUT_END &&
...
test_expect_success 'P: fail on blob mark in shitlink' '
	test_tick &&
	cat >input <<-INPUT_END &&
	blob
	mark :1
...
test_expect_success 'Q: verify first commit' '
	commit1=$(shit rev-parse notes-test~2) &&
	commit2=$(shit rev-parse notes-test^) &&
	commit3=$(shit rev-parse notes-test) &&
	...
	first (:3)
	EOF
	shit cat-file commit notes-test~2 | sed 1d >actual &&
	test_cmp expect actual
'
...
test_expect_success 'Q: verify second commit' '
	...
	second (:5)
	EOF
	shit cat-file commit notes-test^ | sed 1d >actual &&
	test_cmp expect actual
'
...
test_expect_success 'Q: verify first notes commit' '
	...
	notes (:9)
	EOF
	shit cat-file commit refs/notes/foobar~2 | sed 1d >actual &&
	test_cmp expect actual
'
...
test_expect_success 'Q: verify first notes tree' '
	...
	100644 blob $commit3
	EOF
	shit cat-file -p refs/notes/foobar~2^{tree} | sed "s/ [0-9a-f]*	/ /" >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify first note for first commit' '
	echo "$note1_data" >expect &&
	shit cat-file blob refs/notes/foobar~2:$commit1 >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify first note for second commit' '
	echo "$note2_data" >expect &&
	shit cat-file blob refs/notes/foobar~2:$commit2 >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify first note for third commit' '
	echo "$note3_data" >expect &&
	shit cat-file blob refs/notes/foobar~2:$commit3 >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify second notes commit' '
	cat >expect <<-EOF &&
	parent $(shit rev-parse --verify refs/notes/foobar~2)
	author $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE

	notes (:10)
	EOF
	shit cat-file commit refs/notes/foobar^ | sed 1d >actual &&
	test_cmp expect actual
'
...
test_expect_success 'Q: verify second notes tree' '
	...
	100644 blob $commit3
	EOF
	shit cat-file -p refs/notes/foobar^^{tree} | sed "s/ [0-9a-f]*	/ /" >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify second note for first commit' '
	echo "$note1b_data" >expect &&
	shit cat-file blob refs/notes/foobar^:$commit1 >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify first note for second commit' '
	echo "$note2_data" >expect &&
	shit cat-file blob refs/notes/foobar^:$commit2 >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify first note for third commit' '
	echo "$note3_data" >expect &&
	shit cat-file blob refs/notes/foobar^:$commit3 >actual &&
	test_cmp expect actual
'
...
test_expect_success 'Q: verify third notes commit' '
	...
	notes (:11)
	EOF
	shit cat-file commit refs/notes/foobar2 | sed 1d >actual &&
	test_cmp expect actual
'
...
test_expect_success 'Q: verify third notes tree' '
	...
	100644 blob $commit1
	EOF
	shit cat-file -p refs/notes/foobar2^{tree} | sed "s/ [0-9a-f]*	/ /" >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify third note for first commit' '
	echo "$note1c_data" >expect &&
	shit cat-file blob refs/notes/foobar2:$commit1 >actual &&
	test_cmp expect actual
'

test_expect_success 'Q: verify fourth notes commit' '
	cat >expect <<-EOF &&
	parent $(shit rev-parse --verify refs/notes/foobar^)
	author $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
...
test_expect_success 'R: import-marks features forbidden by default' '
	>shit.marks &&
	echo "feature import-marks=shit.marks" >input &&
	test_must_fail shit fast-import <input &&
	echo "feature import-marks-if-exists=shit.marks" >input &&
	test_must_fail shit fast-import <input
'
...
test_expect_success 'R: only one import-marks feature allowed per stream' '
	...
	>shit2.marks &&
	cat >input <<-EOF &&
	feature import-marks=shit.marks
	feature import-marks=shit2.marks
	EOF

	test_must_fail shit fast-import --allow-unsafe-features <input
'

test_expect_success 'R: export-marks feature forbidden by default' '
	echo "feature export-marks=shit.marks" >input &&
	test_must_fail shit fast-import <input
'

test_expect_success 'R: export-marks feature results in a marks file being created' '
	cat >input <<-EOF &&
	feature export-marks=shit.marks
	blob
	mark :1
	data 3
	hi
	...
	shit fast-import --allow-unsafe-features <input &&
	grep :1 shit.marks
'

test_expect_success 'R: export-marks options can be overridden by commandline options' '
	cat >input <<-\EOF &&
	feature export-marks=feature-sub/shit.marks
	blob
	mark :1
	data 3
	hi
	...
	EOF
	shit fast-import --allow-unsafe-features \
...
test_expect_success 'R: catch typo in marks file name' '
	test_must_fail shit fast-import --import-marks=nonexistent.marks </dev/null &&
	echo "feature import-marks=nonexistent.marks" |
	test_must_fail shit fast-import --allow-unsafe-features
'

test_expect_success 'R: import and output marks can be the same file' '
	rm -f io.marks &&
	blob=$(echo hi | shit hash-object --stdin) &&
	cat >expect <<-EOF &&
	:1 $blob
	:2 $blob
	EOF
	shit fast-import --export-marks=io.marks <<-\EOF &&
	blob
	mark :1
	data 3
	hi
	...
	EOF
	shit fast-import --import-marks=io.marks --export-marks=io.marks <<-\EOF &&
	blob
	mark :2
	data 3
	hi
...
test_expect_success 'R: --import-marks=foo --output-marks=foo to create foo fails' '
	rm -f io.marks &&
	test_must_fail shit fast-import --import-marks=io.marks --export-marks=io.marks <<-\EOF
	blob
	mark :1
	data 3
	hi
...
test_expect_success 'R: --import-marks-if-exists' '
	...
	blob=$(echo hi | shit hash-object --stdin) &&
	echo ":1 $blob" >expect &&
	shit fast-import --import-marks-if-exists=io.marks --export-marks=io.marks <<-\EOF &&
	blob
	mark :1
	data 3
	hi
...
test_expect_success 'R: feature import-marks-if-exists' '
	...
	shit fast-import --export-marks=io.marks \
	...
	shit fast-import --import-marks=io.marks \
	...
	shit fast-import --import-marks-if-exists=not_io.marks \
			--export-marks=io.marks \
			--allow-unsafe-features <<-\EOF &&
	feature import-marks-if-exists=io.marks
	EOF
...
test_expect_success 'R: import marks prefers commandline marks file over the stream' '
	...
	shit fast-import --import-marks=marks.out --allow-unsafe-features <input &&
	test_cmp marks.out marks.new
'
...
test_expect_success 'R: multiple --import-marks= should be honoured' '
	...
	shit fast-import --import-marks=one.marks --import-marks=two.marks \
...
test_expect_success 'R: feature relative-marks should be honoured' '
	...
	mkdir -p .shit/info/fast-import/ &&
	cp marks.new .shit/info/fast-import/relative.in &&
	shit fast-import --allow-unsafe-features <input &&
	test_cmp marks.new .shit/info/fast-import/relative.out
'
...
test_expect_success 'R: cat-blob-fd must be a nonnegative integer' '
	test_must_fail shit fast-import --cat-blob-fd=-1 </dev/null
'

test_expect_success !MINGW 'R: print old blob' '
	blob=$(echo "yes it can" | shit hash-object -w --stdin) &&
	cat >expect <<-EOF &&
	${blob} blob 11
	yes it can
	...
	EOF
	echo "cat-blob $blob" |
	shit fast-import --cat-blob-fd=6 6>actual &&
	test_cmp expect actual
'

test_expect_success !MINGW 'R: in-stream cat-blob-fd not respected' '
	echo hello >greeting &&
	blob=$(shit hash-object -w greeting) &&
	cat >expect <<-EOF &&
	${blob} blob 6
	hello

	EOF
	shit fast-import --cat-blob-fd=3 3>actual.3 >actual.1 <<-EOF &&
	cat-blob $blob
	EOF
	test_cmp expect actual.3 &&
	test_must_be_empty actual.1 &&
	shit fast-import 3>actual.3 >actual.1 <<-EOF &&
...
test_expect_success !MINGW 'R: print mark for new blob' '
	echo "effluentish" | shit hash-object --stdin >expect &&
	shit fast-import --cat-blob-fd=6 6>actual <<-\EOF &&
	blob
	mark :1
	data <<BLOB_END
	effluentish
...
test_expect_success !MINGW 'R: print new blob' '
	blob=$(echo "yep yep yep" | shit hash-object --stdin) &&
	cat >expect <<-EOF &&
	${blob} blob 12
	yep yep yep
...
test_expect_success !MINGW 'R: print new blob by sha1' '
	blob=$(echo "a new blob named by sha1" | shit hash-object --stdin) &&
	cat >expect <<-EOF &&
	${blob} blob 25
...
test_expect_success 'R: print two blobs to stdout' '
	blob1=$(shit hash-object big) &&
	blob1_len=$(wc -c <big) &&
	blob2=$(echo hello | shit hash-object --stdin) &&
	{
	...
	shit fast-import >actual &&
	test_cmp expect actual
'

test_expect_success PIPE 'R: copy using cat-file' '
	expect_id=$(shit hash-object big) &&
	expect_len=$(wc -c <big) &&
	...
	mkfifo blobs &&
	(
		export shit_COMMITTER_NAME shit_COMMITTER_EMAIL shit_COMMITTER_DATE &&
		cat <<-\EOF &&
		feature cat-blob
		blob
		mark :1
	...
	shit fast-import --cat-blob-fd=3 3>blobs &&
	shit show copied:file3 >actual &&
	test_cmp expect.response response &&
	test_cmp big actual
'
...
test_expect_success 'R: quiet option results in no stats being output' '
	...
	shit fast-import 2>output <input &&
	test_must_be_empty output
'

test_expect_success 'R: feature done means terminating "done" is mandatory' '
	echo feature done | test_must_fail shit fast-import &&
	test_must_fail shit fast-import --done </dev/null
'

test_expect_success 'R: terminating "done" with trailing gibberish is ok' '
	shit fast-import <<-\EOF &&
	feature done
	done
	trailing gibberish
	EOF
...
test_expect_success 'R: die on unknown option' '
	cat >input <<-EOF &&
	option shit non-existing-option
	EOF
...
test_expect_success 'R: unknown commandline options are rejected' '\
	test_must_fail shit fast-import --non-existing-option < /dev/null
'

test_expect_success 'R: die on invalid option argument' '
	echo "option shit active-branches=-5" |
	test_must_fail shit fast-import &&
	echo "option shit depth=" |
	test_must_fail shit fast-import &&
	test_must_fail shit fast-import --depth="5 elephants" </dev/null
'

test_expect_success 'R: ignore non-shit options' '
	cat >input <<-EOF &&
...
test_expect_success 'R: blob bigger than threshold' '
	...
	test_create_repo R &&
	shit --shit-dir=R/.shit config fastimport.unpackLimit 0 &&
	shit --shit-dir=R/.shit fast-import --big-file-threshold=1 <input
'
...
test_expect_success 'R: verify written objects' '
	shit --shit-dir=R/.shit cat-file blob big-file:big1 >actual &&
	test_cmp_bin expect actual &&
	a=$(shit --shit-dir=R/.shit rev-parse big-file:big1) &&
	b=$(shit --shit-dir=R/.shit rev-parse big-file:big2) &&
	test $a = $b
'
...
test_expect_success 'S: initialize for S tests' '
	...
	shit fast-import --export-marks=marks <input
'
...
test_expect_success 'S: filemodify with garbage after mark must fail' '
	test_must_fail shit fast-import --import-marks=marks <<-EOF 2>err &&
	commit refs/heads/S
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	data <<COMMIT
	commit N
...
test_expect_success 'S: from with garbage after mark must fail' '
	test_must_fail \
	shit fast-import --import-marks=marks --export-marks=marks <<-EOF 2>err &&
	...
	shit fast-import --import-marks=marks --export-marks=marks <<-EOF &&
	commit refs/heads/S2
	mark :303
	committer $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL> $shit_COMMITTER_DATE
	data <<COMMIT
...
test_path_eol_success () {
	...
	test_expect_success "S: paths at EOL with $test must work" '
		test_when_finished "shit branch -D S-path-eol" &&

		shit fast-import --export-marks=marks.out <<-EOF >out 2>err &&
		blob
		mark :401
		data <<BLOB
		hello world
		...
		shit ls-tree $commit_m | sort >tree_m.out &&
		test_cmp tree_m.exp tree_m.out &&

		printf "100644 blob $blob1\thello.c\n" >tree_d.exp &&
		shit ls-tree $commit_d >tree_d.out &&
		...
		shit ls-tree $commit_c | sort >tree_c.out &&
		test_cmp tree_c.exp tree_c.out &&

		printf "100644 blob $blob1\t$unquoted_path\n" >tree_r.exp &&
		shit ls-tree $commit_r >tree_r.out &&
...
test_path_space_success () {
	...
	test_expect_success "S: paths before space with $test must work" '
		test_when_finished "shit branch -D S-path-space" &&

		shit fast-import --export-marks=marks.out <<-EOF 2>err &&
		blob
		mark :401
		data <<BLOB
		hello world
...
test_path_fail () {
	...
	test_expect_success "S: $change with $what must fail" '
		test_must_fail shit fast-import <<-EOF 2>err &&
		blob
		mark :1
		data <<BLOB
		hello world
...
do
	test_expect_success "T: ls root ($root) tree" '
		...
		040000 tree $(shit rev-parse S^{tree})	Z
		EOF
		sha1=$(shit rev-parse --verify S) &&
		shit fast-import --import-marks=marks <<-EOF >actual &&
		ls $sha1 $root
		EOF
		test_cmp expect actual
	'
...
test_expect_success 'T: delete branch' '
	...
	from $ZERO_OID
	EOF
	test_must_fail shit rev-parse --verify refs/heads/to-delete
'

test_expect_success 'T: empty reset doesnt delete branch' '
	shit branch not-to-delete &&
	shit fast-import <<-EOF &&
	reset refs/heads/not-to-delete
	EOF
	shit show-ref &&
	shit rev-parse --verify refs/heads/not-to-delete
'
...
test_expect_success 'U: initialize for U tests' '
	...
	f7id=$(echo "blob 1" | shit hash-object --stdin) &&
	f8id=$(echo "sleep well" | shit hash-object --stdin) &&
	f9id=$(echo "au revoir" | shit hash-object --stdin) &&
	shit fast-import <input
'
...
test_expect_success 'U: validate file delete result' '
	...
	shit diff-tree -M -r U^1 U >actual &&
...
do
	...
	test_expect_success "U: validate root ($root) delete result" '
		...
		shit diff-tree -M -r U U-delete-root >actual &&
...
test_expect_success PIPE 'V: checkpoint updates refs after reset' '
	...
	background_import_then_checkpoint "" input &&
	test "$(shit rev-parse --verify V)" = "$(shit rev-parse --verify U)" &&
	background_import_still_running
'
...
test_expect_success PIPE 'V: checkpoint updates refs and marks after commit' '
	...
	echo ":1 $(shit rev-parse --verify V)" >marks.expected &&
...
test_expect_success PIPE 'V: checkpoint updates refs and marks after commit (no new objects)' '
	...
	echo ":2 $(shit rev-parse --verify V2)" >marks.expected &&

	test "$(shit rev-parse --verify V2)" = "$(shit rev-parse --verify V)" &&
	test_cmp marks.expected marks.actual &&
	background_import_still_running
'
...
test_expect_success !MINGW 'W: get-mark & empty orphan commit with ugly second newline' '
	...
	# does not work with older (pre-2.22) versions of shit, allow apps
...
test_expect_success 'X: handling encoding' '
	...
	shit fast-import <input &&
	shit cat-file -p encoding | grep $(printf "\360") &&
	shit log -1 --format=%B encoding | grep $(printf "\317\200")
'
...
[submodule "sub1"]
	path = sub1
	url = https://void.example.com/main.shit
...
test_expect_success 'Y: rewrite submodules' '
	...
	(
		cd main1 &&
		shit init sub2 &&
		shit -C sub2 fast-import --export-marks=../sub2-marks <../Y-sub-input &&
		shit fast-import --rewrite-submodules-from=sub:../Y-marks \
			--rewrite-submodules-to=sub:sub2-marks <../Y-main-input &&
		test "$(shit rev-parse main)" = "$(test_oid Ymain)"
	)
'
