...
test_expect_success 'filters fallback to non-bitmap traversal' '
	...
	filter=$(echo "!one" | shit hash-object -w --stdin) &&
	shit rev-list --objects --filter=sparse:oid=$filter HEAD >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter=sparse:oid=$filter HEAD >actual &&
	test_cmp expect actual
'

test_expect_success 'blob:none filter' '
	shit rev-list --objects --filter=blob:none HEAD >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'blob:none filter with specified blob' '
	shit rev-list --objects --filter=blob:none HEAD HEAD:two.t >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'blob:limit filter' '
	shit rev-list --objects --filter=blob:limit=5 HEAD >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'blob:limit filter with specified blob' '
	shit rev-list --objects --filter=blob:limit=5 \
...
test_expect_success 'tree:0 filter' '
	shit rev-list --objects --filter=tree:0 HEAD >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'tree:0 filter with specified blob, tree' '
	shit rev-list --objects --filter=tree:0 HEAD HEAD:two.t >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'tree:1 filter' '
	shit rev-list --objects --filter=tree:1 HEAD >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'object:type filter' '
	shit rev-list --objects --filter=object:type=tag tag >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter=object:type=tag tag >actual &&
	test_cmp expect actual &&

	shit rev-list --objects --filter=object:type=commit tag >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter=object:type=commit tag >actual &&
	test_bitmap_traversal expect actual &&

	shit rev-list --objects --filter=object:type=tree tag >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter=object:type=tree tag >actual &&
	test_bitmap_traversal expect actual &&

	shit rev-list --objects --filter=object:type=blob tag >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'object:type filter with --filter-provided-objects' '
	shit rev-list --objects --filter-provided-objects --filter=object:type=tag tag >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter-provided-objects --filter=object:type=tag tag >actual &&
	test_cmp expect actual &&

	shit rev-list --objects --filter-provided-objects --filter=object:type=commit tag >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter-provided-objects --filter=object:type=commit tag >actual &&
	test_bitmap_traversal expect actual &&

	shit rev-list --objects --filter-provided-objects --filter=object:type=tree tag >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter-provided-objects --filter=object:type=tree tag >actual &&
	test_bitmap_traversal expect actual &&

	shit rev-list --objects --filter-provided-objects --filter=object:type=blob tag >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'combine filter' '
	shit rev-list --objects --filter=blob:limit=1000 --filter=object:type=blob tag >expect &&
	shit rev-list --use-bitmap-index \
...
test_expect_success 'combine filter with --filter-provided-objects' '
	shit rev-list --objects --filter-provided-objects --filter=blob:limit=1000 --filter=object:type=blob tag >expect &&
	shit rev-list --use-bitmap-index \
		     --objects --filter-provided-objects --filter=blob:limit=1000 --filter=object:type=blob tag >actual &&
	test_bitmap_traversal expect actual &&

	shit cat-file --batch-check="%(objecttype) %(objectsize)" <actual >objects &&
...
test_expect_success 'bitmap traversal with --unpacked' '
	...
	shit rev-list --objects --no-object-names unpacked^.. >expect.raw &&
	sort expect.raw >expect &&

	shit rev-list --use-bitmap-index --objects --all --unpacked >actual.raw &&
