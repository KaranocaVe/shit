...
test_expect_success 'apply same new filename after rename' '
	shit reset --hard &&
	shit mv same_fn new_fn &&
	modify "s/^d/z/" new_fn &&
	shit add new_fn &&
	shit diff -M --cached > patch1 &&
	modify "s/^e/y/" new_fn &&
	shit diff >> patch1 &&
	cp new_fn new_fn2 &&
	shit reset --hard &&
...
test_expect_success 'apply same old filename after rename -- should fail.' '
	...
	shit add new_fn &&
	shit diff -M --cached > patch1 &&
	shit mv new_fn same_fn &&
	modify "s/^e/y/" same_fn &&
	shit diff >> patch1 &&
	shit reset --hard &&
	test_must_fail shit apply patch1
'
...
test_expect_success 'apply A->B (rename), C->A (rename), A->A -- should pass.' '
	...
	shit add new_fn &&
	shit diff -M --cached > patch1 &&
	shit commit -m "a rename" &&
	shit mv other_fn same_fn &&
	modify "s/^e/y/" same_fn &&
	shit add same_fn &&
	shit diff -M --cached >> patch1 &&
