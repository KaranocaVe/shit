...
test_description='shit status with file system watcher'

. ./test-lib.sh

# Note, after "shit reset --hard HEAD" no extensions exist other than 'TREE'
# "shit update-index --fsmonitor" can be used to get the extension written
...
test_expect_success 'incompatible bare repo' '
	...
	shit init --bare bare-clone &&

	test_must_fail \
		shit -C ./bare-clone -c core.fsmonitor=foo \
			update-index --fsmonitor 2>actual &&
	grep "bare repository .* is incompatible with fsmonitor" actual &&

	test_must_fail \
		shit -C ./bare-clone -c core.fsmonitor=true \
			update-index --fsmonitor 2>actual &&
	grep "bare repository .* is incompatible with fsmonitor" actual
'
...
test_expect_success FSMONITOR_DAEMON 'run fsmonitor-daemon in bare repo' '
	...
	shit init --bare bare-clone &&
	test_must_fail shit -C ./bare-clone fsmonitor--daemon run 2>actual &&
	grep "bare repository .* is incompatible with fsmonitor" actual
'
...
test_expect_success MINGW,FSMONITOR_DAEMON 'run fsmonitor-daemon in virtual repo' '
	...
	shit init fake-virtual-clone &&
	test_must_fail shit -C ./fake-virtual-clone \
...
test_expect_success 'setup' '
	...
	: >dir2/tracked &&
	: >dir2/modified &&
	shit -c core.fsmonitor= add . &&
	shit -c core.fsmonitor= commit -m initial &&
	shit config core.fsmonitor .shit/hooks/fsmonitor-test &&
	cat >.shitignore <<-\EOF
	.shitignore
	expect*
	actual*
...
test_expect_success 'update-index --fsmonitor-valid" sets the fsmonitor valid bit' '
	...
	EOF
	shit update-index --fsmonitor &&
	shit update-index --fsmonitor-valid dir1/modified &&
	shit update-index --fsmonitor-valid dir2/modified &&
	shit update-index --fsmonitor-valid modified &&
	shit ls-files -f >actual &&
...
test_expect_success 'update-index --no-fsmonitor-valid" clears the fsmonitor valid bit' '
	shit update-index --no-fsmonitor-valid dir1/modified &&
	shit update-index --no-fsmonitor-valid dir2/modified &&
	shit update-index --no-fsmonitor-valid modified &&
	shit ls-files -f >actual &&
	test_cmp expect actual
'
...
test_expect_success 'all unmodified files get marked valid' '
	...
	test_must_fail shit update-index --refresh --force-write-index &&
	shit ls-files -f >actual &&
	test_cmp expect actual
'
...
test_expect_success '*only* files returned by the integration script get flagged as invalid' '
	...
	EOF
	clean_repo &&
	shit update-index --refresh --force-write-index &&
	echo 1 >modified &&
	echo 2 >dir1/modified &&
	echo 3 >dir2/modified &&
	test_must_fail shit update-index --refresh --force-write-index &&
...
test_expect_success 'refresh_index() invalidates fsmonitor cache' '
	...
	test_hook --clobber fsmonitor-test<<-\EOF &&
	EOF
	shit commit -m "to reset" &&
	shit reset HEAD~1 &&
	shit status >actual &&
	shit -c core.fsmonitor= status >expect &&
	test_cmp expect actual
'
...
do
	test_expect_success "setup preloadIndex to $preload_val" '
		shit config core.preloadIndex $preload_val &&
		if test $preload_val = true
		then
			shit_TEST_PRELOAD_INDEX=$preload_val && export shit_TEST_PRELOAD_INDEX
		else
			sane_unset shit_TEST_PRELOAD_INDEX
		fi
	'
	...
	do
		test_expect_success "setup untracked cache to $uc_val" '
			shit config core.untrackedcache $uc_val
		'
...
test_expect_success 'splitting the index results in the same state' '
	...
	dirty_repo &&
	shit update-index --fsmonitor  &&
	shit ls-files -f >expect &&
	test-tool dump-fsmonitor >&2 && echo &&
	shit update-index --fsmonitor --split-index &&
	test-tool dump-fsmonitor >&2 && echo &&
	shit ls-files -f >actual &&
	test_cmp expect actual
'

test_expect_success UNTRACKED_CACHE 'ignore .shit changes when invalidating UNTR' '
	test_create_repo dot-shit &&
	(
		cd dot-shit &&
		: >tracked &&
		...
		shit update-index --fsmonitor &&
		shit status &&
		shit_TRACE2_PERF="$TRASH_DIRECTORY/trace-before" \
		shit status &&
		test-tool dump-untracked-cache >../before
	) &&
	cat >>dot-shit/.shit/hooks/fsmonitor-test <<-\EOF &&
	printf ".shit\0"
	printf ".shit/index\0"
	printf "dir1/.shit\0"
	printf "dir1/.shit/index\0"
	EOF
	(
		cd dot-shit &&
		shit_TRACE2_PERF="$TRASH_DIRECTORY/trace-after" \
		shit status &&
...
test_expect_success 'discard_index() also discards fsmonitor info' '
	...
	test_might_fail shit update-index --refresh &&
...
test_expect_success 'status succeeds after staging/unstaging' '
	...
	(
		...
		touch $removed &&
		shit add $removed &&
		shit config core.fsmonitor "$TEST_DIRECTORY/t7519/fsmonitor-env" &&
		FSMONITOR_LIST="$removed" shit restore -S $removed &&
		FSMONITOR_LIST="$removed" shit status
	)
'
...
# during a call to 'shit status'. Otherwise, we verify that we _do_ call it.
check_sparse_index_behavior () {
	shit -C full status --porcelain=v2 >expect &&
	shit_TRACE2_EVENT="$(pwd)/trace2.txt" \
		shit -C sparse status --porcelain=v2 >actual &&
...
test_expect_success 'status succeeds with sparse index' '
	(
		...
		shit clone . full &&
		shit clone --sparse . sparse &&
		shit -C sparse sparse-checkout init --cone --sparse-index &&
		shit -C sparse sparse-checkout set dir1 dir2 &&

		test_hook --clobber fsmonitor-test <<-\EOF &&
			printf "last_update_token\0"
		EOF
		shit -C full config core.fsmonitor ../.shit/hooks/fsmonitor-test &&
		shit -C sparse config core.fsmonitor ../.shit/hooks/fsmonitor-test &&
		check_sparse_index_behavior ! &&
		...
		shit -C sparse sparse-checkout add dir1a &&

		for repo in full sparse
		do
			cp -r $repo/dir1 $repo/dir1a &&
			shit -C $repo add dir1a &&
			shit -C $repo commit -m "add dir1a" || return 1
		done &&
		shit -C sparse sparse-checkout set dir1 dir2 &&
