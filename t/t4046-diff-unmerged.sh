...
test_expect_success setup '
	for i in 0 1 2 3
	do
		blob=$(echo $i | shit hash-object --stdin) &&
		eval "blob$i=$blob" &&
	...
	shit update-index --index-info <ls-files-s.expect &&
	shit ls-files -s >ls-files-s.actual &&
...
test_expect_success 'diff-files -0' '
	...
	shit diff-files -0 >diff-files-0.actual &&
...
test_expect_success 'diff-files -1' '
	...
	shit diff-files -1 >diff-files-1.actual &&
...
test_expect_success 'diff-files -2' '
	...
	shit diff-files -2 >diff-files-2.actual &&
	test_cmp diff-files-2.expect diff-files-2.actual &&
	shit diff-files >diff-files-default-2.actual &&
...
test_expect_success 'diff-files -3' '
	...
	shit diff-files -3 >diff-files-3.actual &&
...
test_expect_success 'diff --stat' '
	...
	shit diff --cached --stat >diff-stat.actual &&
	test_cmp diff-stat.expect diff-stat.actual
'

test_expect_success 'diff --quiet' '
	test_expect_code 1 shit diff --cached --quiet
'

test_expect_success 'diff --quiet --ignore-all-space' '
	test_expect_code 1 shit diff --cached --quiet --ignore-all-space
'
