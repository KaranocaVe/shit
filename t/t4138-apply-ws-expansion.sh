...
test_description='shit apply test patches with whitespace expansion.'
...
test_expect_success setup '
	...
	printf "%64s\n" a b c >>after &&
	printf "\t%s\n" 4 5 6 >>after &&
	test_expect_code 1 shit diff --no-index before after >patch1.patch.raw &&
	...
	printf "\t%s\n" d e f >>after &&
	test_expect_code 1 shit diff --no-index before after >patch2.patch.raw &&
	...
	printf "\t%s\n" d e f >>after &&
	test_expect_code 1 shit diff --no-index before after >patch3.patch.raw &&
	...
	test_expect_code 1 shit diff --no-index before after >patch4.patch.raw &&
	...
	shit config core.whitespace tab-in-indent,tabwidth=63 &&
	shit config apply.whitespace fix
