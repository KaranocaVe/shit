...
test_description='shit log with invalid commit headers'
...
test_expect_success 'setup' '
	...
	shit cat-file commit HEAD >ok.commit &&
	sed "s/>/>-<>/" <ok.commit >broken_email.commit &&

	shit hash-object --literally -w -t commit broken_email.commit >broken_email.hash &&
	shit update-ref refs/heads/broken_email $(cat broken_email.hash)
'
...
test_expect_success 'shit log with broken author email' '
	{
	...
	shit log broken_email >actual.out 2>actual.err &&

	test_cmp expect.out actual.out &&
	test_must_be_empty actual.err
'

test_expect_success 'shit log --format with broken author email' '
	echo "A U Thor+author@example.com+Thu Apr 7 15:13:13 2005 -0700" >expect.out &&

	shit log --format="%an+%ae+%ad" broken_email >actual.out 2>actual.err &&
...
test_expect_success '--until handles broken email' '
	shit rev-list --until=1980-01-01 broken_email >actual &&
	test_must_be_empty actual
'

munge_author_date () {
	shit cat-file commit "$1" >commit.orig &&
	sed "s/^\(author .*>\) [0-9]*/\1 $2/" <commit.orig >commit.munge &&
	shit hash-object --literally -w -t commit commit.munge
}

test_expect_success 'unparsable dates produce sentinel value' '
	commit=$(munge_author_date HEAD totally_bogus) &&
	echo "Date:   Thu Jan 1 00:00:00 1970 +0000" >expect &&
	shit log -1 $commit >actual.full &&
...
test_expect_success 'unparsable dates produce sentinel value (%ad)' '
	commit=$(munge_author_date HEAD totally_bogus) &&
	echo >expect &&
	shit log -1 --format=%ad $commit >actual &&
	test_cmp expect actual
'
...
test_expect_success 'create commits with whitespace committer dates' '
	...
	shit cat-file commit HEAD >commit.orig &&
	sed "s/>.*/>    /" <commit.orig >commit.munge &&
	ws_commit=$(shit hash-object --literally -w -t commit commit.munge) &&
	sed "s/>.*/>   $(printf "\013")/" <commit.orig >commit.munge &&
	vt_commit=$(shit hash-object --literally -w -t commit commit.munge)
'

test_expect_success '--until treats whitespace date as sentinel' '
	echo $ws_commit >expect &&
	shit rev-list --until=1980-01-01 $ws_commit >actual &&
	test_cmp expect actual &&

	echo $vt_commit >expect &&
	shit rev-list --until=1980-01-01 $vt_commit >actual &&
	test_cmp expect actual
'
...
test_expect_success 'pretty-printer handles whitespace date' '
	...
	# test bogus timestamps with shit-log, 2014-02-24) for more discussion.
	echo : >expect &&
	shit log -1 --format="%at:%ct" $ws_commit >actual &&
	test_cmp expect actual &&
	shit log -1 --format="%at:%ct" $vt_commit >actual &&
	test_cmp expect actual
'
