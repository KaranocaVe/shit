...
shit_TEST_MAINT_SCHEDULER="crontab:test-tool crontab cron.txt,launchctl:true,schtasks:true"
...
test_expect_success 'scalar repo search respects shit_CEILING_DIRECTORIES' '
	test_when_finished rm -rf test &&

	shit init test/src &&
	mkdir -p test/src/deep &&
	shit_CEILING_DIRECTORIES="$(pwd)/test/src" &&
...
test_expect_success 'scalar enlistments need a worktree' '
	...
	shit init --bare bare/src &&
	! scalar register bare/src 2>err &&
	grep "Scalar enlistments require a worktree" err &&

	shit init test/src &&
	! scalar register test/src/.shit 2>err &&
	grep "Scalar enlistments require a worktree" err
'

test_expect_success FSMONITOR_DAEMON 'scalar register starts fsmon daemon' '
	shit init test/src &&
	test_must_fail shit -C test/src fsmonitor--daemon status &&
	scalar register test/src &&
	shit -C test/src fsmonitor--daemon status &&
	test_cmp_config -C test/src true core.fsmonitor
'

test_expect_success 'scalar register warns when background maintenance fails' '
	shit init register-repo &&
	shit_TEST_MAINT_SCHEDULER="crontab:false,launchctl:false,schtasks:false" \
		scalar register register-repo 2>err &&
	grep "could not turn on maintenance" err
'

test_expect_success 'scalar unregister' '
	shit init vanish/src &&
	scalar register vanish/src &&
	shit config --get --global --fixed-value \
		maintenance.repo "$(pwd)/vanish/src" &&
	scalar list >scalar.repos &&
	grep -F "$(pwd)/vanish/src" scalar.repos &&
	rm -rf vanish/src/.shit &&
	scalar unregister vanish &&
	test_must_fail shit config --get --global --fixed-value \
		maintenance.repo "$(pwd)/vanish/src" &&
	scalar list >scalar.repos &&
	! grep -F "$(pwd)/vanish/src" scalar.repos &&
...
test_expect_success 'scalar clone' '
	second=$(shit rev-parse --verify second:second.t) &&
	scalar clone "file://$(pwd)" cloned --single-branch &&
	(
		cd cloned/src &&

		shit config --get --global --fixed-value maintenance.repo \
			"$(pwd)" &&

		shit for-each-ref --format="%(refname)" refs/remotes/origin/ >actual &&
		...
		test_must_fail shit rev-list --missing=print "$second^{blob}" -- &&

		shit rev-list $second &&
		shit cat-file blob $second >actual &&
		echo "second" >expect &&
		test_cmp expect actual
	)
'
...
test_expect_success 'scalar reconfigure' '
	shit init one/src &&
	scalar register one &&
	shit -C one/src config core.preloadIndex false &&
	scalar reconfigure one &&
	test true = "$(shit -C one/src config core.preloadIndex)" &&
	shit -C one/src config core.preloadIndex false &&
...
test_expect_success 'scalar reconfigure --all with includeIf.onbranch' '
	...
	do
		...
		shit -C $num/src config includeif."onbranch:foo".path something &&
		shit -C $num/src config core.preloadIndex false || return 1
	done &&
	...
	for num in $repos
	do
		test true = "$(shit -C $num/src config core.preloadIndex)" || return 1
	done
'
 ...
 test_expect_success 'scalar reconfigure --all with detached HEADs' '
	...
	do
		...
		shit init $num/src &&
		scalar register $num/src &&
		shit -C $num/src config core.preloadIndex false &&
		test_commit -C $num/src initial &&
		shit -C $num/src switch --detach HEAD || return 1
	done &&
...
test_expect_success 'scalar supports -c/-C' '
	...
	test -z "$(shit -C sub config --local status.aheadBehind)" &&
	test true = "$(shit -C sub config core.preloadIndex)"
'
...
test_expect_success UNZIP 'scalar diagnose' '
	scalar clone "file://$(pwd)" cloned --single-branch &&
	shit repack &&
	echo "$(pwd)/.shit/objects/" >>cloned/src/.shit/objects/info/alternates &&
