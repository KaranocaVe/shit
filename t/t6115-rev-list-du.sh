...
disk_usage_slow () {
	shit rev-list --no-object-names "$@" |
	shit cat-file --batch-check="%(objectsize:disk)" |
...
check_du () {
	...
	test_expect_success "rev-list --disk-usage without bitmaps ($args)" "
		shit rev-list --disk-usage $args >actual &&
		test_cmp expect actual
	"

	test_expect_success "rev-list --disk-usage with bitmaps ($args)" "
		shit rev-list --disk-usage --use-bitmap-index $args >actual &&
		test_cmp expect actual
	"
}
...
# output from shit cat-file.
test_expect_success 'rev-list --disk-usage=human' '
	shit rev-list --objects HEAD --disk-usage=human >actual &&
...
test_expect_success 'rev-list --disk-usage=human with bitmaps' '
	shit rev-list --objects HEAD --use-bitmap-index --disk-usage=human >actual &&
	disk_usage_slow --objects HEAD >actual_size &&
	grep "$(cat actual_size) bytes" actual
'

test_expect_success 'rev-list use --disk-usage unproperly' '
	test_must_fail shit rev-list --objects HEAD --disk-usage=typo 2>err &&
	cat >expect <<-\EOF &&
