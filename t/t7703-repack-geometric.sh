...
test_description='shit repack --geometric works correctly'
...
test_expect_success '--geometric with no packs' '
	...
	(
		...
		shit repack --write-midx --geometric 2 >out &&
...
test_expect_success '--geometric with small- and large-pack rollup' '
	...
	(
		...
		# Two packs in total; the largest pack from before running "shit
...
test_expect_success '--geometric ignores kept packs' '
	...
	(
		...
		KEPT=$(shit pack-objects --revs $objdir/pack/pack <<-EOF
		refs/tags/kept
		EOF
		) &&
		PACK=$(shit pack-objects --revs $objdir/pack/pack <<-EOF
		refs/tags/pack
		^refs/tags/kept
		EOF
		) &&
		...
		# be repacked, too.
		shit repack --geometric 2 -d --pack-kept-objects &&
...
test_expect_success '--geometric with pack.packSizeLimit' '
	...
	(
		...
		shit rev-parse HEAD:foo HEAD:bar >p1.objects &&
		shit rev-parse HEAD HEAD^{tree} >p2.objects &&

		# These two packs each contain two objects, so the following
		# `--geometric` repack will try to combine them.
		p1="$(shit pack-objects $packdir/pack <p1.objects)" &&
		p2="$(shit pack-objects $packdir/pack <p2.objects)" &&
		...
		shit repack --geometric 2 -d --max-pack-size=1048576 &&
...
test_expect_success '--geometric --write-midx with packfiles in main and alternate ODB' '
	...
	# repository. We then create a commit in it so that shit-repack(1) has
	# something to repack.
	# of the shared object database.
	shit clone --shared shared member &&
	test_commit -C member unique-object &&
	shit -C member repack --geometric=2 --write-midx 2>err &&
	test_must_be_empty err &&

	# We should see that a new packfile was generated.
	find shared/.shit/objects/pack -type f -name "*.pack" >packs &&
	...
	test_path_is_file member/.shit/objects/pack/multi-pack-index &&
	test-tool read-midx member/.shit/objects >packs.midx &&
	grep "^pack-.*\.idx$" packs.midx | sort >actual &&
	basename member/.shit/objects/pack/pack-*.idx >expect &&
	test_cmp expect actual
'
...
test_expect_success '--geometric with same pack in main and alternate ODB' '
	...
	# same packfile.
	cp -r shared member &&
	test-tool path-utils real_path shared/.shit/objects >member/.shit/objects/info/alternates &&
	find shared/.shit/objects -type f >expected-files &&
	...
	# in shit-pack-objects(1).
	shit -C member repack --geometric 2 2>err &&
	test_must_be_empty err &&
	# Nothing should have changed.
	find shared/.shit/objects -type f >actual-files &&
...
test_expect_success '--geometric -l with non-intact geometric sequence across ODBs' '
	...
	packed_objects shared/.shit/objects/pack/pack-*.idx >shared-objects &&
	packed_objects member/.shit/objects/pack/pack-*.idx >member-objects &&
	...
	test-tool chmtime --verbose =0 member/.shit/objects/pack/* >expected-member-packs &&
	shit -C member repack --geometric=2 -l -d &&
	test-tool chmtime --verbose member/.shit/objects/pack/* >actual-member-packs &&
	...
	test-tool chmtime --verbose =0 shared/.shit/objects/pack/* >expected-shared-packs &&
	shit -C member repack --geometric=2 -d &&
	test-tool chmtime --verbose shared/.shit/objects/pack/* >actual-shared-packs &&
	...
	ls member/.shit/objects/pack/pack-*.idx >actual &&
	test_line_count = 1 actual &&
	packed_objects member/.shit/objects/pack/pack-*.idx >actual-objects &&
...
test_expect_success '--geometric -l disables writing bitmaps with non-local packfiles' '
	...
	# will be disabled.
	shit -C member repack -l --geometric=2 --write-midx --write-bitmap-index 2>err &&
	cat >expect <<-EOF &&
	warning: disabling bitmap writing, as some objects are not being packed
	EOF
	test_cmp expect err &&
	test_path_is_missing member/.shit/objects/pack/multi-pack-index-*.bitmap &&

	# On the other hand, when we repack without `-l`, we should see that
	# the bitmap gets created.
	shit -C member repack --geometric=2 --write-midx --write-bitmap-index 2>err &&
	test_must_be_empty err &&
	test_path_is_file member/.shit/objects/pack/multi-pack-index-*.bitmap
'
