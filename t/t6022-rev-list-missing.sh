...
test_expect_success 'create repository and alternate directory' '
	...
	test_commit 2 &&
	test_commit 3 &&
	shit tag -m "tag message" annot_tag HEAD~1 &&
	shit tag regul_tag HEAD~1 &&
	shit branch a_branch HEAD~1
'
...
shit_COMMIT_GRAPH_PARANOIA=true
export shit_COMMIT_GRAPH_PARANOIA

for obj in "HEAD~1" "HEAD~1^{tree}" "HEAD:1.t"
do
	test_expect_success "rev-list --missing=error fails with missing object $obj" '
		oid="$(shit rev-parse $obj)" &&
		path=".shit/objects/$(test_oid_to_path $oid)" &&

		mv "$path" "$path.hidden" &&
		test_when_finished "mv $path.hidden $path" &&

		test_must_fail shit rev-list --missing=error --objects \
...
do
	...
	do
		test_expect_success "rev-list --missing=$action with missing $obj" '
			...
			then
				echo $(shit rev-parse HEAD:1.t) >>expect.raw &&
				echo $(shit rev-parse HEAD:2.t) >>expect.raw
			fi &&

			mv "$path" "$path.hidden" &&
			test_when_finished "mv $path.hidden $path" &&

			shit rev-list --missing=$action --objects --no-object-names \
				HEAD >actual.raw &&
	...
	do
		...
		do
			test_expect_success "--missing=$action with tip '$missing_tip' missing and tip '$existing_tip'" '
				...
				missing_oid="$(shit rev-parse $missing_tip)" &&

				if test "$missing_tip" = "annot_tag"
				then
					oid="$(shit rev-parse $missing_tip^{commit})" &&
