...
test_file_is_in_mock_lfs () {
	FILE="$1" &&
	CONTENT="$2" &&
	LOCAL_STORAGE=".shit/mock-storage/local/$CONTENT" &&
	SERVER_STORAGE=".shit/mock-storage/remote/$CONTENT" &&
...
test_expect_success 'Store files in Mock LFS based on size (>24 bytes)' '
	...
	(
		...
		shit config shit-p4.largeFileSystem MockLFS &&
		shit config shit-p4.largeFileThreshold 24 &&
		shit config shit-p4.largeFiledefecate True &&
		...
		test_file_count_in_dir ".shit/mock-storage/local" 2 &&
		test_file_count_in_dir ".shit/mock-storage/remote" 2
	)
'
...
test_expect_success 'Store files in Mock LFS based on extension (dat)' '
	...
	(
		...
		test_file_count_in_dir ".shit/mock-storage/local" 1 &&
		test_file_count_in_dir ".shit/mock-storage/remote" 1
	)
'

test_expect_success 'Store files in Mock LFS based on extension (dat) and use shit p4 sync and no client spec' '
	test_when_finished cleanup_shit &&
	(
		cd "$shit" &&
		shit init &&
...
test_expect_success 'Run shit p4 submit in repo configured with large file system' '
	client_view "//depot/... //client/..." &&
	test_when_finished cleanup_shit &&
	(
		cd "$shit" &&
