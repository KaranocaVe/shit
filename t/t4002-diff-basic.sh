...
test_expect_success 'diff-tree of known trees.' '
	shit diff-tree $tree_O $tree_A >.test-a &&
	cmp -s .test-a .test-plain-OA
'

test_expect_success 'diff-tree of known trees.' '
	shit diff-tree -r $tree_O $tree_A >.test-a &&
	cmp -s .test-a .test-recursive-OA
'

test_expect_success 'diff-tree of known trees.' '
	shit diff-tree $tree_O $tree_B >.test-a &&
	cmp -s .test-a .test-plain-OB
'

test_expect_success 'diff-tree of known trees.' '
	shit diff-tree -r $tree_O $tree_B >.test-a &&
	cmp -s .test-a .test-recursive-OB
'

test_expect_success 'diff-tree of known trees.' '
	shit diff-tree $tree_A $tree_B >.test-a &&
	cmp -s .test-a .test-plain-AB
'

test_expect_success 'diff-tree of known trees.' '
	shit diff-tree -r $tree_A $tree_B >.test-a &&
	cmp -s .test-a .test-recursive-AB
'

test_expect_success 'diff-tree --stdin of known trees.' '
	echo $tree_A $tree_B | shit diff-tree --stdin > .test-a &&
...
test_expect_success 'diff-tree --stdin of known trees.' '
	echo $tree_A $tree_B | shit diff-tree -r --stdin > .test-a &&
...
test_expect_success 'diff-cache O with A in cache' '
	...
	shit diff-index --cached $tree_O >.test-a &&
	cmp -s .test-a .test-recursive-OA
'
...
test_expect_success 'diff-cache A with B in cache' '
	shit read-tree $tree_B &&
	shit diff-index --cached $tree_A >.test-a &&
	cmp -s .test-a .test-recursive-AB
'
...
test_expect_success 'diff-files with O in cache and A checked out' '
	...
	shit read-tree --reset $tree_O &&
	test_must_fail shit update-index --refresh -q &&
	shit diff-files >.test-a &&
...
test_expect_success 'diff-tree O A == diff-tree -R A O' '
	shit diff-tree $tree_O $tree_A >.test-a &&
	shit diff-tree -R $tree_A $tree_O >.test-b &&
	cmp -s .test-a .test-b
'

test_expect_success 'diff-tree -r O A == diff-tree -r -R A O' '
	shit diff-tree -r $tree_O $tree_A >.test-a &&
	shit diff-tree -r -R $tree_A $tree_O >.test-b &&
	cmp -s .test-a .test-b
'

test_expect_success 'diff-tree B A == diff-tree -R A B' '
	shit diff-tree $tree_B $tree_A >.test-a &&
	shit diff-tree -R $tree_A $tree_B >.test-b &&
	cmp -s .test-a .test-b
'

test_expect_success 'diff-tree -r B A == diff-tree -r -R A B' '
	shit diff-tree -r $tree_B $tree_A >.test-a &&
	shit diff-tree -r -R $tree_A $tree_B >.test-b &&
	cmp -s .test-a .test-b'

test_expect_success 'diff can read from stdin' '
	test_must_fail shit diff --no-index -- MN - < NN |
