...
test_expect_success 'setup' '
	...
	blob_file1=$(shit hash-object file1) &&
	blob_file2=$(shit hash-object subdir/file2) &&
	shit add . &&
	shit commit -m one
'
...
check_diff () {
	...
	expect=$1
	shift
	short_blob=$(shit rev-parse --short $blob_file2)
	cat >expected <<-EOF
	diff --shit a/$expect b/$expect
...
check_numstat () {
	...
	test_expect_success "--numstat $*" "
		...
		shit -C '$dir' diff --numstat $* HEAD^ >actual &&
		test_cmp expected actual
	"
}
...
check_stat () {
	...
	test_expect_success "--stat $*" "
		shit -C '$dir' diff --stat $* HEAD^ >actual &&
		test_cmp expected actual
	"
}
...
check_raw () {
	...
	test_expect_success "--raw $*" "
		shit -C '$dir' diff --no-abbrev --raw $* HEAD^ >actual &&
		test_cmp expected actual
	"
}
...
check_diff_no_relative_option () {
	...
	shift
	test_expect_success "config diff.relative $relative_opt -p $*" "
		short_blob_file1=\$(shit rev-parse --short $blob_file1) &&
		short_blob_file2=\$(shit rev-parse --short $blob_file2) &&
		cat >expected <<-EOF &&
...
test_expect_success 'external diff with --relative' '
	...
	+changed
	EOF
	shit_EXTERNAL_DIFF=./mydiff shit diff --relative=subdir >actual &&
...
test_expect_success 'diff --relative without change in subdir' '
	shit switch br2 &&
	test_when_finished "shit merge --abort" &&
	test_must_fail shit merge one &&
	shit -C subdir diff --relative >out &&
	test_must_be_empty out &&
	shit -C subdir diff --relative --name-only >out &&
	test_must_be_empty out
'
...
test_expect_success 'diff --relative --name-only with change in subdir' '
	shit switch br3 &&
	test_when_finished "shit merge --abort" &&
	test_must_fail shit merge sub1 &&
...
test_expect_failure 'diff --relative with change in subdir' '
	shit switch br3 &&
	br1_blob=$(shit rev-parse --short --verify br1:subdir/file0) &&
	br3_blob=$(shit rev-parse --short --verify br3:subdir/file0) &&
	test_when_finished "shit merge --abort" &&
	test_must_fail shit merge br1 &&
	cat >expected <<-EOF &&
	diff --cc file0
