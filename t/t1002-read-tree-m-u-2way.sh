...
test_expect_success '1, 2, 3 - no carry forward' '
	rm -f .shit/index nitfol bozbar rezrov frotz &&
...
test_expect_success '4 - carry forward local addition.' '
	...
	test_might_fail shit diff -U0 --no-index M.out 4.out >4diff.out &&
...
test_expect_success '5 - carry forward local addition.' '
	...
	test_might_fail shit diff -U0 --no-index M.out 5.out >5diff.out &&
...
test_expect_success '14 - unchanged in two heads.' '
	...
	test_must_fail shit diff -U0 --no-index M.out 14.out >14diff.out &&
...
test_expect_success '15 - unchanged in two heads.' '
	...
	test_must_fail shit diff -U0 --no-index M.out 15.out >15diff.out &&
