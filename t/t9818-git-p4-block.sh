...
test_description='shit p4 fetching changes in multiple blocks'
...
test_expect_success 'Clone the repo' '
	shit p4 clone --dest="$shit" --changes-block-size=7 --verbose //depot/included@all
'
...
test_expect_success 'file.txt is correct' '
	echo 55 >expected &&
	test_cmp expected "$shit/file.txt"
'

test_expect_success 'Correct number of commits' '
	(cd "$shit" && shit log --oneline) >log &&
	wc -l log &&
	test_line_count = 43 log
'

test_expect_success 'Previous version of file.txt is correct' '
	(cd "$shit" && shit checkout HEAD^^) &&
	echo 53 >expected &&
	test_cmp expected "$shit/file.txt"
'

# Test shit-p4 sync, with some files outside the client specification.
...
test_expect_success 'Syncing files' '
	(
		cd "$shit" &&
		shit p4 sync --changes-block-size=7 &&
...
#    shit p4 clone //depot/pathA //depot/pathB
#
...
test_expect_success 'Clone repo with multiple depot paths' '
	...
	(
		cd "$shit" &&
		shit p4 clone --changes-block-size=4 //depot/pathA@all //depot/pathB@all \
			--destination=dest &&
		ls -1 dest >log &&
...
test_expect_success 'Clone repo with self-sizing block size' '
	test_when_finished cleanup_shit &&
	shit p4 clone --changes-block-size=1000000 //depot@all --destination="$shit" &&
	shit -C "$shit" log --oneline >log &&
