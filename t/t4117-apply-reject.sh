...
test_description='shit apply with rejects
...
test_expect_success setup '
	...
	mv file1 file2 &&
	shit update-index --add --remove file1 file2 &&
	shit diff -M HEAD >patch.2 &&
...
test_expect_success 'apply --reject is incompatible with --3way' '
	...
	shit diff >patch.0 &&
	shit checkout file1 &&
	test_must_fail shit apply --reject --3way patch.0 &&
	shit diff --exit-code
'

test_expect_success 'apply without --reject should fail' '

	test_must_fail shit apply patch.1 &&
	test_cmp file1 saved.file1
'

test_expect_success 'apply without --reject should fail' '

	test_must_fail shit apply --verbose patch.1 &&
	test_cmp file1 saved.file1
'
...
test_expect_success 'apply with --reject should fail but update the file' '
	...
	test_must_fail shit apply --reject patch.1 &&
...
test_expect_success 'apply with --reject should fail but update the file' '
	...
	test_must_fail shit apply --reject patch.2 >rejects &&
...
test_expect_success 'the same test with --verbose' '
	...
	test_must_fail shit apply --reject --verbose patch.2 >rejects &&
	test_path_is_missing file1 &&
	test_cmp expected file2 &&
...
test_expect_success 'apply cleanly with --verbose' '
	...
	shit cat-file -p HEAD:file1 >file1 &&
	rm -f file?.rej file2 &&

	shit apply --verbose patch.1 &&
