...
test_description='shit p4 filetype tests'
...
test_expect_success 'ensure blobs store only lf newlines' '
	...
	(
		cd "$shit" &&
		shit init &&
		shit p4 sync //depot@all &&

		# verify the files in .shit are stored only with newlines
		o=$(shit ls-tree p4/master -- f-unix | cut -f1 | cut -d\  -f3) &&
		shit cat-file blob $o >f-unix-blob &&
		test_cmp "$cli"/f-unix-orig f-unix-blob &&

		o=$(shit ls-tree p4/master -- f-win | cut -f1 | cut -d\  -f3) &&
		shit cat-file blob $o >f-win-blob &&
...
test_expect_success 'shitattributes setting eol=lf produces lf newlines' '
	test_when_finished cleanup_shit &&
	(
		# checkout the files and make sure core.eol works as planned
		cd "$shit" &&
		shit init &&
		echo "* eol=lf" >.shitattributes &&
		shit p4 sync //depot@all &&
		shit checkout -b master p4/master &&
		test_cmp "$cli"/f-unix-orig f-unix &&
		test_cmp "$cli"/f-win-as-lf f-win
	)
'

test_expect_success 'shitattributes setting eol=crlf produces crlf newlines' '
	test_when_finished cleanup_shit &&
	(
		# checkout the files and make sure core.eol works as planned
		cd "$shit" &&
		shit init &&
		echo "* eol=crlf" >.shitattributes &&
		shit p4 sync //depot@all &&
		shit checkout -b master p4/master &&
...
test_expect_success 'keyword file test' '
	...
	(
		...
		# utf16, even though p4 expands keywords, shit p4 does not
		# try to undo that
...
test_expect_success SYMLINKS 'empty symlink target' '
	...
	# make sure shit p4 handles it without error
	shit p4 clone --dest="$shit" //depot@all &&
...
test_expect_success SYMLINKS 'utf-8 with and without BOM in text file' '
	...
	(
		cd "$shit" &&
		shit checkout refs/remotes/p4/master &&
