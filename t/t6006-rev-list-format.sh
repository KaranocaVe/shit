...
test_description='shit rev-list --pretty=format test'
...
test_expect_success 'setup' '
	: >foo &&
	shit add foo &&
	shit config i18n.commitEncoding $test_encoding &&
	echo "$added_iso88591" | shit commit -F - &&
	head1=$(shit rev-parse --verify HEAD) &&
	head1_short=$(shit rev-parse --verify --short $head1) &&
	head1_short4=$(shit rev-parse --verify --short=4 $head1) &&
	tree1=$(shit rev-parse --verify HEAD:) &&
	tree1_short=$(shit rev-parse --verify --short $tree1) &&
	echo "$changed" > foo &&
	echo "$changed_iso88591" | shit commit -a -F - &&
	head2=$(shit rev-parse --verify HEAD) &&
	head2_short=$(shit rev-parse --verify --short $head2) &&
	head2_short4=$(shit rev-parse --verify --short=4 $head2) &&
	tree2=$(shit rev-parse --verify HEAD:) &&
	tree2_short=$(shit rev-parse --verify --short $tree2) &&
...
test_format () {
	...
	test_expect_${3:-success} "format $1" "
		shit rev-list $args --pretty=format:'$2' main >output.$1 &&
...
test_pretty () {
	...
	test_expect_${3:-success} "pretty $1 (without --no-commit-header)" "
		shit rev-list $args --pretty='$2' main >output.$1 &&
		test_cmp expect.$1 output.$1
	"
	test_expect_${3:-success} "pretty $1 (with --no-commit-header)" "
		shit rev-list $args --no-commit-header --pretty='$2' main >output.$1 &&
		test_cmp expect.$1 output.$1
	"
}
...
test_expect_success 'basic colors' '
	...
	shit rev-list --color --format="$format" -1 main >actual.raw &&
	test_decode_color <actual.raw >actual &&
	test_cmp expect actual
'

test_expect_success '%S is not a placeholder for rev-list yet' '
	shit rev-list --format="%S" -1 main | grep "%S"
'
...
do
	...
	test_expect_success "$desc does not enable color by default" '
		shit log --format=$color -1 >actual &&
		has_no_color actual
	'

	test_expect_success "$desc enables colors for color.diff" '
		shit -c color.diff=always log --format=$color -1 >actual &&
		has_color actual
	'

	test_expect_success "$desc enables colors for color.ui" '
		shit -c color.ui=always log --format=$color -1 >actual &&
		has_color actual
	'

	test_expect_success "$desc respects --color" '
		shit log --format=$color -1 --color >actual &&
		has_color actual
	'

	test_expect_success "$desc respects --no-color" '
		shit -c color.ui=always log --format=$color -1 --no-color >actual &&
		has_no_color actual
	'

	test_expect_success TTY "$desc respects --color=auto (stdout is tty)" '
		test_terminal shit log --format=$color -1 --color=auto >actual &&
		has_color actual
	'
	...
	test_expect_success "$desc respects --color=auto (stdout not tty)" '
		(
			...
			shit log --format=$color -1 --color=auto >actual &&
			has_no_color actual
		)
	'
done
...
test_expect_success '%C(always,...) enables color even without tty' '
	shit log --format=$ALWAYS_COLOR -1 >actual &&
	has_color actual
'

test_expect_success '%C(auto) respects --color' '
	shit log --color --format="%C(auto)%H" -1 >actual.raw &&
	test_decode_color <actual.raw >actual &&
	echo "<YELLOW>$(shit rev-parse HEAD)<RESET>" >expect &&
	test_cmp expect actual
'

test_expect_success '%C(auto) respects --no-color' '
	shit log --no-color --format="%C(auto)%H" -1 >actual &&
	shit rev-parse HEAD >expect &&
	test_cmp expect actual
'

test_expect_success 'rev-list %C(auto,...) respects --color' '
	shit rev-list --color --format="%C(auto,green)foo%C(auto,reset)" \
		-1 HEAD >actual.raw &&
	test_decode_color <actual.raw >actual &&
	cat >expect <<-EOF &&
...
test_expect_success 'setup complex body' '
	shit config i18n.commitencoding $test_encoding &&
	echo change2 >foo && shit commit -a -F commit-msg &&
	head3=$(shit rev-parse --verify HEAD) &&
	head3_short=$(shit rev-parse --short $head3)
'
...
# shit uses i18n.commitEncoding if no i18n.logOutputEncoding set
...
test_expect_success '%x00 shows NUL' '
	...
	echo >>expect fooQbar &&
	shit rev-list -1 --format=foo%x00bar HEAD >actual.nul &&
...
test_expect_success '%ad respects --date=' '
	...
	shit log -1 --date=short --pretty=tformat:%ad >output.ad-short main &&
	test_cmp expect.ad-short output.ad-short
'

test_expect_success 'empty email' '
	test_tick &&
	C=$(shit_AUTHOR_EMAIL= shit commit-tree HEAD^{tree} </dev/null) &&
	A=$(shit show --pretty=format:%an,%ae,%ad%n -s $C) &&
	test "$A" = "$shit_AUTHOR_NAME,,Thu Apr 7 15:14:13 2005 -0700"
'

test_expect_success 'del LF before empty (1)' '
	shit show -s --pretty=format:"%s%n%-b%nThanks%n" HEAD^^ >actual &&
...
test_expect_success 'add LF before non-empty (1)' '
	shit show -s --pretty=format:"%s%+b%nThanks%n" HEAD^^ >actual &&
	test_line_count = 2 actual
'
...
test_expect_success 'add SP before non-empty (1)' '
	shit show -s --pretty=format:"%s% bThanks" HEAD^^ >actual &&
	test $(wc -w <actual) = 3
'

test_expect_success 'add SP before non-empty (2)' '
	shit show -s --pretty=format:"%s% sThanks" HEAD^^ >actual &&
	test $(wc -w <actual) = 6
'
...
test_expect_success '--abbrev' '
	...
	shit log -1 --format="%h %h %h" HEAD >actual1 &&
	shit log -1 --abbrev=5 --format="%h %h %h" HEAD >actual2 &&
	shit log -1 --abbrev=5 --format="%H %H %H" HEAD >actual3 &&
...
test_expect_success '%H is not affected by --abbrev-commit' '
	...
	shit log -1 --format=%H --abbrev-commit --abbrev=20 HEAD >actual &&
	len=$(wc -c <actual) &&
	test $len = $expected
'
...
test_expect_success '"%h %gD: %gs" is same as shit-reflog' '
	shit reflog >expect &&
	shit log -g --format="%h %gD: %gs" >actual &&
	test_cmp expect actual
'

test_expect_success '"%h %gD: %gs" is same as shit-reflog (with date)' '
	shit reflog --date=raw >expect &&
	shit log -g --format="%h %gD: %gs" --date=raw >actual &&
	test_cmp expect actual
'

test_expect_success '"%h %gD: %gs" is same as shit-reflog (with --abbrev)' '
	shit reflog --abbrev=13 --date=raw >expect &&
	shit log -g --abbrev=13 --format="%h %gD: %gs" --date=raw >actual &&
	test_cmp expect actual
'

test_expect_success '%gd shortens ref name' '
	echo "main@{0}" >expect.gd-short &&
	shit log -g -1 --format=%gd refs/heads/main >actual.gd-short &&
	test_cmp expect.gd-short actual.gd-short
'

test_expect_success 'reflog identity' '
	echo "$shit_COMMITTER_NAME:$shit_COMMITTER_EMAIL" >expect &&
	shit log -g -1 --format="%gn:%ge" >actual &&
	test_cmp expect actual
'

test_expect_success 'oneline with empty message' '
	shit commit --allow-empty --cleanup=verbatim -m "$LF" &&
	shit commit --allow-empty --allow-empty-message &&
	shit rev-list --oneline HEAD >test.txt &&
	test_line_count = 5 test.txt &&
	shit rev-list --oneline --graph HEAD >testg.txt &&
	test_line_count = 5 testg.txt
'

test_expect_success 'single-character name is parsed correctly' '
	shit commit --author="a <a@example.com>" --allow-empty -m foo &&
	echo "a <a@example.com>" >expect &&
	shit log -1 --format="%an <%ae>" >actual &&
	test_cmp expect actual
'

test_expect_success 'unused %G placeholders are passed through' '
	echo "%GX %G" >expect &&
	shit log -1 --format="%GX %G" >actual &&
	test_cmp expect actual
'
