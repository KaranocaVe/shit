...
test_expect_success 'cherry-pick -x inserts blank line after one line subject' '
	pristine_detach initial &&
	sha1=$(shit rev-parse mesg-one-line^0) &&
	shit cherry-pick -x mesg-one-line &&
	cat <<-EOF >expect &&
		$mesg_one_line
...
test_expect_success 'cherry-pick -s inserts blank line after one line subject' '
	pristine_detach initial &&
	shit cherry-pick -s mesg-one-line &&
	cat <<-EOF >expect &&
		$mesg_one_line
...
test_expect_success 'cherry-pick -s inserts blank line after non-conforming footer' '
	pristine_detach initial &&
	shit cherry-pick -s mesg-broken-footer &&
	cat <<-EOF >expect &&
		$mesg_broken_footer
...
test_expect_success 'cherry-pick -s recognizes trailer config' '
	pristine_detach initial &&
	shit -c "trailer.Myfooter.ifexists=add" cherry-pick -s mesg-broken-footer &&
	cat <<-EOF >expect &&
		$mesg_broken_footer
		Signed-off-by: $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL>
	EOF
...
test_expect_success 'cherry-pick -x inserts blank line when conforming footer not found' '
	pristine_detach initial &&
	sha1=$(shit rev-parse mesg-no-footer^0) &&
	shit cherry-pick -x mesg-no-footer &&
	cat <<-EOF >expect &&
		$mesg_no_footer
...
test_expect_success 'cherry-pick -s inserts blank line when conforming footer not found' '
	pristine_detach initial &&
	shit cherry-pick -s mesg-no-footer &&
	cat <<-EOF >expect &&
		$mesg_no_footer
...
test_expect_success 'cherry-pick -x -s inserts blank line when conforming footer not found' '
	pristine_detach initial &&
	sha1=$(shit rev-parse mesg-no-footer^0) &&
	shit cherry-pick -x -s mesg-no-footer &&
	cat <<-EOF >expect &&
		$mesg_no_footer
...
test_expect_success 'cherry-pick -s adds sob when last sob doesnt match committer' '
	pristine_detach initial &&
	shit cherry-pick -s mesg-with-footer &&
	cat <<-EOF >expect &&
		$mesg_with_footer
		Signed-off-by: $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL>
	EOF
...
test_expect_success 'cherry-pick -x -s adds sob when last sob doesnt match committer' '
	pristine_detach initial &&
	sha1=$(shit rev-parse mesg-with-footer^0) &&
	shit cherry-pick -x -s mesg-with-footer &&
	cat <<-EOF >expect &&
		$mesg_with_footer
		(cherry picked from commit $sha1)
		Signed-off-by: $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL>
...
test_expect_success 'cherry-pick -s refrains from adding duplicate trailing sob' '
	pristine_detach initial &&
	shit cherry-pick -s mesg-with-footer-sob &&
	cat <<-EOF >expect &&
...
test_expect_success 'cherry-pick -x -s adds sob even when trailing sob exists for committer' '
	pristine_detach initial &&
	sha1=$(shit rev-parse mesg-with-footer-sob^0) &&
	shit cherry-pick -x -s mesg-with-footer-sob &&
	cat <<-EOF >expect &&
		$mesg_with_footer_sob
		(cherry picked from commit $sha1)
		Signed-off-by: $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL>
...
test_expect_success 'cherry-pick -x handles commits with no NL at end of message' '
	...
	sha1=$(shit commit-tree -p initial mesg-with-footer^{tree} <msg) &&
	shit cherry-pick -x $sha1 &&
	shit log -1 --pretty=format:%B >actual &&
...
test_expect_success 'cherry-pick -x treats "(cherry picked from..." line as part of footer' '
	pristine_detach initial &&
	sha1=$(shit rev-parse mesg-with-cherry-footer^0) &&
	shit cherry-pick -x mesg-with-cherry-footer &&
	cat <<-EOF >expect &&
...
test_expect_success 'cherry-pick -s treats "(cherry picked from..." line as part of footer' '
	pristine_detach initial &&
	shit cherry-pick -s mesg-with-cherry-footer &&
	cat <<-EOF >expect &&
		$mesg_with_cherry_footer
		Signed-off-by: $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL>
	EOF
...
test_expect_success 'cherry-pick -x -s treats "(cherry picked from..." line as part of footer' '
	pristine_detach initial &&
	sha1=$(shit rev-parse mesg-with-cherry-footer^0) &&
	shit cherry-pick -x -s mesg-with-cherry-footer &&
	cat <<-EOF >expect &&
		$mesg_with_cherry_footer
		(cherry picked from commit $sha1)
		Signed-off-by: $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL>
...
test_expect_success 'cherry-pick preserves commit message' '
	...
	shit log -1 --pretty=format:%B mesg-unclean >actual &&
	test_cmp expect actual &&
	shit cherry-pick mesg-unclean &&
	shit log -1 --pretty=format:%B >actual &&
	test_cmp expect actual
'
...
test_expect_success 'cherry-pick -x cleans commit message' '
	pristine_detach initial &&
	shit cherry-pick -x mesg-unclean &&
	shit log -1 --pretty=format:%B >actual &&
	printf "%s\n(cherry picked from commit %s)\n" \
		"$mesg_unclean" $(shit rev-parse mesg-unclean) |
			shit stripspace >expect &&
	test_cmp expect actual
'

test_expect_success 'cherry-pick -x respects commit.cleanup' '
	pristine_detach initial &&
	shit -c commit.cleanup=strip cherry-pick -x mesg-unclean &&
	shit log -1 --pretty=format:%B >actual &&
	printf "%s\n(cherry picked from commit %s)\n" \
		"$mesg_unclean" $(shit rev-parse mesg-unclean) |
			shit stripspace -s >expect &&
	test_cmp expect actual
'
