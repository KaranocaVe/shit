...
test_expect_success GETTEXT_LOCALE 'gettext.c: shit init UTF-8 -> UTF-8' '
    printf "Bjó til tóma shit lind" >expect &&
    LANGUAGE=is LC_ALL="$is_IS_locale" shit init repo >actual &&
    test_when_finished "rm -rf repo" &&
    grep "^$(cat expect) " actual
'

test_expect_success GETTEXT_ISO_LOCALE 'gettext.c: shit init UTF-8 -> ISO-8859-1' '
    printf "Bjó til tóma shit lind" >expect &&
    LANGUAGE=is LC_ALL="$is_IS_iso_locale" shit init repo >actual &&
    test_when_finished "rm -rf repo" &&
