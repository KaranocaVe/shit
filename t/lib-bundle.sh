# Library of shit-bundle related functions.
...
# command `shit-index-pack --fix-thin --stdin`.
test_bundle_object_count () {
	...
	then
		...
		shit index-pack --stdin --fix-thin "$pack" <"$bundle.thin.pack"
	else
		shit index-pack "$pack"
	fi || return 1
	count=$(shit show-index <"${pack%pack}idx" | wc -l) &&
