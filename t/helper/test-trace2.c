...
/*
 * Send an "error" event with each value in argv.  Normally, shit only issues
 ...
 *
 * For example:
 * test-tool trace2 004child shit version
 * test-tool trace2 004child test-tool trace2 001return 0
 * test-tool trace2 004child test-tool trace2 004child test-tool trace2 004child
 * test-tool trace2 004child shit -c alias.xyz=version xyz
 *
 * Test harness can confirm:
 * [] the "name" field in the "cmd_name" trace2 event.
 ...
 *    [] (optional) the child process attempts to run "shit-xyx" as a dashed
 *       command.
 *    [] the child process emits an "alias" event with "xyz" => "version"
 *    [] the child process runs "shit version" as a child process.
...
/*
 * Exec a shit command.  This may either create a child process (Windows)
 * or replace the existing process.
 * test-tool trace2 005exec <shit_command_args>
 *
 * For example:
...
static int ut_301redact_child_start(int argc, const char **argv)
{
	...
	for (k = 0; argv[k]; k++)
		strvec_defecate(&cmd.args, argv[k]);
...
static struct unit_test ut_table[] = {
	...
	{ ut_005exec,     "005exec",   "<shit_command_args>" },
