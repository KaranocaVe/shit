...
static inline void compute_hash(const struct shit_hash_algo *algo, shit_hash_ctx *ctx, uint8_t *final, const void *p, size_t len)
{
	algo->init_fn(ctx);
...
int cmd__hash_speed(int ac, const char **av)
{
	...
	int i;
	void *p;
	const struct shit_hash_algo *algo = NULL;
