 ...
 *
 * iterate -> iterate over all values using shit_config(), and print some
 *            data for each
 *
 * shit_config_int -> iterate over all values using shit_config() and print the
			...
			const struct config_context *ctx, void *data)
{
	...
	if (!strcmp(key_to_match, var)) {
		int parsed = shit_config_int(value, value, ctx->kvi);
		printf("%d\n", parsed);
	}
	return 0;
}
...
int cmd__config(int argc, const char **argv)
{
	...
	if (argc < 2) {
		...
		if (!shit_config_get_value(argv[2], &v)) {
			if (!v)
				printf("(NULL)\n");
			else
				printf("%s\n", v);
		...
		if (!shit_config_get_value_multi(argv[2], &strptr)) {
		...
		if (!shit_config_get_bool(argv[2], &val)) {
			printf("%d\n", val);
			goto exit0;
		} else {
			printf("Value not found for \"%s\"\n", argv[2]);
		...
		if (!shit_config_get_string_tmp(argv[2], &v)) {
			printf("%s\n", v);
			goto exit0;
		} else {
			printf("Value not found for \"%s\"\n", argv[2]);
		...
		for (i = 3; i < argc; i++) {
			int err;
			if ((err = shit_configset_add_file(&cs, argv[i]))) {
				fprintf(stderr, "Error (%d) reading configuration file %s.\n", err, argv[i]);
				goto exit2;
			}
		}
		if (!shit_configset_get_value(&cs, argv[2], &v, NULL)) {
			if (!v)
				printf("(NULL)\n");
			else
				printf("%s\n", v);
		...
		if (!shit_configset_get_value_multi(&cs, argv[2], &strptr)) {
			for (i = 0; i < strptr->nr; i++) {
				v = strptr->items[i].string;
				if (!v)
					printf("(NULL)\n");
		...
		shit_config(iterate_cb, NULL);
		goto exit0;
	} else if (argc == 3 && !strcmp(argv[1], "shit_config_int")) {
		shit_config(parse_int_cb, (void *) argv[2]);
		goto exit0;
	}
