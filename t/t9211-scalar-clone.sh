...
test_expect_success 'creates content in enlistment root' '
	...
	test_path_is_dir $enlistment/src/.shit &&
...
test_expect_success 'partial clone if supported by server' '
	...
	(
		...
		ls .shit/objects/pack/pack-*.promisor >promisorlist &&
...
test_expect_success '--single-branch clones HEAD only' '
	...
	(
		cd $enlistment/src &&
		shit for-each-ref refs/remotes/origin >out &&
...
test_expect_success 'progress without tty' '
	...
	shit_PROGRESS_DELAY=0 scalar clone "file://$(pwd)/to-clone" "$enlistment" 2>stderr &&
