...
test_description='shit p4 rcs keywords'
...
# Compare scrubbed $cli/file to $shit/file, should be same.
#
scrub_k_check () {
	file="$1" &&
	scrub="$TRASH_DIRECTORY/$file" &&
	"$PYTHON_PATH" "$TRASH_DIRECTORY/scrub_k.py" <"$shit/$file" >"$scrub" &&
	! test_cmp "$cli/$file" "$scrub" &&
	test_cmp "$shit/$file" "$scrub" &&
	rm "$scrub"
}
scrub_ko_check () {
	file="$1" &&
	scrub="$TRASH_DIRECTORY/$file" &&
	"$PYTHON_PATH" "$TRASH_DIRECTORY/scrub_ko.py" <"$shit/$file" >"$scrub" &&
	! test_cmp "$cli/$file" "$scrub" &&
	test_cmp "$shit/$file" "$scrub" &&
	rm "$scrub"
}
...
test_expect_success 'edit far away from RCS lines' '
	...
	(
		...
		shit commit -m "filek line7 edit" filek &&
		shit p4 submit &&
		scrub_k_check filek
	)
'
...
test_expect_success 'edit near RCS lines' '
	...
	(
		...
		mv -f filek.tmp filek &&
		shit commit -m "filek line4 edit" filek &&
		shit p4 submit &&
		scrub_k_check filek
	)
'
...
test_expect_success 'edit keyword lines' '
	...
	(
		...
		mv -f filek.tmp filek &&
		shit commit -m "filek remove Revision line" filek &&
		shit p4 submit &&
		scrub_k_check filek
	)
'
...
test_expect_success 'scrub ko files differently' '
	...
	(
		...
		shit commit -m "fileko line4 edit" fileko &&
		shit p4 submit &&
...
# hack; shit p4 submit should do it on its own
...
#
# This might happen only if the shit repo is behind the p4 repo at
...
test_expect_success 'do not scrub plain text' '
	...
	(
		...
		shit commit -m "file_text line4 edit" file_text &&
		(
			cd "$cli" &&
			p4 open file_text &&
		...
		echo s | test_expect_code 1 shit p4 submit &&
		(
...
test_expect_success 'cope with rcs keyword expansion damage' '
	...
	(
		...
		shit add kwfile1.c &&
		shit commit -m "Zap an RCS kw line" &&
		shit p4 submit &&
		shit p4 rebase &&
		shit diff p4/master &&
		shit p4 commit &&
		...
		cd "$shit" &&
		echo "line from shit at the top" | cat - kwfile1.c >kwfile1.c.new &&
		mv kwfile1.c.new kwfile1.c &&
		shit commit -m "Add line in shit at the top" kwfile1.c &&
		shit p4 rebase &&
		shit p4 submit
	)
'
...
test_expect_success 'cope with rcs keyword file deletion' '
	...
	(
		...
		shit rm -f kwdelfile.c &&
		shit commit -m "Delete a file containing RCS keywords" &&
		shit config shit-p4.skipSubmitEdit true &&
		shit config shit-p4.attemptRCSCleanup true &&
		shit p4 submit
	) &&
...
# If you add keywords in shit of the form $Header$ then everything should
# work fine without any special handling.
test_expect_success 'Add keywords in shit which match the default p4 values' '
	test_when_finished cleanup_shit &&
	shit p4 clone --dest="$shit" //depot &&
	(
		cd "$shit" &&
		...
		shit add kwfile1.c &&
		shit commit -m "Adding RCS keywords in shit" &&
		shit config shit-p4.skipSubmitEdit true &&
		shit config shit-p4.attemptRCSCleanup true &&
		shit p4 submit
	) &&
...
# If you add keywords in shit of the form $Header:#1$ then things will fail
# unless shit-p4 takes steps to scrub the *shit* commit.
#
test_expect_failure 'Add keywords in shit which do not match the default p4 values' '
	test_when_finished cleanup_shit &&
	shit p4 clone --dest="$shit" //depot &&
	(
		cd "$shit" &&
