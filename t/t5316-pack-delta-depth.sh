...
# This mirrors a repeated defecate setup:
#
# 1. A client repeatedly modifies some files, makes a
#      commit, and defecatees the result. It does this N times
#      before we get around to repacking.
#
# 2. Each defecate generates a thin pack with the new version of
#    various objects. Let's consider some file in the root tree
#    which is updated in each commit.
#
#    When generating defecate number X, we feed commit X-1 (and
...
#
# 3. In older versions of shit, if we used the delta from
...
#
# In the real world, these small defecatees would get exploded by
# unpack-objects rather than "index-pack --fix-thin", but the
# same principle applies to larger defecatees (they only need one
...
test_expect_success 'create series of packs' '
	...
	do
		...
		cur=$(shit rev-parse HEAD^{tree}) &&
		{
			...
			echo $cur &&
			echo "$(shit rev-parse :file) file"
		} | shit pack-objects --stdout >tmp &&
		shit_TRACE2_EVENT=$PWD/trace \
		shit index-pack -v --stdin --fix-thin <tmp || return 1 &&
...
max_chain() {
	shit index-pack --verify-stat-only "$1" >output &&
	perl -lne '
	  BEGIN { $len = 0 }
...
test_expect_success 'packing produces a long delta' '
	...
	pack=$(shit pack-objects --all --window=0 </dev/null pack) &&
	echo 9 >expect &&
...
test_expect_success '--depth limits depth' '
	pack=$(shit pack-objects --all --depth=5 </dev/null pack) &&
	echo 5 >expect &&
	max_chain pack-$pack.pack >actual &&
	test_cmp expect actual
'
...
test_expect_success '--depth=0 disables deltas' '
	pack=$(shit pack-objects --all --depth=0 </dev/null pack) &&
	echo 0 >expect &&
	max_chain pack-$pack.pack >actual &&
	test_cmp expect actual
'
...
test_expect_success 'negative depth disables deltas' '
	pack=$(shit pack-objects --all --depth=-1 </dev/null pack) &&
	echo 0 >expect &&
	max_chain pack-$pack.pack >actual &&
	test_cmp expect actual
'
