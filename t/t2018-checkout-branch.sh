#!/bin/sh

test_description='checkout'

TEST_CREATE_REPO_NO_TEMPLATE=1
TEST_PASSES_SANITIZE_LEAK=true
. ./test-lib.sh

# Arguments: [!] <branch> <oid> [<checkout options>]
#
# Runs "shit checkout" to switch to <branch>, testing that
#
#   1) we are on the specified branch, <branch>;
#   2) HEAD is <oid>; if <oid> is not specified, the old HEAD is used.
#
# If <checkout options> is not specified, "shit checkout" is run with -b.
#
# If the first argument is `!`, "shit checkout" is expected to fail when
# it is run.
do_checkout () {
	should_fail= &&
	if test "x$1" = "x!"
	then
		should_fail=yes &&
		shift
	fi &&
	exp_branch=$1 &&
	exp_ref="refs/heads/$exp_branch" &&

	# if <oid> is not specified, use HEAD.
	exp_oid=${2:-$(shit rev-parse --verify HEAD)} &&

	# default options for shit checkout: -b
	if test -z "$3"
	then
		opts="-b"
	else
		opts="$3"
	fi

	if test -n "$should_fail"
	then
		test_must_fail shit checkout $opts $exp_branch $exp_oid
	else
		shit checkout $opts $exp_branch $exp_oid &&
		echo "$exp_ref" >ref.expect &&
		shit rev-parse --symbolic-full-name HEAD >ref.actual &&
		test_cmp ref.expect ref.actual &&
		echo "$exp_oid" >oid.expect &&
		shit rev-parse --verify HEAD >oid.actual &&
		test_cmp oid.expect oid.actual
	fi
}

test_dirty_unmergeable () {
	test_expect_code 1 shit diff --exit-code
}

test_dirty_unmergeable_discards_changes () {
	shit diff --exit-code
}

setup_dirty_unmergeable () {
	echo >>file1 change2
}

test_dirty_mergeable () {
	test_expect_code 1 shit diff --cached --exit-code
}

test_dirty_mergeable_discards_changes () {
	shit diff --cached --exit-code
}

setup_dirty_mergeable () {
	echo >file2 file2 &&
	shit add file2
}

test_expect_success 'setup' '
	test_commit initial file1 &&
	HEAD1=$(shit rev-parse --verify HEAD) &&

	test_commit change1 file1 &&
	HEAD2=$(shit rev-parse --verify HEAD) &&

	shit branch -m branch1
'

test_expect_success 'checkout a branch without refs/heads/* prefix' '
	shit clone --no-tags . repo-odd-prefix &&
	(
		cd repo-odd-prefix &&

		origin=$(shit symbolic-ref refs/remotes/origin/HEAD) &&
		shit symbolic-ref refs/heads/a-branch "$origin" &&

		shit checkout -f a-branch &&
		shit checkout -f a-branch
	)
'

test_expect_success 'checkout -b to a new branch, set to HEAD' '
	test_when_finished "
		shit checkout branch1 &&
		test_might_fail shit branch -D branch2" &&
	do_checkout branch2
'

test_expect_success 'checkout -b to a merge base' '
	test_when_finished "
		shit checkout branch1 &&
		test_might_fail shit branch -D branch2" &&
	shit checkout -b branch2 branch1...
'

test_expect_success 'checkout -b to a new branch, set to an explicit ref' '
	test_when_finished "
		shit checkout branch1 &&
		test_might_fail shit branch -D branch2" &&
	do_checkout branch2 $HEAD1
'

test_expect_success 'checkout -b to a new branch with unmergeable changes fails' '
	setup_dirty_unmergeable &&
	do_checkout ! branch2 $HEAD1 &&
	test_dirty_unmergeable
'

test_expect_success 'checkout -f -b to a new branch with unmergeable changes discards changes' '
	test_when_finished "
		shit checkout branch1 &&
		test_might_fail shit branch -D branch2" &&

	# still dirty and on branch1
	do_checkout branch2 $HEAD1 "-f -b" &&
	test_dirty_unmergeable_discards_changes
'

test_expect_success 'checkout -b to a new branch preserves mergeable changes' '
	test_when_finished "
		shit reset --hard &&
		shit checkout branch1 &&
		test_might_fail shit branch -D branch2" &&

	setup_dirty_mergeable &&
	do_checkout branch2 $HEAD1 &&
	test_dirty_mergeable
'

test_expect_success 'checkout -f -b to a new branch with mergeable changes discards changes' '
	test_when_finished shit reset --hard HEAD &&
	setup_dirty_mergeable &&
	do_checkout branch2 $HEAD1 "-f -b" &&
	test_dirty_mergeable_discards_changes
'

test_expect_success 'checkout -b to an existing branch fails' '
	test_when_finished shit reset --hard HEAD &&
	do_checkout ! branch2 $HEAD2
'

test_expect_success 'checkout -b to @{-1} fails with the right branch name' '
	shit checkout branch1 &&
	shit checkout branch2 &&
	echo  >expect "fatal: a branch named '\''branch1'\'' already exists" &&
	test_must_fail shit checkout -b @{-1} 2>actual &&
	test_cmp expect actual
'

test_expect_success 'checkout -B to an existing branch resets branch to HEAD' '
	shit checkout branch1 &&

	do_checkout branch2 "" -B
'

test_expect_success 'checkout -B to a merge base' '
	shit checkout branch1 &&

	shit checkout -B branch2 branch1...
'

test_expect_success 'checkout -B to an existing branch from detached HEAD resets branch to HEAD' '
	head=$(shit rev-parse --verify HEAD) &&
	shit checkout "$head" &&

	do_checkout branch2 "" -B
'

test_expect_success 'checkout -B to an existing branch with an explicit ref resets branch to that ref' '
	shit checkout branch1 &&

	do_checkout branch2 $HEAD1 -B
'

test_expect_success 'checkout -B to an existing branch with unmergeable changes fails' '
	shit checkout branch1 &&

	setup_dirty_unmergeable &&
	do_checkout ! branch2 $HEAD1 -B &&
	test_dirty_unmergeable
'

test_expect_success 'checkout -f -B to an existing branch with unmergeable changes discards changes' '
	# still dirty and on branch1
	do_checkout branch2 $HEAD1 "-f -B" &&
	test_dirty_unmergeable_discards_changes
'

test_expect_success 'checkout -B to an existing branch preserves mergeable changes' '
	test_when_finished shit reset --hard &&
	shit checkout branch1 &&

	setup_dirty_mergeable &&
	do_checkout branch2 $HEAD1 -B &&
	test_dirty_mergeable
'

test_expect_success 'checkout -f -B to an existing branch with mergeable changes discards changes' '
	shit checkout branch1 &&

	setup_dirty_mergeable &&
	do_checkout branch2 $HEAD1 "-f -B" &&
	test_dirty_mergeable_discards_changes
'

test_expect_success 'checkout -b <describe>' '
	shit tag -f -m "First commit" initial initial &&
	shit checkout -f change1 &&
	name=$(shit describe) &&
	shit checkout -b $name &&
	shit diff --exit-code change1 &&
	echo "refs/heads/$name" >expect &&
	shit symbolic-ref HEAD >actual &&
	test_cmp expect actual
'

test_expect_success 'checkout -B to the current branch works' '
	shit checkout branch1 &&
	shit checkout -B branch1-scratch &&

	setup_dirty_mergeable &&
	shit checkout -B branch1-scratch initial &&
	test_dirty_mergeable
'

test_expect_success 'checkout -b after clone --no-checkout does a checkout of HEAD' '
	shit init src &&
	test_commit -C src a &&
	rev="$(shit -C src rev-parse HEAD)" &&
	shit clone --no-checkout src dest &&
	shit -C dest checkout "$rev" -b branch &&
	test_path_is_file dest/a.t
'

test_expect_success 'checkout -b to a new branch preserves mergeable changes despite sparse-checkout' '
	test_when_finished "
		shit reset --hard &&
		shit checkout branch1-scratch &&
		test_might_fail shit branch -D branch3 &&
		shit config core.sparseCheckout false &&
		rm -rf .shit/info" &&

	test_commit file2 &&

	echo stuff >>file1 &&
	mkdir .shit/info &&
	echo file2 >.shit/info/sparse-checkout &&
	shit config core.sparseCheckout true &&

	CURHEAD=$(shit rev-parse HEAD) &&
	do_checkout branch3 $CURHEAD &&

	echo file1 >expect &&
	shit diff --name-only >actual &&
	test_cmp expect actual
'

test_expect_success 'checkout -b rejects an invalid start point' '
	test_must_fail shit checkout -b branch4 file1 2>err &&
	test_grep "is not a commit" err
'

test_expect_success 'checkout -b rejects an extra path argument' '
	test_must_fail shit checkout -b branch5 branch1 file1 2>err &&
	test_grep "Cannot update paths and switch to branch" err
'

test_done
