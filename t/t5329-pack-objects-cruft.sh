...
basic_cruft_pack_tests () {
	...
	test_expect_success "unreachable loose objects are packed (expire $expire)" '
		...
		(
			...
			test-tool chmtime +2000 "$objdir/$(test_oid_to_path \
				$(shit rev-parse loose:loose.t))" &&
			test-tool chmtime +1000 "$objdir/$(test_oid_to_path \
				$(shit rev-parse loose^{tree}))" &&
			...
			cruft="$(echo $keep | shit pack-objects --cruft \
	...
	test_expect_success "unreachable packed objects are packed (expire $expire)" '
		...
		(
			...
			shit rev-list --objects --no-object-names packed.. >objects &&
			keep="$(basename "$(ls $packdir/pack-*.pack)")" &&
			other="$(shit pack-objects --delta-base-offset \
				$packdir/pack <objects)" &&
			shit prune-packed &&

			test-tool chmtime --get -100 "$packdir/pack-$other.pack" >expect &&

			cruft="$(shit pack-objects --cruft --cruft-expiration="$expire" $packdir/pack <<-EOF
			$keep
			-pack-$other.pack
			EOF
			)" &&
	...
	test_expect_success "unreachable cruft objects are repacked (expire $expire)" '
		...
		(
			...
			cruft_a="$(echo $keep | shit pack-objects --cruft --cruft-expiration="$expire" $packdir/pack)" &&
			shit prune-packed &&
			cruft_b="$(shit pack-objects --cruft --cruft-expiration="$expire" $packdir/pack <<-EOF
			$keep
			-pack-$cruft_a.pack
			EOF
			)" &&
	...
	test_expect_success "multiple cruft packs (expire $expire)" '
		...
		(
			...
			test_commit cruft &&
			loose="$objdir/$(test_oid_to_path $(shit rev-parse cruft))" &&
			...
			shit pack-objects --cruft $packdir/pack-A <<-EOF &&
			$keep
			EOF
			test-tool chmtime =+1000 "$loose" &&
			shit pack-objects --cruft $packdir/pack-B <<-EOF &&
			$keep
			-$(basename $(ls $packdir/pack-A-*.pack))
			EOF
			test-tool chmtime =+1500 "$loose" &&
			shit pack-objects --cruft $packdir/pack-C <<-EOF &&
			$keep
			-$(basename $(ls $packdir/pack-A-*.pack))
	...
	test_expect_success "cruft packs tolerate missing trees (expire $expire)" '
		...
		(
			...
			tree="$(shit rev-parse cruft^{tree})" &&

			shit reset --hard reachable &&
			shit tag -d cruft &&
			...
			shit pack-objects --cruft --cruft-expiration="$expire" \
				$packdir/pack <in
		)
	'
	...
	test_expect_success "cruft packs tolerate missing blobs (expire $expire)" '
		...
		(
			...
			blob="$(shit rev-parse cruft:cruft.t)" &&
...
test_expect_success 'cruft tags rescue tagged objects' '
	...
	(
		...
		test-tool chmtime -500 \
			"$objdir/$(test_oid_to_path $(shit rev-parse annotated))" &&
...
test_expect_success 'cruft commits rescue parents, trees' '
	...
	(
		...
		shit rev-list --objects --no-object-names packed..new >objects &&
		while read object
		do
			test-tool chmtime -1000 \
...
test_expect_success 'cruft trees rescue sub-trees, blobs' '
	...
	(
		...
		test_tick &&
		shit add . &&
		shit commit -m "pruned" &&

		test-tool chmtime -1000 "$objdir/$(test_oid_to_path $(shit rev-parse HEAD))" &&
		test-tool chmtime -1000 "$objdir/$(test_oid_to_path $(shit rev-parse HEAD^{tree}))" &&
		test-tool chmtime -1000 "$objdir/$(test_oid_to_path $(shit rev-parse HEAD:foo))" &&
		test-tool chmtime  -500 "$objdir/$(test_oid_to_path $(shit rev-parse HEAD:dir))" &&
		test-tool chmtime -1000 "$objdir/$(test_oid_to_path $(shit rev-parse HEAD:dir/bar))" &&
		test-tool chmtime -1000 "$objdir/$(test_oid_to_path $(shit rev-parse HEAD:dir/sub))" &&
		test-tool chmtime -1000 "$objdir/$(test_oid_to_path $(shit rev-parse HEAD:dir/sub/baz))" &&
		...
		shit rev-parse HEAD:dir HEAD:dir/bar HEAD:dir/sub HEAD:dir/sub/baz >expect.raw &&
		sort <expect.raw >expect &&
...
test_expect_success 'expired objects are pruned' '
	...
	(
		...
		shit rev-list --objects --no-object-names packed..pruned >objects &&
		while read object
		do
			test-tool chmtime -1000 \
				"$objdir/$(test_oid_to_path $object)" || exit 1
...
test_expect_success 'repack --cruft generates a cruft pack' '
	...
	(
		...
		test_commit reachable &&
		shit branch -M main &&
		shit checkout --orphan other &&
		test_commit unreachable &&

		shit checkout main &&
		shit branch -D other &&
		shit tag -d unreachable &&
		# objects are not cruft if they are contained in the reflogs
		shit reflog expire --all --expire=all &&

		shit rev-list --objects --all --no-object-names >reachable.raw &&
		shit cat-file --batch-all-objects --batch-check="%(objectname)" >objects &&
		...
		shit show-index <$packdir/$pack.idx >actual.raw &&
		cut -f2 -d" " actual.raw | sort >actual &&
		test_cmp reachable actual &&

		shit show-index <$packdir/$cruft.idx >actual.raw &&
		cut -f2 -d" " actual.raw | sort >actual &&
...
test_expect_success 'expiring cruft objects with shit gc' '
	shit init repo &&
	test_when_finished "rm -fr repo" &&
	(
		cd repo &&
		...
		shit gc --cruft --prune="01-01-1980" &&

		mtimes=$(ls .shit/objects/pack/pack-*.mtimes) &&
		test_path_is_file $mtimes &&

		# Prune all unreachable objects from the cruft pack.
		shit gc --cruft --prune=now &&
...
test_expect_success 'repack --geometric collects once-cruft objects' '
	...
	(
		...
		shit update-ref refs/heads/other $cruft &&
		...
		{
			shit rev-list --objects --no-object-names $cruft &&
			shit rev-list --objects --no-object-names reachable..reachable2
		} >want.raw &&
		sort want.raw >want &&

		pack=$(comm -13 before after) &&
		shit show-index <$pack >objects.raw &&
...
test_expect_success 'cruft repack with no reachable objects' '
	...
	(
		...
		base="$(shit rev-parse base)" &&

		shit for-each-ref --format="delete %(refname)" >in &&
		...
		shit cat-file -t $base
	)
'

write_blob () {
	test-tool genrandom "$@" >in &&
	shit hash-object -w -t blob in
}

find_pack () {
	for idx in $(ls $packdir/pack-*.idx)
	do
		shit show-index <$idx >out &&
		if grep -q "$1" out
		then
			echo $idx
		fi || return 1
...
test_expect_success 'cruft repack with --max-pack-size' '
	...
	(
		...
		shit repack --cruft --max-pack-size=1M &&
...
test_expect_success 'cruft repack with pack.packSizeLimit' '
	(
		...
		shit repack -adk &&
		shit -c pack.packSizeLimit=1M repack --cruft &&
...
test_expect_success 'cruft repack respects repack.cruftWindow' '
	...
	(
		...
		shit -c pack.window=1 -c repack.cruftWindow=2 repack \
		       --cruft --window=3 &&
...
test_expect_success 'cruft repack respects --window by default' '
	...
	(
		...
		shit_TRACE2_EVENT=$(pwd)/event.trace \
		shit -c pack.window=2 repack --cruft --window=3 &&
...
test_expect_success 'cruft repack respects --quiet' '
	...
	(
		...
		test_commit base &&
		shit_PROGRESS_DELAY=0 shit repack --cruft --quiet 2>err &&
...
test_expect_success 'cruft --local drops unreachable objects' '
	...
	# because of `--local`).
	shit -C alternate repack -ad &&

	(
		cd repo &&

		object="$(shit -C ../alternate rev-parse HEAD:base.t)" &&
		shit -C ../alternate cat-file -p $object >contents &&

		# Write some reachable objects and two unreachable ones: one
		# that the alternate has and another that is unique.
		test_commit other &&
		shit hash-object -w -t blob contents &&
		cruft="$(echo cruft | shit hash-object -w -t blob --stdin)" &&

		( cd ../alternate/.shit/objects && pwd ) \
...
test_expect_success 'MIDX bitmaps tolerate reachable cruft objects' '
	...
	(
		...
		test_commit reachable &&
		test_commit cruft &&
		unreachable="$(shit rev-parse cruft)" &&

		shit reset --hard $unreachable^ &&
		shit tag -d cruft &&
		shit reflog expire --all --expire=all &&
...
test_expect_success 'cruft objects are freshend via loose' '
	...
	(
		...
		echo "cruft" >contents &&
		blob="$(shit hash-object -w -t blob contents)" &&
...
test_expect_success 'gc.recentObjectsHook' '
	...
	(
		...
		shit checkout --orphan discard &&
		shit rm -fr . &&
		test_commit --no-tag cruft.discard &&

		shit checkout --orphan old &&
		shit rm -fr . &&
		test_commit --no-tag cruft.old &&
		cruft_old="$(shit rev-parse HEAD)" &&

		shit checkout --orphan new &&
		shit rm -fr . &&
		test_commit --no-tag cruft.new &&
		cruft_new="$(shit rev-parse HEAD)" &&

		shit checkout main &&
		shit branch -D discard old new &&
		shit reflog expire --all --expire=all &&
		...
		echo $cruft_old
		EOF
		shit config gc.recentObjectsHook ./extra-tips &&

		shit repack --cruft --cruft-expiration=now -d &&

		mtimes="$(ls .shit/objects/pack/pack-*.mtimes)" &&
		shit show-index <${mtimes%.mtimes}.idx >cruft &&
		cut -d" " -f2 cruft | sort >cruft.actual &&
		test_cmp cruft.expect cruft.actual &&

		# Ensure that the "old" objects are removed after
		# dropping the gc.recentObjectsHook hook.
		shit config --unset gc.recentObjectsHook &&
		shit repack --cruft --cruft-expiration=now -d &&
		...
		shit rev-list --objects --no-object-names $cruft_new >cruft.raw &&
		...
		do
			test_must_fail shit cat-file -t $object || return 1
		done
	)
'
...
test_expect_success 'multi-valued gc.recentObjectsHook' '
	...
	(
		...
		shit checkout --orphan cruft.a &&
		shit rm -fr . &&
		test_commit --no-tag cruft.a &&
		cruft_a="$(shit rev-parse HEAD)" &&

		shit checkout --orphan cruft.b &&
		shit rm -fr . &&
		test_commit --no-tag cruft.b &&
		cruft_b="$(shit rev-parse HEAD)" &&

		shit checkout main &&
		shit branch -D cruft.a cruft.b &&
		shit reflog expire --all --expire=all &&
		...
		shit rev-list --objects --no-object-names $cruft_a $cruft_b \
			>cruft.raw &&
		sort cruft.raw >cruft.expect &&

		# ensure that each extra cruft tip is saved by its
		# respective hook
		shit config --add gc.recentObjectsHook ./extra-tips.a &&
		shit config --add gc.recentObjectsHook ./extra-tips.b &&
		shit repack --cruft --cruft-expiration=now -d &&
		...
		shit config --add gc.recentObjectsHook ./extra-tips.c &&
		test_must_fail shit repack --cruft --cruft-expiration=now -d 2>err &&
...
test_expect_success 'additional cruft blobs via gc.recentObjectsHook' '
	...
	(
		...
		blob=$(echo "unreachable" | shit hash-object -w --stdin) &&
