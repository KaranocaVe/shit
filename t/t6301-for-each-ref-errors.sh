...
test_expect_success setup '
	...
	shit tag testtag &&
	shit for-each-ref >full-list &&
	shit for-each-ref --format="%(objectname) %(refname)" >brief-list
'

test_expect_success REFFILES 'Broken refs are reported correctly' '
	r=refs/heads/bogus &&
	: >.shit/$r &&
	test_when_finished "rm -f .shit/$r" &&
	echo "warning: ignoring broken ref $r" >broken-err &&
	shit for-each-ref >out 2>err &&
...
test_expect_success REFFILES 'NULL_SHA1 refs are reported correctly' '
	...
	shit for-each-ref --format="%(objectname) %(refname)" >brief-out 2>brief-err &&
	test_cmp brief-list brief-out &&
	test_cmp zeros-err brief-err
'

test_expect_success 'Missing objects are reported correctly' '
	test_when_finished "shit update-ref -d refs/heads/missing" &&
	...
	test_must_fail shit for-each-ref 2>err &&
