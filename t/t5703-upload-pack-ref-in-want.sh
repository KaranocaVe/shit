...
get_actual_commits () {
	...
	shit index-pack o.pack &&
	shit verify-pack -v o.idx >objs &&
...
# have $(shit rev-parse a)
# EOF
#
# write_fetch_command <<-EOF
# want $(shit rev-parse b)
# have $(shit rev-parse a)
# EOF
#
write_fetch_command () {
...
test_expect_success 'setup repository' '
	test_commit a &&
	shit branch -M main &&
	shit checkout -b o/foo &&
	test_commit b &&
	test_commit c &&
	shit checkout -b o/bar b &&
	test_commit d &&
	shit checkout -b baz a &&
	test_commit e &&
	shit checkout main &&
	test_commit f
'
...
test_expect_success 'config controls ref-in-want advertisement' '
	...
	shit config uploadpack.allowRefInWant false &&
	test-tool serve-v2 --advertise-capabilities >out &&
	perl -ne "/ref-in-want/ and print" out >out.filter &&
	test_must_be_empty out.filter &&

	shit config uploadpack.allowRefInWant true &&
	test-tool serve-v2 --advertise-capabilities >out &&
	perl -ne "/ref-in-want/ and print" out >out.filter &&
...
test_expect_success 'basic want-ref' '
	oid=$(shit rev-parse f) &&
	cat >expected_refs <<-EOF &&
	$oid refs/heads/main
	EOF
	shit rev-parse f >expected_commits &&
...
test_expect_success 'multiple want-ref lines' '
	oid_c=$(shit rev-parse c) &&
	oid_d=$(shit rev-parse d) &&
	cat >expected_refs <<-EOF &&
	$oid_c refs/heads/o/foo
	$oid_d refs/heads/o/bar
	EOF
	shit rev-parse c d >expected_commits &&

	write_fetch_command >pkt <<-EOF &&
	want-ref refs/heads/o/foo
	want-ref refs/heads/o/bar
	have $(shit rev-parse b)
	EOF
	test-tool pkt-line pack <pkt >in &&
...
test_expect_success 'mix want and want-ref' '
	...
	$oid refs/heads/main
	EOF
	shit rev-parse e f >expected_commits &&

	write_fetch_command >pkt <<-EOF &&
	want-ref refs/heads/main
	want $(shit rev-parse e)
	have $(shit rev-parse a)
	EOF
	test-tool pkt-line pack <pkt >in &&
...
test_expect_success 'setup repos for fetching with ref-in-want tests' '
	...
	shit -C "$REPO" config uploadpack.allowRefInWant true &&
	shit -C "$LOCAL_PRISTINE" config protocol.version 2
'
...
test_expect_success 'fetching with exact OID' '
	...
	oid=$(shit -C "$REPO" rev-parse d) &&
	shit_TRACE_PACKET="$(pwd)/log" shit_TRACE2_EVENT="$(pwd)/trace2" \
		shit -C local fetch origin \
		"$oid":refs/heads/actual &&

	grep \"key\":\"total_rounds\",\"value\":\"2\" trace2 &&
	shit -C "$REPO" rev-parse "d" >expected &&
	shit -C local rev-parse refs/heads/actual >actual &&
...
test_expect_success 'fetching multiple refs' '
	...
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	shit_TRACE_PACKET="$(pwd)/log" shit -C local fetch origin main baz &&

	shit -C "$REPO" rev-parse "main" "baz" >expected &&
	shit -C local rev-parse refs/remotes/origin/main refs/remotes/origin/baz >actual &&
	test_cmp expected actual &&
...
test_expect_success 'fetching ref and exact OID' '
	...
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	oid=$(shit -C "$REPO" rev-parse b) &&
	shit_TRACE_PACKET="$(pwd)/log" shit -C local fetch origin \
		main "$oid":refs/heads/actual &&

	shit -C "$REPO" rev-parse "main" "b" >expected &&
	shit -C local rev-parse refs/remotes/origin/main refs/heads/actual >actual &&
	test_cmp expected actual &&
	grep "want $oid" log &&
	grep "want-ref refs/heads/main" log
'
...
test_expect_success 'fetching with wildcard that does not match any refs' '
	...
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	shit -C local fetch origin refs/heads/none*:refs/heads/* >out &&
...
test_expect_success 'fetching with wildcard that matches multiple refs' '
	...
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	shit_TRACE_PACKET="$(pwd)/log" shit -C local fetch origin refs/heads/o*:refs/heads/o* &&

	shit -C "$REPO" rev-parse "o/foo" "o/bar" >expected &&
	shit -C local rev-parse "o/foo" "o/bar" >actual &&
	test_cmp expected actual &&
...
test_expect_success 'setup namespaced repo' '
	(
		...
		shit checkout a &&
		test_commit d &&
		shit update-ref refs/heads/ns-no b &&
		shit update-ref refs/namespaces/ns/refs/heads/ns-yes c &&
		shit update-ref refs/namespaces/ns/refs/heads/hidden d
	) &&
	shit -C "$REPO" config uploadpack.allowRefInWant true
'
...
test_expect_success 'with namespace: want-ref is considered relative to namespace' '
	...
	oid=$(shit -C "$REPO" rev-parse "refs/namespaces/ns/$wanted_ref") &&
	cat >expected_refs <<-EOF &&
	$oid $wanted_ref
	EOF
	cat >expected_commits <<-EOF &&
	$oid
	$(shit -C "$REPO" rev-parse a)
	EOF
	...
	shit_NAMESPACE=ns test-tool -C "$REPO" serve-v2 --stateless-rpc >out <in &&
	check_output
'
...
test_expect_success 'with namespace: want-ref outside namespace is unknown' '
	...
	test_must_fail env shit_NAMESPACE=ns \
...
test_expect_success 'without namespace: want-ref outside namespace succeeds' '
	...
	oid=$(shit -C "$REPO" rev-parse $wanted_ref) &&
	cat >expected_refs <<-EOF &&
	$oid $wanted_ref
	EOF
	cat >expected_commits <<-EOF &&
...
test_expect_success 'with namespace: hideRefs is matched, relative to namespace' '
	...
	shit -C "$REPO" config transfer.hideRefs $wanted_ref &&
...
test_expect_success 'with namespace: want-ref succeeds if hideRefs is removed' '
	wanted_ref=refs/heads/hidden &&
	shit -C "$REPO" config --unset transfer.hideRefs $wanted_ref &&
...
test_expect_success 'without namespace: relative hideRefs does not match' '
	...
	shit -C "$REPO" config transfer.hideRefs refs/heads/hidden &&
...
test_expect_success 'setup repos for change-while-negotiating test' '
	(
		...
		# more than 1 request/response pair)
		rm -rf "$LOCAL_PRISTINE" &&
		shit clone "http://127.0.0.1:$LIB_HTTPD_PORT/smart/repo" "$LOCAL_PRISTINE" &&
		cd "$LOCAL_PRISTINE" &&
		shit checkout -b side &&
		test_commit_bulk --id=s 33 &&
	...
	shit -C "$LOCAL_PRISTINE" remote set-url origin "http://127.0.0.1:$LIB_HTTPD_PORT/one_time_perl/repo" &&
	shit -C "$LOCAL_PRISTINE" config protocol.version 2
'
...
inconsistency () {
	...
	oid1=$(shit -C "$REPO" rev-parse $1) &&
	oid2=$(shit -C "$REPO" rev-parse $2) &&
	echo "s/$oid1/$oid2/" >"$HTTPD_ROOT_PATH/one-time-perl"
}

test_expect_success 'server is initially ahead - no ref in want' '
	shit -C "$REPO" config uploadpack.allowRefInWant false &&
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	inconsistency main $(test_oid numeric) &&
	test_must_fail shit -C local fetch 2>err &&
...
test_expect_success 'server is initially ahead - ref in want' '
	...
	shit -C "$REPO" rev-parse --verify main >expected &&
	shit -C local rev-parse --verify refs/remotes/origin/main >actual &&
	test_cmp expected actual
'
