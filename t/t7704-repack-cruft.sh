...
test_expect_success '--expire-to stores pruned objects (now)' '
	...
	(
		...
		shit checkout -b cruft &&
		test_commit --no-tag cruft &&

		shit rev-list --objects --no-object-names main..cruft >moved.raw &&
		sort moved.raw >moved.want &&

		shit rev-list --all --objects --no-object-names >expect.raw &&
		...
		shit init --bare expired.shit &&
		shit repack -d \
			--cruft --cruft-expiration="now" \
			--expire-to="expired.shit/objects/pack/pack" &&

		expired="$(ls expired.shit/objects/pack/pack-*.idx)" &&
		test_path_is_file "${expired%.idx}.mtimes" &&

		# Since the `--cruft-expiration` is "now", the effective
		# behavior is to move _all_ unreachable objects out to
		# the location in `--expire-to`.
		shit show-index <$expired >expired.raw &&
		cut -d" " -f2 expired.raw | sort >expired.objects &&
		shit rev-list --all --objects --no-object-names \
			>remaining.objects &&

		# ...in other words, the combined contents of this
		# repository and expired.shit should be the same as the
		# set of objects we started with.
		sort expired.objects remaining.objects >actual &&
		test_cmp expect actual &&

		# The "moved" objects (i.e., those in expired.shit)
...
test_expect_success '--expire-to stores pruned objects (5.minutes.ago)' '
	shit init expire-to-5.minutes.ago &&
	(
		...
		do
			shit checkout -b $kind main &&
			test_commit --no-tag $kind || return 1
		done &&

		shit rev-list --objects --no-object-names main..stale >in &&
		stale="$(shit pack-objects $objdir/pack/pack <in)" &&
		mtime="$(test-tool chmtime --get =-600 $objdir/pack/pack-$stale.pack)" &&

		# expect holds the set of objects we expect to find in
		# this repository after repacking
		shit rev-list --objects --no-object-names recent >expect.raw &&
		sort expect.raw >expect &&

		# moved.want holds the set of objects we expect to find
		# in expired.shit
		shit rev-list --objects --no-object-names main..stale >out &&
		sort out >moved.want &&

		shit checkout main &&
		shit branch -D stale recent &&
		shit reflog expire --all --expire=all &&
		shit prune-packed &&
		...
		shit cat-file --batch-all-objects --batch-check="%(objectname)" \
			>remaining.objects &&
		...
		(
			...
			# expected mtime as were moved to expired.shit.
			#
...
generate_random_blob() {
	...
	shit hash-object -w -t blob blob &&
	rm blob
}

pack_random_blob () {
	generate_random_blob "$@" &&
	shit repack -d -q >/dev/null
}
...
generate_cruft_pack () {
	...
	pack="$(shit pack-objects --cruft $packdir/pack <in)" &&
	shit prune-packed &&
...
test_expect_success '--max-cruft-size creates new packs when above threshold' '
	shit init max-cruft-size-large &&
	(
		...
		shit repack --cruft -d --max-cruft-size=1M &&
...
test_expect_success '--max-cruft-size combines existing packs when below threshold' '
	shit init max-cruft-size-small &&
	(
		...
		bar="$(pack_random_blob bar $((1*1024*1024)))" &&
		shit repack --cruft -d --max-cruft-size=10M &&
...
test_expect_success '--max-cruft-size combines smaller packs first' '
	shit init max-cruft-size-consume-small &&
	(
		...
		shit repack -d --cruft --max-cruft-size=2M &&
...
test_expect_success 'setup --max-cruft-size with freshened objects' '
	shit init max-cruft-size-freshen &&
	(
...
test_expect_success '--max-cruft-size with freshened objects (packed)' '
	(
		...
		foo_pack="$(printf "%s\n" $foo $bar | shit pack-objects $packdir/pack)" &&
		shit prune-packed &&
...
test_expect_success '--max-cruft-size with pruning' '
	shit init max-cruft-size-prune &&
	(
		...
		shit repack -d --cruft --max-cruft-size=1M &&
		...
		test_must_fail shit cat-file -e $foo &&
		shit cat-file -e $bar &&
		shit cat-file -e $baz
	)
'
...
test_expect_success '--max-cruft-size ignores non-local packs' '
	...
	shit clone --reference=$repo $repo $repo-alt &&
	(
		cd $repo-alt &&
		...
		# crash
		shit repack --cruft --max-cruft-size=1M -d
	)
'
...
test_expect_success 'reachable packs are preferred over cruft ones' '
	...
	(
		...
		# accordingly.
		sane_unset shit_TEST_MULTI_PACK_INDEX &&

		test_commit base &&
		test_commit --no-tag cruft &&

		non_cruft="$(echo base | shit pack-objects --revs $packdir/pack)" &&
		...
		cruft="$(echo pack-$non_cruft.pack | shit pack-objects --cruft $packdir/pack-A)" &&
		test-tool chmtime -1000 $packdir/pack-A-$cruft.pack &&

		test_commit other &&
		shit repack -d &&

		shit repack --geometric 2 -d --write-midx --write-bitmap-index &&
