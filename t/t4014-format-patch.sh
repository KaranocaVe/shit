...
test_expect_success setup '
	...
	test_chmod +x elif &&
	test_tick &&
	shit commit -m "Side changes #1" &&

	test_write_lines D E F >>file &&
	shit update-index file &&
	test_tick &&
	shit commit -m "Side changes #2" &&
	shit tag C2 &&

	test_write_lines 5 6 1 2 3 A 4 B C 7 8 9 10 D E F >file &&
	shit update-index file &&
	test_tick &&
	shit commit -m "Side changes #3 with \\n backslash-n in it." &&

	shit checkout main &&
	shit diff-tree -p C2 >patch &&
	shit apply --index <patch &&
	test_tick &&
	shit commit -m "Main accepts moral equivalent of #2" &&

	shit checkout side &&
	shit checkout -b patchid &&
	...
	shit add file file2 file3 &&
	test_tick &&
	shit commit -m "patchid 1" &&
	test_write_lines 4 A B 7 8 9 10 >file2 &&
	test_write_lines 8 9 10 5 6 >file3 &&
	shit add file2 file3 &&
	test_tick &&
	shit commit -m "patchid 2" &&
	test_write_lines 10 5 6 >file &&
	shit add file &&
	test_tick &&
	shit commit -m "patchid 3" &&

	shit checkout -b empty main &&
	test_tick &&
...
test_expect_success 'format-patch --ignore-if-in-upstream' '
	shit format-patch --stdout main..side >patch0 &&
	grep "^From " patch0 >from0 &&
	test_line_count = 3 from0
'

test_expect_success 'format-patch --ignore-if-in-upstream' '
	shit format-patch --stdout \
...
test_expect_success 'format-patch --ignore-if-in-upstream handles tags' '
	shit tag -a v1 -m tag side &&
	shit tag -a v2 -m tag main &&
	shit format-patch --stdout --ignore-if-in-upstream v2..v1 >patch1 &&
	grep "^From " patch1 >from1 &&
	test_line_count = 2 from1
'

test_expect_success "format-patch doesn't consider merge commits" '
	shit checkout -b feature main &&
	echo "Another line" >>file &&
	test_tick &&
	shit commit -am "Feature branch change #1" &&
	echo "Yet another line" >>file &&
	test_tick &&
	shit commit -am "Feature branch change #2" &&
	shit checkout -b merger main &&
	test_tick &&
	shit merge --no-ff feature &&
	shit format-patch -3 --stdout >patch &&
	grep "^From " patch >from &&
	test_line_count = 3 from
'

test_expect_success 'format-patch result applies' '
	shit checkout -b rebuild-0 main &&
	shit am -3 patch0 &&
	shit rev-list main.. >list &&
	test_line_count = 2 list
'

test_expect_success 'format-patch --ignore-if-in-upstream result applies' '
	shit checkout -b rebuild-1 main &&
	shit am -3 patch1 &&
	shit rev-list main.. >list &&
	test_line_count = 2 list
'
...
test_expect_success 'commit did not screw up the log message' '
	shit cat-file commit side >actual &&
...
test_expect_success 'replay did not screw up the log message' '
	shit cat-file commit rebuild-1 >actual &&
	grep "^Side .* with .* backslash-n" actual
'

test_expect_success 'format-patch empty commit' '
	shit format-patch --stdout main..empty >empty &&
	grep "^From " empty >from &&
	test_line_count = 1 from
'

test_expect_success 'extra headers' '
	shit config format.headers "To: R E Cipient <rcipient@example.com>
" &&
	shit config --add format.headers "Cc: S E Cipient <scipient@example.com>
" &&
	shit format-patch --stdout main..side >patch2 &&
...
test_expect_success 'extra headers without newlines' '
	shit config --replace-all format.headers "To: R E Cipient <rcipient@example.com>" &&
	shit config --add format.headers "Cc: S E Cipient <scipient@example.com>" &&
	shit format-patch --stdout main..side >patch3 &&
...
test_expect_success 'extra headers with multiple To:s' '
	shit config --replace-all format.headers "To: R E Cipient <rcipient@example.com>" &&
	shit config --add format.headers "To: S E Cipient <scipient@example.com>" &&
	shit format-patch --stdout main..side >patch4 &&
...
test_expect_success 'additional command line cc (ascii)' '
	shit config --replace-all format.headers "Cc: R E Cipient <rcipient@example.com>" &&
	shit format-patch --cc="S E Cipient <scipient@example.com>" --stdout main..side >patch5 &&
...
test_expect_success 'command line headers' '
	shit config --unset-all format.headers &&
	shit format-patch --add-header="Cc: R E Cipient <rcipient@example.com>" --stdout main..side >patch6 &&
...
test_expect_success 'configuration headers and command line headers' '
	shit config --replace-all format.headers "Cc: R E Cipient <rcipient@example.com>" &&
	shit format-patch --add-header="Cc: S E Cipient <scipient@example.com>" --stdout main..side >patch7 &&
...
test_expect_success 'command line To: header (ascii)' '
	shit config --unset-all format.headers &&
	shit format-patch --to="R E Cipient <rcipient@example.com>" --stdout main..side >patch8 &&
...
test_expect_failure 'command line To: header (rfc2047)' '
	shit format-patch --to="R Ä Cipient <rcipient@example.com>" --stdout main..side >patch8 &&
	sed -e "/^\$/q" patch8 >hdrs8 &&
	grep "^To: =?UTF-8?q?R=20=C3=84=20Cipient?= <rcipient@example.com>\$" hdrs8
'

test_expect_success 'configuration To: header (ascii)' '
	shit config format.to "R E Cipient <rcipient@example.com>" &&
	shit format-patch --stdout main..side >patch9 &&
...
test_expect_failure 'configuration To: header (rfc2047)' '
	shit config format.to "R Ä Cipient <rcipient@example.com>" &&
	shit format-patch --stdout main..side >patch9 &&
	sed -e "/^\$/q" patch9 >hdrs9 &&
...
test_expect_success 'format.from=false' '
	shit -c format.from=false format-patch --stdout main..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch patch &&
...
test_expect_success 'format.from=true' '
	shit -c format.from=true format-patch --stdout main..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
	grep "^From: C O Mitter <committer@example.com>\$" hdrs
'
...
test_expect_success 'format.from with address' '
	shit -c format.from="F R Om <from@example.com>" format-patch --stdout main..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
...
test_expect_success '--no-from overrides format.from' '
	shit -c format.from="F R Om <from@example.com>" format-patch --no-from --stdout main..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
	! grep "^From: F R Om <from@example.com>\$" hdrs
'
...
test_expect_success '--from overrides format.from' '
	shit -c format.from="F R Om <from@example.com>" format-patch --from --stdout main..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
	! grep "^From: F R Om <from@example.com>\$" hdrs
'
...
test_expect_success '--no-to overrides config.to' '
	shit config --replace-all format.to \
		"R E Cipient <rcipient@example.com>" &&
	shit format-patch --no-to --stdout main..side >patch10 &&
...
test_expect_success '--no-to and --to replaces config.to' '
	...
	shit format-patch --no-to --to="Someone Else <else@out.there>" \
...
test_expect_success '--no-cc overrides config.cc' '
	shit config --replace-all format.cc \
		"C E Cipient <rcipient@example.com>" &&
	shit format-patch --no-cc --stdout main..side >patch12 &&
...
test_expect_success '--no-add-header overrides config.headers' '
	shit config --replace-all format.headers \
		"Header1: B E Cipient <rcipient@example.com>" &&
	shit format-patch --no-add-header --stdout main..side >patch13 &&
...
test_expect_success 'multiple files' '
	rm -rf patches/ &&
	shit checkout side &&
	shit format-patch -o patches/ main &&
...
test_expect_success 'filename length limit' '
	...
	for len in 15 25 35
	do
		shit format-patch --filename-max-length=$len -3 side &&
		max=$(
...
test_expect_success 'filename length limit from config' '
	...
	for len in 15 25 35
	do
		shit -c format.filenameMaxLength=$len format-patch -3 side &&
		max=$(
			for patch in 000[1-9]-*.patch
			do
				echo "$patch" | wc -c || exit 1
...
test_expect_success 'filename limit applies only to basename' '
	...
	for len in 15 25 35
	do
		shit format-patch -o patches --filename-max-length=$len -3 side &&
		max=$(
...
test_expect_success 'reroll count' '
	rm -fr patches &&
	shit format-patch -o patches --cover-letter --reroll-count 4 main..side >list &&
...
test_expect_success 'reroll count (-v)' '
	rm -fr patches &&
	shit format-patch -o patches --cover-letter -v4 main..side >list &&
	! grep -v "^patches/v4-000[0-3]-" list &&
	sed -n -e "/^Subject: /p" $(cat list) >subjects &&
	! grep -v "^Subject: \[PATCH v4 [0-3]/3\] " subjects
'
...
test_expect_success 'reroll count (-v) with a fractional number' '
	rm -fr patches &&
	shit format-patch -o patches --cover-letter -v4.4 main..side >list &&
...
test_expect_success 'reroll (-v) count with a non number' '
	rm -fr patches &&
	shit format-patch -o patches --cover-letter -v4rev2 main..side >list &&
...
test_expect_success 'reroll (-v) count with a non-pathname character' '
	rm -fr patches &&
	shit format-patch -o patches --cover-letter -v4---..././../--1/.2//  main..side >list &&
...
check_threading () {
	expect="$1" &&
	shift &&
	shit format-patch --stdout "$@" >patch &&
...
test_expect_success 'excessive subject' '
	...
	shit update-index file &&
	shit commit -m "This is an excessively long subject line for a message due to the habit some projects have of not having a short, one-line subject at the start of the commit message, but rather sticking a whole paragraph right at the start as the only thing in the commit message. It had better not become the filename for the patch." &&
	shit format-patch -o patches/ main..side &&
...
test_expect_success 'failure to write cover-letter aborts gracefully' '
	...
	test_must_fail shit format-patch --no-renames --cover-letter -1
'

test_expect_success 'cover-letter inherits diff options' '
	shit mv file foo &&
	shit commit -m foo &&
	shit format-patch --no-renames --cover-letter -1 &&
	check_patch 0000-cover-letter.patch &&
	! grep "file => foo .* 0 *\$" 0000-cover-letter.patch &&
	shit format-patch --cover-letter -1 -M &&
	grep "file => foo .* 0 *\$" 0000-cover-letter.patch
'
...
test_expect_success 'shortlog of cover-letter wraps overly-long onelines' '
	shit format-patch --cover-letter -2 &&
...
test_expect_success 'format-patch from a subdirectory (3)' '
	...
	filename=$(
		...
		mkdir -p sub/dir &&
		cd sub/dir &&
		shit format-patch -1 -o "$TRASH_DIRECTORY"
	) &&
	basename=$(expr "$filename" : ".*/\(.*\)") &&
	test -f "$basename"
'
...
test_expect_success 'format-patch --in-reply-to' '
	shit format-patch -1 --stdout --in-reply-to "baz@foo.bar" >patch8 &&
	grep "^In-Reply-To: <baz@foo.bar>" patch8 &&
	grep "^References: <baz@foo.bar>" patch8
'

test_expect_success 'format-patch --signoff' '
	shit format-patch -1 --signoff --stdout >out &&
	grep "^Signed-off-by: $shit_COMMITTER_NAME <$shit_COMMITTER_EMAIL>" out
'

test_expect_success 'format-patch --notes --signoff' '
	shit notes --ref test add -m "test message" HEAD &&
	shit format-patch -1 --signoff --stdout --notes=test >out &&
...
test_expect_success 'format-patch notes output control' '
	shit notes add -m "notes config message" HEAD &&
	test_when_finished shit notes remove HEAD &&

	shit format-patch -1 --stdout >out &&
	! grep "notes config message" out &&
	shit format-patch -1 --stdout --notes >out &&
	grep "notes config message" out &&
	shit format-patch -1 --stdout --no-notes >out &&
	! grep "notes config message" out &&
	shit format-patch -1 --stdout --notes --no-notes >out &&
	! grep "notes config message" out &&
	shit format-patch -1 --stdout --no-notes --notes >out &&
	grep "notes config message" out &&
...
test_expect_success 'format-patch with multiple notes refs' '
	shit notes --ref note1 add -m "this is note 1" HEAD &&
	test_when_finished shit notes --ref note1 remove HEAD &&
	shit notes --ref note2 add -m "this is note 2" HEAD &&
	test_when_finished shit notes --ref note2 remove HEAD &&

	shit format-patch -1 --stdout >out &&
	! grep "this is note 1" out &&
	! grep "this is note 2" out &&
	shit format-patch -1 --stdout --notes=note1 >out &&
	grep "this is note 1" out &&
	! grep "this is note 2" out &&
	shit format-patch -1 --stdout --notes=note2 >out &&
	! grep "this is note 1" out &&
	grep "this is note 2" out &&
	shit format-patch -1 --stdout --notes=note1 --notes=note2 >out &&
	grep "this is note 1" out &&
	grep "this is note 2" out &&
	...
	grep "this is note 1" out &&
	grep "this is note 2" out &&
	shit format-patch -1 --stdout --no-notes --notes=note2 >out &&
	! grep "this is note 1" out &&
	grep "this is note 2" out &&

	shit config --add format.notes note2 &&
	shit format-patch -1 --stdout >out &&
	grep "this is note 1" out &&
	grep "this is note 2" out &&
	shit format-patch -1 --stdout --no-notes >out &&
...
test_expect_success 'format-patch with multiple notes refs in config' '
	...
	shit config format.notes note1 &&
	shit format-patch -1 --stdout >out &&
	grep "this is note 1" out &&
	! grep "this is note 2" out &&
	shit config format.notes note2 &&
	shit format-patch -1 --stdout >out &&
	! grep "this is note 1" out &&
	grep "this is note 2" out &&
	shit config --add format.notes note1 &&
	shit format-patch -1 --stdout >out &&
	grep "this is note 1" out &&
	grep "this is note 2" out &&

	shit config --replace-all format.notes note1 &&
	shit config --add format.notes false &&
	shit format-patch -1 --stdout >out &&
	! grep "this is note 1" out &&
	! grep "this is note 2" out &&
	shit config --add format.notes note2 &&
...
test_expect_success 'options no longer allowed for format-patch' '
	test_must_fail shit format-patch --name-only 2>output &&
	test_cmp expect.name-only output &&
	test_must_fail shit format-patch --name-status 2>output &&
	test_cmp expect.name-status output &&
	test_must_fail shit format-patch --check 2>output &&
	test_cmp expect.check output
'

test_expect_success 'format-patch --numstat should produce a patch' '
	shit format-patch --numstat --stdout main..side >output &&
	grep "^diff --shit a/" output >diff &&
	test_line_count = 5 diff
'

test_expect_success 'format-patch -- <path>' '
	rm -f *.patch &&
	shit checkout -b pathspec main &&

	echo file_a 1 >file_a &&
	echo file_b 1 >file_b &&
	shit add file_a file_b &&
	shit commit -m pathspec_initial &&

	echo file_a 2 >>file_a &&
	shit add file_a &&
	shit commit -m pathspec_a &&

	echo file_b 2 >>file_b &&
	shit add file_b &&
	shit commit -m pathspec_b &&

	echo file_a 3 >>file_a &&
	echo file_b 3 >>file_b &&
	shit add file_a file_b &&
	shit commit -m pathspec_ab &&
	...
	shit format-patch main..pathspec -- file_a >output &&
	test_cmp expect output &&
	! grep file_b *.patch
'
...
test_expect_success 'format-patch --ignore-if-in-upstream HEAD' '
	shit checkout side &&
	shit format-patch --ignore-if-in-upstream HEAD
'

test_expect_success 'get shit version' '
	shit_version=$(shit --version) &&
	shit_version=${shit_version##* }
'
...
test_expect_success 'format-patch --signature' '
	shit format-patch --stdout --signature="my sig" -1 >patch &&
	tail -n 3 patch >output &&
...
test_expect_success 'format-patch with format.signature config' '
	shit config format.signature "config sig" &&
...
test_expect_success 'format-patch --signature overrides format.signature' '
	shit config format.signature "config sig" &&
	shit format-patch --stdout --signature="overrides" -1 >output &&
	! grep "config sig" output &&
...
test_expect_success 'format-patch --no-signature ignores format.signature' '
	shit config format.signature "config sig" &&
	shit format-patch --stdout --signature="my sig" --no-signature \
		-1 >output &&
	check_patch output &&
	! grep "config sig" output &&
	! grep "my sig" output &&
...
test_expect_success 'format-patch --signature --cover-letter' '
	shit config --unset-all format.signature &&
	shit format-patch --stdout --signature="my sig" --cover-letter \
		-1 >output &&
...
test_expect_success 'format-patch --no-signature suppresses signatures' '
	shit config --unset-all format.signature &&
	shit format-patch --stdout --no-signature -1 >output &&
	check_patch output &&
	! grep "^-- \$" output
'

test_expect_success 'format-patch --signature="" suppresses signatures' '
	shit format-patch --stdout --signature="" -1 >output &&
	check_patch output &&
	! grep "^-- \$" output
'
...
test_expect_success 'prepare mail-signature input' '
	...
	http://shit.kernel.org/cshit/shit/shit.shit

	shit.kernel.org/?p=shit/shit.shit;a=summary

	EOF
'

test_expect_success '--signature-file=file works' '
	shit format-patch --stdout --signature-file=mail-signature -1 >output &&
	check_patch output &&
...
test_expect_success '--signature overrides format.signaturefile' '
	test_config format.signaturefile mail-signature &&
	shit format-patch --stdout --signature="my sig" -1 >output &&
	check_patch output &&
	grep "my sig" output
'
...
test_expect_success TTY 'format-patch --stdout paginates' '
	rm -f pager_used &&
	test_terminal env shit_PAGER="wc >pager_used" shit format-patch --stdout --all &&
	test_path_is_file pager_used
'

 test_expect_success TTY 'format-patch --stdout pagination can be disabled' '
	rm -f pager_used &&
	test_terminal env shit_PAGER="wc >pager_used" shit --no-pager format-patch --stdout --all &&
	test_terminal env shit_PAGER="wc >pager_used" shit -c "pager.format-patch=false" format-patch --stdout --all &&
	test_path_is_missing pager_used &&
	test_path_is_missing .shit/pager_used
'
...
check_author() {
	echo content >>file &&
	shit add file &&
	shit_AUTHOR_NAME=$1 shit commit -m author-check &&
	shit format-patch --stdout -1 >patch &&
...
test_expect_success 'format-patch wraps extremely long from-header (non-ASCII without Q-encoding)' '
	echo content >>file &&
	shit add file &&
	shit_AUTHOR_NAME="Foö Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar Foo Bar" \
	shit commit -m author-check &&
	shit format-patch --no-encode-email-headers --stdout -1 >patch &&
	sed -n "/^From: /p; /^ /p; /^$/q" patch >actual &&
	test_cmp expect actual
'
...
test_expect_success 'subject lines are unencoded with --no-encode-email-headers' '
	...
	shit add file &&
	shit commit -m "Foö" &&
	shit format-patch --no-encode-email-headers -1 --stdout >patch &&
...
test_expect_success 'subject lines are unencoded with format.encodeEmailHeaders=false' '
	...
	shit add file &&
	shit commit -m "Foö" &&
	shit config format.encodeEmailHeaders false &&
...
test_expect_success '--encode-email-headers overrides format.encodeEmailHeaders' '
	...
	shit commit -m "Foö" &&
	shit config format.encodeEmailHeaders false &&
	shit format-patch --encode-email-headers -1 --stdout >patch &&
	grep ^Subject: patch >actual &&
	test_cmp expect actual
'
...
test_expect_success 'subject lines do not have 822 atom-quoting' '
	echo content >>file &&
	shit add file &&
	shit commit -m "header with . in it" &&
	shit format-patch -k -1 --stdout >patch &&
	grep ^Subject: patch >actual &&
	test_cmp expect actual
'
...
test_expect_success 'subject prefixes have space prepended' '
	shit format-patch -n -1 --stdout --subject-prefix=PREFIX >patch &&
	grep ^Subject: patch >actual &&
	test_cmp expect actual
'
...
test_expect_success 'empty subject prefix does not have extra space' '
	shit format-patch -n -1 --stdout --subject-prefix= >patch &&
	grep ^Subject: patch >actual &&
	test_cmp expect actual
'
...
test_expect_success '--rfc and --no-rfc' '
	...
	EOF
	shit format-patch -n -1 --stdout --rfc >patch &&
	grep "^Subject:" patch >actual &&
	test_cmp expect actual &&
	shit format-patch -n -1 --stdout --rfc --no-rfc >patch &&
...
test_expect_success '--rfc=WIP and --rfc=' '
	...
	EOF
	shit format-patch -n -1 --stdout --rfc=WIP >patch &&
	grep "^Subject:" patch >actual &&
	test_cmp expect actual &&
	shit format-patch -n -1 --stdout --rfc --rfc= >patch &&
...
test_expect_success '--rfc does not overwrite prefix' '
	...
	EOF
	shit -c format.subjectPrefix="PATCH foobar" \
...
test_expect_success '--rfc is argument order independent' '
	...
	Subject: [RFC PATCH foobar 1/1] header with . in it
	EOF
	shit format-patch -n -1 --stdout --rfc \
...
test_expect_success '--subject-prefix="<non-empty>" and -k cannot be used together' '
	...
	test_must_fail shit format-patch -1 --stdout --subject-prefix="MYPREFIX" -k >actual.out 2>actual.err &&
...
test_expect_success '--subject-prefix="" and -k cannot be used together' '
	echo "fatal: options '\''--subject-prefix/--rfc'\'' and '\''-k'\'' cannot be used together" >expect.err &&
	test_must_fail shit format-patch -1 --stdout --subject-prefix="" -k >actual.out 2>actual.err &&
	test_must_be_empty actual.out &&
	test_cmp expect.err actual.err
'
...
test_expect_success '--rfc and -k cannot be used together' '
	echo "fatal: options '\''--subject-prefix/--rfc'\'' and '\''-k'\'' cannot be used together" >expect.err &&
	test_must_fail shit format-patch -1 --stdout --rfc -k >actual.out 2>actual.err &&
	test_must_be_empty actual.out &&
	test_cmp expect.err actual.err
'

test_expect_success '--from=ident notices bogus ident' '
	test_must_fail shit format-patch -1 --stdout --from=foo >patch
'

test_expect_success '--from=ident replaces author' '
	shit format-patch -1 --stdout --from="Me <me@example.com>" >patch &&
	cat >expect <<-\EOF &&
...
test_expect_success '--from uses committer ident' '
	shit format-patch -1 --stdout --from >patch &&
	cat >expect <<-\EOF &&
...
test_expect_success '--from omits redundant in-body header' '
	shit format-patch -1 --stdout --from="A U Thor <author@example.com>" >patch &&
	cat >expect <<-\EOF &&
	From: A U Thor <author@example.com>
...
test_expect_success 'with --force-in-body-from, redundant in-body from is kept' '
	shit format-patch --force-in-body-from \
...
test_expect_success 'format.forceInBodyFrom, equivalent to --force-in-body-from' '
	shit -c format.forceInBodyFrom=yes format-patch \
		-1 --stdout --from="A U Thor <author@example.com>" >patch &&
	cat >expect <<-\EOF &&
	From: A U Thor <author@example.com>
...
test_expect_success 'format.forceInBodyFrom, equivalent to --force-in-body-from' '
	shit -c format.forceInBodyFrom=yes format-patch --no-force-in-body-from \
		-1 --stdout --from="A U Thor <author@example.com>" >patch &&
	cat >expect <<-\EOF &&
	From: A U Thor <author@example.com>
...
test_expect_success 'in-body headers trigger content encoding' '
	test_env shit_AUTHOR_NAME="éxötìc" test_commit exotic &&
...
append_signoff()
{
	C=$(shit commit-tree HEAD^^{tree} -p HEAD) &&
	shit format-patch --stdout --signoff $C^..$C >append_signoff.patch &&
...
test_expect_success 'format patch ignores color.ui' '
	...
	shit format-patch --stdout -1 >expect &&
	test_config color.ui always &&
	shit format-patch --stdout -1 >actual &&
	test_cmp expect actual
'
...
test_expect_success 'format patch respects diff.relative' '
	...
	shit add subdir/file2 &&
	shit commit -F msg &&
	test_unconfig diff.relative &&
	shit format-patch --relative=subdir --stdout -1 >expect &&
	test_config diff.relative true &&
	shit -C subdir format-patch --stdout -1 >actual &&
	test_cmp expect actual
'
...
body" &&
	test_must_fail shit format-patch --cover-letter --cover-from-description garbage main &&
	test_config format.coverFromDescription garbage &&
	test_must_fail shit format-patch --cover-letter main
'
...
body" &&
	...
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter main >actual &&
...
body" &&
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter --cover-from-description default main >actual &&
	grep "^Subject: \[PATCH 0/2\] \*\*\* SUBJECT HERE \*\*\*$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	grep "^config subject$" actual &&
	grep "^body$" actual
...
body" &&
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter --cover-from-description none main >actual &&
	grep "^Subject: \[PATCH 0/2\] \*\*\* SUBJECT HERE \*\*\*$" actual &&
	grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	! grep "^config subject$" actual &&
	! grep "^body$" actual
...
body" &&
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter --cover-from-description message main >actual &&
	grep "^Subject: \[PATCH 0/2\] \*\*\* SUBJECT HERE \*\*\*$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	grep "^config subject$" actual &&
	grep "^body$" actual
...
body" &&
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter --cover-from-description subject main >actual &&
	grep "^Subject: \[PATCH 0/2\] config subject$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	! grep "^config subject$" actual &&
	grep "^body$" actual
...
body" &&
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter --cover-from-description subject --encode-email-headers main >actual &&
...
body" &&
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter --cover-from-description auto main >actual &&
	grep "^Subject: \[PATCH 0/2\] config subject$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	! grep "^config subject$" actual &&
	grep "^body$" actual
...
test_expect_success 'cover letter using branch description (2)' '
	shit checkout rebuild-1 &&
	test_config branch.rebuild-1.description hello &&
	shit format-patch --stdout --cover-letter rebuild-1~2..rebuild-1 >actual &&
	grep hello actual
'
...
test_expect_success 'cover letter using branch description (3)' '
	shit checkout rebuild-1 &&
	test_config branch.rebuild-1.description hello &&
	shit format-patch --stdout --cover-letter ^main rebuild-1 >actual &&
	grep hello actual
'
...
test_expect_success 'cover letter using branch description (5)' '
	shit checkout rebuild-1 &&
	test_config branch.rebuild-1.description hello &&
	shit format-patch --stdout --cover-letter -2 HEAD >actual &&
	grep hello actual
'
...
test_expect_success 'cover letter using branch description (6)' '
	shit checkout rebuild-1 &&
	test_config branch.rebuild-1.description hello &&
	shit format-patch --stdout --cover-letter -2 >actual &&
	grep hello actual
'
...
test_expect_success 'cover letter with --description-file' '
	...
	EOF
	shit checkout rebuild-1 &&
	shit format-patch --stdout --cover-letter --cover-from-description auto \
...
test_expect_success 'cover letter with nothing' '
	shit format-patch --stdout --cover-letter >actual &&
...
test_expect_success 'cover letter auto' '
	...
	test_when_finished "rm -rf tmp;
		shit config --unset format.coverletter" &&

	shit config format.coverletter auto &&
	shit format-patch -o tmp -1 >list &&
	test_line_count = 1 list &&
	shit format-patch -o tmp -2 >list &&
...
test_expect_success 'cover letter auto user override' '
	...
	shit config format.coverletter auto &&
	shit format-patch -o tmp --cover-letter -1 >list &&
	test_line_count = 2 list &&
	shit format-patch -o tmp --cover-letter -2 >list &&
	test_line_count = 3 list &&
	shit format-patch -o tmp --no-cover-letter -1 >list &&
	test_line_count = 1 list &&
	shit format-patch -o tmp --no-cover-letter -2 >list &&
	test_line_count = 2 list
'

test_expect_success 'format-patch --zero-commit' '
	shit format-patch --zero-commit --stdout v2..v1 >patch2 &&
...
test_expect_success 'From line has expected format' '
	shit format-patch --stdout v2..v1 >patch2 &&
...
test_expect_success 'format-patch -o with no leading directories' '
	rm -fr patches &&
	shit format-patch -o patches main..side &&
	count=$(shit rev-list --count main..side) &&
	ls patches >list &&
...
test_expect_success 'format-patch -o with leading existing directories' '
	rm -rf existing-dir &&
	mkdir existing-dir &&
	shit format-patch -o existing-dir/patches main..side &&
	count=$(shit rev-list --count main..side) &&
...
test_expect_success 'format-patch -o with leading non-existing directories' '
	...
	shit format-patch -o non-existing-dir/patches main..side &&
	count=$(shit rev-list --count main..side) &&
...
test_expect_success 'format-patch -o overrides format.outputDirectory' '
	...
	shit format-patch main..side -o patchset &&
...
test_expect_success 'format-patch forbids multiple outputs' '
	...
	test_must_fail \
		shit format-patch --stdout --output-directory=outdir &&
	test_must_fail \
		shit format-patch --stdout --output=outfile &&
	test_must_fail \
		shit format-patch --output=outfile --output-directory=outdir
'
...
test_expect_success 'configured outdir does not conflict with output options' '
	...
	shit format-patch --output=outfile &&
	test_path_is_missing outdir
'

test_expect_success 'format-patch --output' '
	rm -fr outfile &&
	shit format-patch -3 --stdout HEAD >expect &&
	shit format-patch -3 --output=outfile HEAD &&
	test_cmp expect outfile
'

test_expect_success 'format-patch --cover-letter --output' '
	rm -fr outfile &&
	shit format-patch --cover-letter -3 --stdout HEAD >expect &&
	shit format-patch --cover-letter -3 --output=outfile HEAD &&
	test_cmp expect outfile
'

test_expect_success 'format-patch --base' '
	shit checkout patchid &&

	shit format-patch --stdout --base=HEAD~3 -1 >patch &&
	tail -n 7 patch >actual1 &&

	shit format-patch --stdout --base=HEAD~3 HEAD~.. >patch &&
	tail -n 7 patch >actual2 &&

	echo >expect &&
	shit rev-parse HEAD~3 >commit-id-base &&
	echo "base-commit: $(cat commit-id-base)" >>expect &&

	shit show --patch HEAD~2 >patch &&
	shit patch-id --stable <patch >patch.id.raw &&
	awk "{print \"prerequisite-patch-id:\", \$1}" <patch.id.raw >>expect &&

	shit show --patch HEAD~1 >patch &&
	shit patch-id --stable <patch >patch.id.raw &&
	awk "{print \"prerequisite-patch-id:\", \$1}" <patch.id.raw >>expect &&
	...
	shit show --patch HEAD~2 >patch &&
	shit patch-id --unstable <patch >patch.id.raw &&
...
test_expect_success 'format-patch --base errors out when base commit is in revision list' '
	test_must_fail shit format-patch --base=HEAD -2 &&
	test_must_fail shit format-patch --base=HEAD~1 -2 &&
	shit format-patch --stdout --base=HEAD~2 -2 >patch &&
	grep "^base-commit:" patch >actual &&
	shit rev-parse HEAD~2 >commit-id-base &&
	echo "base-commit: $(cat commit-id-base)" >expect &&
	test_cmp expect actual
'
...
test_expect_success 'format-patch --base errors out when base commit is not ancestor of revision list' '
	...
	shit checkout -b topic1 main &&
	shit rev-parse HEAD >commit-id-base &&
	test_commit P &&
	shit rev-parse HEAD >commit-id-P &&
	test_commit Z &&
	shit rev-parse HEAD >commit-id-Z &&
	test_commit Y &&
	shit checkout -b topic2 main &&
	test_commit W &&
	shit merge topic1 &&
	test_commit X &&
	test_must_fail shit format-patch --base=$(cat commit-id-P) -3 &&
	test_must_fail shit format-patch --base=$(cat commit-id-Z) -3 &&
	shit format-patch --stdout --base=$(cat commit-id-base) -3 >patch &&
	grep "^base-commit:" patch >actual &&
	echo "base-commit: $(cat commit-id-base)" >expect &&
	test_cmp expect actual
'
...
test_expect_success 'format-patch --base=auto' '
	shit checkout -b upstream main &&
	shit checkout -b local upstream &&
	shit branch --set-upstream-to=upstream &&
	test_commit N1 &&
	test_commit N2 &&
	shit format-patch --stdout --base=auto -2 >patch &&
	grep "^base-commit:" patch >actual &&
	shit rev-parse upstream >commit-id-base &&
	echo "base-commit: $(cat commit-id-base)" >expect &&
	test_cmp expect actual
'
...
test_expect_success 'format-patch errors out when history involves criss-cross' '
	...
	shit branch --set-upstream-to=xbc &&
	shit merge xb &&
	shit checkout xbc &&
	test_commit D &&
	shit checkout xcb &&
	test_commit E &&
	test_must_fail 	shit format-patch --base=auto -1
'
...
test_expect_success 'format-patch --base overrides format.useAutoBase' '
	test_config format.useAutoBase true &&
	shit format-patch --stdout --base=HEAD~1 -1 >patch &&
	grep "^base-commit:" patch >actual &&
	shit rev-parse HEAD~1 >commit-id-base &&
	echo "base-commit: $(cat commit-id-base)" >expect &&
	test_cmp expect actual
'
...
test_expect_success 'format-patch --no-base overrides format.useAutoBase' '
	test_config format.useAutoBase true &&
	shit format-patch --stdout --no-base -1 >patch &&
	! grep "^base-commit:" patch
'
...
test_expect_success 'format-patch --base with --attach' '
	shit format-patch --attach=mimemime --stdout --base=HEAD~ -1 >patch &&
...
test_expect_success 'format-patch --attach cover-letter only is non-multipart' '
	...
	shit format-patch -o patches --cover-letter --attach=mimemime --base=HEAD~ -1 &&
...
test_expect_success 'format-patch with format.attach' '
	...
	filename=$(shit format-patch -o patches -1) &&
...
test_expect_success '-c format.mboxrd format-patch' '
	...
	C=$(shit commit-tree HEAD^^{tree} -p HEAD <msg) &&
	shit -c format.mboxrd format-patch --stdout -1 $C~1..$C >patch &&
	shit format-patch --pretty=mboxrd --stdout -1 $C~1..$C >compat &&
	test_cmp patch compat &&
	shit grep -h --no-index -A11 \
		"^>From could trip up a loose mbox parser" patch >actual &&
	test_cmp expect actual
'

test_expect_success 'interdiff: setup' '
	shit checkout -b boop main &&
...
test_expect_success 'interdiff: cover-letter' '
	...
	--q
	EOF
	shit format-patch --cover-letter --interdiff=boop~2 -1 boop &&
...
test_expect_success 'interdiff: reroll-count' '
	shit format-patch --cover-letter --interdiff=boop~2 -v2 -1 boop &&
	test_grep "^Interdiff ..* v1:$" v2-0000-cover-letter.patch
'

test_expect_success 'interdiff: reroll-count with a non-integer' '
	shit format-patch --cover-letter --interdiff=boop~2 -v2.2 -1 boop &&
...
test_expect_success 'interdiff: solo-patch' '
	...
	EOF
	shit format-patch --interdiff=boop~2 -1 boop &&
	test_grep "^Interdiff:$" 0001-fleep.patch &&
