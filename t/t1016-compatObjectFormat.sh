...
do
	...
	test_expect_success "setup $hash repository" '
		shit init --object-format=$hash &&
		shit config core.repositoryformatversion 1 &&
		shit config extensions.objectformat $hash &&
		shit config extensions.compatobjectformat $(compat_hash $hash) &&
		shit config gpg.program $TEST_DIRECTORY/t1016/gpg &&
		echo "Hellow World!" > hello &&
		eval hello_${hash}_oid=$(shit hash-object hello) &&
		shit update-index --add hello &&
		shit commit -m "Initial commit" &&
		eval commit_${hash}_oid=$(shit rev-parse HEAD) &&
		eval tree_${hash}_oid=$(shit rev-parse HEAD^{tree})
	'
	test_expect_success "create a $hash  tagged blob" '
		shit tag --no-sign -m "This is a tag" hellotag $(hello_oid $hash) &&
		eval hellotag_${hash}_oid=$(shit rev-parse hellotag)
	'
	test_expect_success "create a $hash tagged tree" '
		shit tag --no-sign -m "This is a tag" treetag $(tree_oid $hash) &&
		eval treetag_${hash}_oid=$(shit rev-parse treetag)
	'
	test_expect_success "create a $hash tagged commit" '
		shit tag --no-sign -m "This is a tag" committag $(commit_oid $hash) &&
		eval committag_${hash}_oid=$(shit rev-parse committag)
	'
	test_expect_success GPG2 "create a $hash signed commit" '
		shit commit --gpg-sign --allow-empty -m "This is a signed commit" &&
		eval signedcommit_${hash}_oid=$(shit rev-parse HEAD)
	'
	test_expect_success GPG2 "create a $hash signed tag" '
		shit tag -s -m "This is a signed tag" signedtag HEAD &&
		eval signedtag_${hash}_oid=$(shit rev-parse signedtag)
	'
	test_expect_success "create a $hash branch" '
		shit checkout -b branch $(commit_oid $hash) &&
		echo "More more more give me more!" > more &&
		eval more_${hash}_oid=$(shit hash-object more) &&
		echo "Another and another and another" > another &&
		eval another_${hash}_oid=$(shit hash-object another) &&
		shit update-index --add more another &&
		shit commit -m "Add more files!" &&
		eval commit2_${hash}_oid=$(shit rev-parse HEAD) &&
		eval tree2_${hash}_oid=$(shit rev-parse HEAD^{tree})
	'
	test_expect_success GPG2 "create another $hash signed tag" '
		shit tag -s -m "This is another signed tag" signedtag2 $(commit2_oid $hash) &&
		eval signedtag2_${hash}_oid=$(shit rev-parse signedtag2)
	'
	test_expect_success GPG2 "merge the $hash branches together" '
		shit merge -S -m "merge some signed tags together" signedtag signedtag2 &&
		eval signedcommit2_${hash}_oid=$(shit rev-parse HEAD)
	'
	test_expect_success GPG2 "create additional $hash signed commits" '
		shit commit --gpg-sign --allow-empty -m "This is an additional signed commit" &&
		shit cat-file commit HEAD | del_sigcommit sha256 > "../${hash}_signedcommit3" &&
		shit cat-file commit HEAD | del_sigcommit sha1 > "../${hash}_signedcommit4" &&
		eval signedcommit3_${hash}_oid=$(shit hash-object -t commit -w ../${hash}_signedcommit3) &&
		eval signedcommit4_${hash}_oid=$(shit hash-object -t commit -w ../${hash}_signedcommit4)
	'
	test_expect_success GPG2 "create additional $hash signed tags" '
		shit tag -s -m "This is an additional signed tag" signedtag34 HEAD &&
		shit cat-file tag signedtag34 | del_sigtag "${hash}" sha256 > ../${hash}_signedtag3 &&
		shit cat-file tag signedtag34 | del_sigtag "${hash}" sha1 > ../${hash}_signedtag4 &&
		eval signedtag3_${hash}_oid=$(shit hash-object -t tag -w ../${hash}_signedtag3) &&
		eval signedtag4_${hash}_oid=$(shit hash-object -t tag -w ../${hash}_signedtag4)
	'
done
cd "$base"
...
compare_oids () {
    ...
    shit --shit-dir=repo-sha1/.shit rev-parse --output-object-format=sha256 ${sha1_oid} > ${name}_sha1_sha256_found
    shit --shit-dir=repo-sha256/.shit rev-parse --output-object-format=sha1 ${sha256_oid} > ${name}_sha256_sha1_found
    local sha1_sha256_oid="$(cat ${name}_sha1_sha256_found)"
    local sha256_sha1_oid="$(cat ${name}_sha256_sha1_found)"

    test_expect_success $PREREQ "Verify ${type} ${name}'s sha1 oid" '
	shit --shit-dir=repo-sha256/.shit rev-parse --output-object-format=sha1 ${sha256_oid} > ${name}_sha1 &&
	test_cmp ${name}_sha1 ${name}_sha1_expected
'

    test_expect_success $PREREQ "Verify ${type} ${name}'s sha256 oid" '
	shit --shit-dir=repo-sha1/.shit rev-parse --output-object-format=sha256 ${sha1_oid} > ${name}_sha256 &&
	test_cmp ${name}_sha256 ${name}_sha256_expected
'

    test_expect_success $PREREQ "Verify ${name}'s sha1 type" '
	shit --shit-dir=repo-sha1/.shit cat-file -t ${sha1_oid} > ${name}_type1 &&
	shit --shit-dir=repo-sha256/.shit cat-file -t ${sha256_sha1_oid} > ${name}_type2 &&
...
'
	...
	shit --shit-dir=repo-sha256/.shit cat-file -t ${sha256_oid} > ${name}_type3 &&
	shit --shit-dir=repo-sha1/.shit cat-file -t ${sha1_sha256_oid} > ${name}_type4 &&
...
'
	...
	shit --shit-dir=repo-sha1/.shit cat-file -s ${sha1_oid} > ${name}_size1 &&
	shit --shit-dir=repo-sha256/.shit cat-file -s ${sha256_sha1_oid} > ${name}_size2 &&
	test_cmp ${name}_size1 ${name}_size2
'

    test_expect_success $PREREQ "Verify ${name}'s sha256 size" '
	shit --shit-dir=repo-sha256/.shit cat-file -s ${sha256_oid} > ${name}_size3 &&
	shit --shit-dir=repo-sha1/.shit cat-file -s ${sha1_sha256_oid} > ${name}_size4 &&
	test_cmp ${name}_size3 ${name}_size4
'

    test_expect_success $PREREQ "Verify ${name}'s sha1 pretty content" '
	shit --shit-dir=repo-sha1/.shit cat-file -p ${sha1_oid} > ${name}_content1 &&
	shit --shit-dir=repo-sha256/.shit cat-file -p ${sha256_sha1_oid} > ${name}_content2 &&
	test_cmp ${name}_content1 ${name}_content2
'

    test_expect_success $PREREQ "Verify ${name}'s sha256 pretty content" '
	shit --shit-dir=repo-sha256/.shit cat-file -p ${sha256_oid} > ${name}_content3 &&
	shit --shit-dir=repo-sha1/.shit cat-file -p ${sha1_sha256_oid} > ${name}_content4 &&
	test_cmp ${name}_content3 ${name}_content4
'

    test_expect_success $PREREQ "Verify ${name}'s sha1 content" '
	shit --shit-dir=repo-sha1/.shit cat-file ${type} ${sha1_oid} > ${name}_content5 &&
	shit --shit-dir=repo-sha256/.shit cat-file ${type} ${sha256_sha1_oid} > ${name}_content6 &&
	test_cmp ${name}_content5 ${name}_content6
'

    test_expect_success $PREREQ "Verify ${name}'s sha256 content" '
	shit --shit-dir=repo-sha256/.shit cat-file ${type} ${sha256_oid} > ${name}_content7 &&
	shit --shit-dir=repo-sha1/.shit cat-file ${type} ${sha1_sha256_oid} > ${name}_content8 &&
