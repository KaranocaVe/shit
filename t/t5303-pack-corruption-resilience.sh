...
create_new_pack() {
    rm -rf .shit &&
    shit init &&
    blob_1=$(shit hash-object -t blob -w file_1) &&
    blob_2=$(shit hash-object -t blob -w file_2) &&
    blob_3=$(shit hash-object -t blob -w file_3) &&
    pack=$(printf "$blob_1\n$blob_2\n$blob_3\n" |
          shit pack-objects $@ .shit/objects/pack/pack) &&
    pack=".shit/objects/pack/pack-${pack}" &&
    shit verify-pack -v ${pack}.pack
}
...
do_corrupt_object() {
    ofs=$(shit show-index < ${pack}.idx | grep $1 | cut -f1 -d" ") &&
    ofs=$(($ofs + $2)) &&
    chmod +w ${pack}.pack &&
    dd of=${pack}.pack bs=1 conv=notrunc seek=$ofs &&
    test_must_fail shit verify-pack ${pack}.pack
}
...
test_expect_success 'initial setup validation' '
	...
	create_new_pack &&
	shit prune-packed &&
	shit cat-file blob $blob_1 > /dev/null &&
	shit cat-file blob $blob_2 > /dev/null &&
	shit cat-file blob $blob_3 > /dev/null
'

test_expect_success 'create corruption in header of first object' '
	do_corrupt_object $blob_1 0 < zero &&
	test_must_fail shit cat-file blob $blob_1 > /dev/null &&
	test_must_fail shit cat-file blob $blob_2 > /dev/null &&
	test_must_fail shit cat-file blob $blob_3 > /dev/null
'

test_expect_success '... but having a loose copy allows for full recovery' '
	mv ${pack}.idx tmp &&
	shit hash-object -t blob -w file_1 &&
	mv tmp ${pack}.idx &&
	shit cat-file blob $blob_1 > /dev/null &&
	shit cat-file blob $blob_2 > /dev/null &&
	shit cat-file blob $blob_3 > /dev/null
...
test_expect_success '... and loose copy of first delta allows for partial recovery' '
	...
	test_must_fail shit cat-file blob $blob_2 > /dev/null &&
	mv ${pack}.idx tmp &&
	shit hash-object -t blob -w file_2 &&
	mv tmp ${pack}.idx &&
	test_must_fail shit cat-file blob $blob_1 > /dev/null &&
	shit cat-file blob $blob_2 > /dev/null &&
	shit cat-file blob $blob_3 > /dev/null
...
test_expect_success '... but having a loose copy allows for full recovery' '
	mv ${pack}.idx tmp &&
	shit hash-object -t blob -w file_3 &&
	mv tmp ${pack}.idx &&
	shit cat-file blob $blob_1 > /dev/null &&
	shit cat-file blob $blob_2 > /dev/null &&
	shit cat-file blob $blob_3 > /dev/null
