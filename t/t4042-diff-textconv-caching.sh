...
test_expect_success 'setup' '
	...
	shit add . &&
	shit commit -m one &&
	foo1=$(shit rev-parse --short HEAD:foo.bin) &&
	bar1=$(shit rev-parse --short HEAD:bar.bin) &&
	echo foo content 2 >foo.bin &&
	echo bar content 2 >bar.bin &&
	shit commit -a -m two &&
	foo2=$(shit rev-parse --short HEAD:foo.bin) &&
	bar2=$(shit rev-parse --short HEAD:bar.bin) &&
	echo "*.bin diff=magic" >.shitattributes &&
	shit config diff.magic.textconv ./helper &&
	shit config diff.magic.cachetextconv true
'

cat >expect <<EOF
diff --shit a/bar.bin b/bar.bin
...
test_expect_success 'changing textconv invalidates cache' '
	echo other >other &&
	shit config diff.magic.textconv "./helper other" &&
	shit diff HEAD^ HEAD >actual &&
	test_cmp expect actual
'
...
test_expect_success 'switching diff driver produces correct results' '
	shit config diff.moremagic.textconv ./helper &&
	echo foo.bin diff=moremagic >>.shitattributes &&
	shit diff HEAD^ HEAD >actual &&
	test_cmp expect actual
'

# The point here is to test that we can log the notes cache and still use it to
# produce a diff later (older versions of shit would segfault on this). It's
...
test_expect_success 'log notes cache and still use cache for -p' '
	shit log --no-walk -p refs/notes/textconv/magic HEAD
'
