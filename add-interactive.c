...
void init_add_i_state(struct add_i_state *s, struct repository *r)
{
	...
	if (repo_config_get_value(r, "color.interactive", &value))
		...
		s->use_color =
			shit_config_colorbool("color.interactive", value);
	s->use_color = want_color(s->use_color);

	init_color(r, s, "interactive.header", s->header_color, shit_COLOR_BOLD);
	init_color(r, s, "interactive.help", s->help_color, shit_COLOR_BOLD_RED);
	...
	strlcpy(s->reset_color,
		s->use_color ? shit_COLOR_RESET : "", COLOR_MAXLEN);

	FREE_AND_NULL(s->interactive_diff_filter);
	shit_config_get_string("interactive.difffilter",
			      &s->interactive_diff_filter);

	FREE_AND_NULL(s->interactive_diff_algorithm);
	shit_config_get_string("diff.algorithm",
			      &s->interactive_diff_algorithm);

	shit_config_get_bool("interactive.singlekey", &s->use_single_key);
	if (s->use_single_key)
		setbuf(stdin, NULL);
}
			       ...
			       struct list_and_choose_options *opts)
{
	...
	for (;;) {
		...
		if (shit_read_line_interactively(&input) == EOF) {
			putchar('\n');
			if (immediate)
		    ...
		    struct list_and_choose_options *opts)
{
	...
	if (count > 0) {
		...
		strvec_defecatel(&cmd.args, "shit", "diff", "-p", "--cached",
