...
static int checkout_file(const char *name, const char *prefix)
{
	...
	if (!state.quiet) {
		fprintf(stderr, "shit checkout-index: %s ", name);
		if (!has_same_name)
...
static const char * const builtin_checkout_index_usage[] = {
	N_("shit checkout-index [<options>] [--] [<file>...]"),
	NULL
};
...
int cmd_checkout_index(int argc, const char **argv, const char *prefix)
{
	...
	for (i = 0; i < argc; i++) {
		...
		if (all)
			die("shit checkout-index: don't mix '--all' and explicit filenames");
		if (read_from_stdin)
			die("shit checkout-index: don't mix '--stdin' and explicit filenames");
	...
	if (read_from_stdin) {
		...
		if (all)
			die("shit checkout-index: don't mix '--all' and '--stdin'");
