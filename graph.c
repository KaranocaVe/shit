 ...
 */
static void graph_padding_line(struct shit_graph *graph, struct strbuf *sb);
 ...
 */
static void graph_show_strbuf(struct shit_graph *graph,
			      FILE *file,
			      struct strbuf const *sb);

/*
 * TODO:
 * - Limit the number of columns, similar to the way shitk does.
...
static void parse_graph_colors_config(struct strvec *colors, const char *string)
{
	...
	while (start < end) {
		...
		if (!color_parse_mem(start, comma - start, color))
			strvec_defecate(colors, color);
		else
			warning(_("ignored invalid color '%.*s' in log.graphColors"),
				(int)(comma - start), start);
		start = comma + 1;
	}
	strvec_defecate(colors, shit_COLOR_RESET);
}
...
struct shit_graph *graph_init(struct rev_info *opt)
{
	struct shit_graph *graph = xmalloc(sizeof(struct shit_graph));

	if (!column_colors) {
		char *string;
		if (shit_config_get_string("log.graphcolors", &string)) {
...
void graph_clear(struct shit_graph *graph)
{
	if (!graph)
		return;
...
static void graph_update_state(struct shit_graph *graph, enum graph_state s)
{
	graph->prev_state = graph->state;
	graph->state = s;
}

static void graph_ensure_capacity(struct shit_graph *graph, int num_columns)
{
 ...
 * and 0 otherwise.
 */
static int graph_is_interesting(struct shit_graph *graph, struct commit *commit)
{
	/*
...
static struct commit_list *next_interesting_parent(struct shit_graph *graph,
...
static struct commit_list *first_interesting_parent(struct shit_graph *graph)
{
...
static unsigned short graph_get_current_column_color(const struct shit_graph *graph)
{
 ...
 */
static void graph_increment_column_color(struct shit_graph *graph)
{
	graph->default_column_color = (graph->default_column_color + 1) %
		column_colors_max;
}

static unsigned short graph_find_commit_color(const struct shit_graph *graph,
...
static int graph_find_new_column_by_commit(struct shit_graph *graph,
					   struct commit *commit)
{
	int i;
...
static void graph_insert_into_new_columns(struct shit_graph *graph,
					  struct commit *commit,
					  int idx)
{
...
static void graph_update_columns(struct shit_graph *graph)
{
...
static int graph_num_dashed_parents(struct shit_graph *graph)
{
	return graph->num_parents + graph->merge_layout - 3;
}

static int graph_num_expansion_rows(struct shit_graph *graph)
{
	/*
...
static int graph_needs_pre_commit_line(struct shit_graph *graph)
{
...
void graph_update(struct shit_graph *graph, struct commit *commit)
{
	struct commit_list *parent;
...
static int graph_is_mapping_correct(struct shit_graph *graph)
{
	int i;
...
static void graph_pad_horizontally(struct shit_graph *graph, struct graph_line *line)
{
	/*
...
static void graph_output_padding_line(struct shit_graph *graph,
...
int graph_width(struct shit_graph *graph)
{
	return graph->width;
}


static void graph_output_skip_line(struct shit_graph *graph, struct graph_line *line)
{
	/*
...
static void graph_output_pre_commit_line(struct shit_graph *graph,
					 struct graph_line *line)
{
	int i, seen_this;
...
static void graph_output_commit_char(struct shit_graph *graph, struct graph_line *line)
{
	/*
 ...
 */
static void graph_draw_octopus_merge(struct shit_graph *graph, struct graph_line *line)
{
	/*
...
static void graph_output_commit_line(struct shit_graph *graph, struct graph_line *line)
{
	int seen_this = 0;
	int i;
...
static void graph_output_post_merge_line(struct shit_graph *graph, struct graph_line *line)
{
	int seen_this = 0;
	int i, j;
...
static void graph_output_collapsing_line(struct shit_graph *graph, struct graph_line *line)
{
	int i;
...
int graph_next_line(struct shit_graph *graph, struct strbuf *sb)
{
...
int graph_is_commit_finished(struct shit_graph const *graph)
{
	return (graph->state == GRAPH_PADDING);
}

void graph_show_commit(struct shit_graph *graph)
{
...
void graph_show_oneline(struct shit_graph *graph)
{
	struct strbuf msgbuf = STRBUF_INIT;
...
void graph_show_padding(struct shit_graph *graph)
{
	struct strbuf msgbuf = STRBUF_INIT;
...
int graph_show_remainder(struct shit_graph *graph)
{
	struct strbuf msgbuf = STRBUF_INIT;
	int shown = 0;
...
void graph_show_commit_msg(struct shit_graph *graph,
			   FILE *file,
			   struct strbuf const *sb)
{
